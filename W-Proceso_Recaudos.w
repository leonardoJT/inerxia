...
DO:
    DO WITH FRAME F_Cre:
        IF INTEGER(SUBSTRING(Cmb_Instancias:SCREEN-VALUE,1,5)) LE 0 OR NOT AVAIL(Consulta) OR W_NitW LE " " THEN
            RETURN.
        ...
        IF AVAIL Instancias THEN DO:
            IF Instancias.Programa NE 0 THEN DO:
                ...
                IF AVAIL Programas THEN DO:
                    ...
                    /* MESSAGE Programas.Ejecutable VIEW-AS ALERT-BOX.*/
                    RUN VALUE(Programas.Ejecutable) (INPUT Cmb_Instancias:SCREEN-VALUE,
                                                     INPUT W_NitCliente:SCREEN-VALUE,
                                                     INPUT Consulta.CodProducto,
                                                     INPUT Consulta.TipProducto,
                                                     INPUT Consulta.Num_Credito,
                                                     INPUT STRING(Instancias.Cod_Operacion,"999999999"),INPUT 1).
        ...
        END.
        ELSE
            MESSAGE "No existe la instancia"
                VIEW-AS ALERT-BOX.
    END.
END.
...
FOR EACH Creditos WHERE Creditos.Nit EQ Clientes.Nit NO-LOCK:
    IF Creditos.Estado EQ 2 THEN DO:
        ...
        IF Creditos.For_Pago EQ 2 THEN
            Consulta.PagaX = "N".
        ELSE
            IF Creditos.For_pago EQ 3 THEN
                Consulta.PagaX = "D".
    END.
...
IF AVAILABLE Castigados THEN DO:
    MESSAGE "El cliente tiene credito castigados" SKIP
            "desea ver el registro de esos creditos?"
        VIEW-AS ALERT-BOX UPDATE pga AS LOGICAL.
