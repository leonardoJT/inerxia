...
DEFINE VARIABLE h_dubicacion AS HANDLE NO-UNDO.
DEFINE VARIABLE h_dynbrowser AS HANDLE NO-UNDO.
DEFINE VARIABLE h_dyntoolbar AS HANDLE NO-UNDO.
DEFINE VARIABLE h_vubicacion AS HANDLE NO-UNDO.
DEFINE VARIABLE h_wfubicaciones AS HANDLE NO-UNDO.
...
DEFINE VARIABLE W_orden AS CHARACTER FORMAT "X(256)":U 
     ...
     LIST-ITEM-PAIRS "Ubicación","Ubicacion",
                     "Tipo","tipo",
                     "Nombre","nombre"
     DROP-DOWN-LIST
     SIZE 24.14 BY .92
...
DO:
  DYNAMIC-FUNCTION('setQuerySort':U IN h_dubicacion, INPUT SELF:SCREEN-VALUE).
  DYNAMIC-FUNCTION('OpenQuery':U IN h_dubicacion).
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE currentPage: 
    ...
    WHEN 0 THEN DO:
       RUN constructObject (
             INPUT  'dubicacion.wDB-AWARE':U ,
             INPUT  FRAME fMain:HANDLE ,
             INPUT  'AppServiceASInfoASUsePrompt?CacheDuration0CheckCurrentChangedyesDestroyStatelessyesDisconnectAppServernoServerOperatingModeNONEShareDatanoUpdateFromSourcenoForeignFieldsObjectNamedubicacionOpenOnInityesPromptColumns(NONE)PromptOnDeleteyesRowsToBatch200RebuildOnReposnoToggleDataTargetsyes':U ,
             OUTPUT h_dubicacion ).
       RUN repositionObject IN h_dubicacion ( 1.81 , 1.00 ) NO-ERROR.
       /* Size in AB:  ( 1.50 , 7.72 ) */

       RUN constructObject (
             INPUT  'wfubicaciones.w':U ,
             INPUT  {&WINDOW-NAME} ,
             INPUT  'LogicalObjectNamePhysicalObjectNameDynamicObjectnoRunAttributeHideOnInityesDisableOnInityesObjectLayout':U ,
             OUTPUT h_wfubicaciones ).
       /* Position in AB:  ( 4.50 , 2.43 ) */
       /* Size in AB:  ( 1.50 , 7.72 ) */

       RUN constructObject (
             INPUT  'vubicacion.w':U ,
             INPUT  FRAME fMain:HANDLE ,
             INPUT  'EnabledObjFldsToDisable?ModifyFields(All)DataSourceNamesUpdateTargetNamesLogicalObjectNameLogicalObjectNamePhysicalObjectNameDynamicObjectnoRunAttributeHideOnInitnoDisableOnInitnoObjectLayout':U ,
             OUTPUT h_vubicacion ).
       RUN repositionObject IN h_vubicacion ( 6.65 , 20.00 ) NO-ERROR.
       /* Size in AB:  ( 11.58 , 76.00 ) */
       ...
       /* Links to SmartDataObject h_dubicacion. */
       RUN addLink ( h_dyntoolbar , 'Navigation':U , h_dubicacion ).
       RUN addLink ( h_wfubicaciones , 'Filter':U , h_dubicacion ).

       /* Links to SmartWindow h_wfubicaciones. */
       RUN addLink ( h_dubicacion , 'Data':U , h_wfubicaciones ).

       /* Links to SmartDataViewer h_vubicacion. */
       RUN addLink ( h_dubicacion , 'Data':U , h_vubicacion ).
       RUN addLink ( h_vubicacion , 'Update':U , h_dubicacion ).
       RUN addLink ( h_dyntoolbar , 'TableIO':U , h_vubicacion ).

       /* Adjust the tab order of the smart objects. */
       RUN adjustTabOrder ( h_vubicacion ,
             B_pagina:HANDLE IN FRAME fMain , 'BEFORE':U ).
    ...
    WHEN 2 THEN DO:
       RUN constructObject (
             ...
             INPUT  FRAME fMain:HANDLE ,
             INPUT  'DisplayedFieldsTipo,Ubicacion,Nombre,Estado,BasuraEnabledFieldsBrowseColumnTypesBrowseColumnItemsBrowseColumnItemPairsBrowseColumnInnerLinesBrowseColumnSortsBrowseColumnMaxCharsBrowseColumnAutoCompletionsBrowseColumnUniqueMatchesBrowseColumnDelimitersScrollRemotenoNumDown0CalcWidthnoMaxWidth80FetchOnReposToEndyesUseSortIndicatoryesSearchFieldTipoDataSourceNamesdubicacionUpdateTargetNames?LogicalObjectNameHideOnInitnoDisableOnInitnoObjectLayout':U ,
             OUTPUT h_dynbrowser ).
       RUN repositionObject IN h_dynbrowser ( 9.88 , 17.00 ) NO-ERROR.
       RUN resizeObject IN h_dynbrowser ( 7.81 , 87.00 ) NO-ERROR.

       /* Links to SmartDataBrowser h_dynbrowser. */
       RUN addLink ( h_dubicacion , 'Data':U , h_dynbrowser ).
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN W_orden:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "ubicacion".
