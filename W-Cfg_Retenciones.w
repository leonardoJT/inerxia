...
&Scoped-define FIELDS-IN-QUERY-brwConceptos cfg_retenciones.concepto ~
...
DEFINE FRAME F-MAIN
     brwConceptos AT ROW 8.54 COL 5 WIDGET-ID 200
...
"cfg_retenciones.concepto" "Concepto" ? "character" ? ? ? 10 ? ? no ? no no "47.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > bdcentral.cfg_retenciones.tipo
"cfg_retenciones.tipo" "Tipo" ? "character" ? ? ? 10 ? ? no ? no no "35.43" yes no no "U" "" "" "DROP-DOWN-LIST" "," ? "Salarios, SAL,Honorarios, HON,Servicios, SRV,Arrendamientos, ARR,Redimientos financieros, RENFIN,Compras, CMP,Impuesto a las ventas, IVA,Industria y Comercio, INDCOM,Otras, OTR" 15 no 0 no no
     _FldNameList[3]   > bdcentral.cfg_retenciones.base
"cfg_retenciones.base" "Base" ? "decimal" ? ? ? 10 ? ? no ? no no "19.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
  ...
  IF AVAIL(cfg_retenciones) THEN DO:
     ...
     FIND FIRST Cuentas WHERE Cuentas.Cuenta = cfg_retenciones.cuenta NO-LOCK NO-ERROR.
