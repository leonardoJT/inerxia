...
/*inicializacion de variables */
    DEFINE TEMP-TABLE tzonas
        FIELD tcodzona  AS INTEGER 
        FIELD tnombre   AS CHARACTER FORMAT "X(20)"
        FIELD tagencia  LIKE  agencias.agencia.
    INPUT FROM blce_zonas.txt.
    REPEAT:
       CREATE Tzonas.
       IMPORT DELIMITER ";" tzonas.
       FIND FIRST agencias WHERE agencias.agencia EQ tagencia NO-LOCK NO-ERROR.
       IF NOT AVAILABLE(agencias) THEN DO:
         MESSAGE "El código de agencia: " Tagencia " No Existe" SKIP
             "Verifiquelo antes de continuar con la importación"
    ...
    END.
    INPUT CLOSE.
    FOR EACH tzonas WHERE tagencia NE ? AND tagencia NE 0 NO-LOCK BREAK BY tcodzona:
      IF FIRST-OF(tzonas.tcodzona) THEN
        W_Ok = Cmb_Agencia:ADD-LAST(STRING(tzonas.tcodzona,"999") + " - " + tzonas.tNombre).
    END.
...
/*fin inicializacion de variables*/
  ...
  DO:
     ...
     DEFINE VAR zcontador AS INTEGER INITIAL 0.
     DEFINE VAR  zfin_age AS INTEGER INITIAL 0.
     FIND LAST agencias NO-LOCK NO-ERROR.
     zfin_age = agencias.agencia.
     w_contodas = TRUE.
     w_codzona = integer(SUBSTRING(Cmb_Agencia:SCREEN-VALUE,1,3)).
