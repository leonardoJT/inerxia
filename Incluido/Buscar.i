...
DEFINE VARIABLE Find_Text AS CHARACTER LABEL "&Buscar"
  FORMAT "x(128)" VIEW-AS FILL-IN SIZE 40 By 1 {&STDPH_FILL} NO-UNDO.
DEFINE VARIABLE Replace_Text AS CHARACTER LABEL "&Remplazar Con"
  FORMAT "x(128)" VIEW-AS FILL-IN SIZE 40 By 1 {&STDPH_FILL} NO-UNDO.

DEFINE VAR Find_Direction AS CHAR LABEL "Direccion" INIT "DOWN"
	VIEW-AS RADIO-SET HORIZONTAL
		RADIO-BUTTONS "&Al Inicio",   "UP",
			       "Al &Final", "DOWN" .
...
DEFINE VARIABLE Find_Filters AS LOGICAL EXTENT 2
  INITIAL ["FALSE","TRUE"]
  LABEL "&Coincidir May/Min.":L20,
        "Ciclo de &Busqueda":L20
...
DEFINE BUTTON btn_Find_Help LABEL "&Ayuda"
    {&STDPH_OKBTN}.
...
def var vDirection AS CHAR LABEL "Direccion" VIEW-AS FILL-IN SIZE 1 BY 1.
...
FORM
    ...
    WITH FRAME FindText 
         VIEW-AS DIALOG-BOX TITLE "Buscar" SIDE-LABELS
...
/*---------------- Replace Dialog Box ----------------*/    
  ...
  FORM
      ...
      Replace_Text &IF "{&WINDOW-SYSTEM}" <> "OSF/Motif"
                   &THEN COLON 18 &ELSE COLON 15 &ENDIF
      SKIP( {&VM_WIDG} )
