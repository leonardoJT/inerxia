...
&Scoped-define FIELDS-IN-QUERY-brwListas cfg_listasSarlaft.concepto ~
cfg_listasSarlaft.tipo cfg_listasSarlaft.base cfg_listasSarlaft.porcentaje 
...
&Scoped-Define ENABLED-OBJECTS Nombre_Cuenta Cuenta_fill Btn_Deshacer ~
Btn-Guardar Btn-Eliminar Tipo_fill Concepto_fill Base_fill Porcentaje_fill ~
Btn-Salvar Btn_Done Btn-Editar RECT-325 
&Scoped-Define DISPLAYED-OBJECTS Nombre_Cuenta Cuenta_fill Tipo_fill ~
Concepto_fill Base_fill Porcentaje_fill 
...
DEFINE VARIABLE Cuenta_fill AS CHARACTER FORMAT "X(15)":U 
     LABEL "Número de cuenta contable" 
     VIEW-AS FILL-IN 
     SIZE 34 BY 1.08 NO-UNDO.

DEFINE VARIABLE Nombre_Cuenta AS CHARACTER FORMAT "X(256)":U 
     LABEL "Nomb. cuenta" 
     VIEW-AS FILL-IN 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brwListas C-Win _STRUCTURED
  QUERY brwListas SHARE-LOCK NO-WAIT DISPLAY
      cfg_listasSarlaft.concepto COLUMN-LABEL "Concepto" FORMAT "x(30)":U
            WIDTH 47.43 LABEL-BGCOLOR 10
      cfg_listasSarlaft.tipo COLUMN-LABEL "Tipo" FORMAT "x(25)":U
            WIDTH 35.43 LABEL-BGCOLOR 10 VIEW-AS COMBO-BOX INNER-LINES 15
                      LIST-ITEM-PAIRS "Restrictiva","RES",
                                      "No restrictiva","NORES"
                      DROP-DOWN-LIST 
      cfg_listasSarlaft.base COLUMN-LABEL "Base" FORMAT ">>>,>>>,>>9.99":U
            WIDTH 19.43 LABEL-BGCOLOR 10
      cfg_listasSarlaft.porcentaje COLUMN-LABEL "Porcentaje" FORMAT ">>9.9999":U
            LABEL-BGCOLOR 10
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 126 BY 19.92 ROW-HEIGHT-CHARS .61 FIT-LAST-COLUMN.


/* ************************  Frame Definitions  *********************** */

DEFINE FRAME F-MAIN
     brwListas AT ROW 8.54 COL 5 WIDGET-ID 200
     Nombre_Cuenta AT ROW 3.42 COL 79 COLON-ALIGNED WIDGET-ID 152
     Cuenta_fill AT ROW 3.42 COL 29 COLON-ALIGNED WIDGET-ID 148
     ...
     Tipo_fill AT ROW 6.65 COL 7.86 COLON-ALIGNED WIDGET-ID 140
     Concepto_fill AT ROW 4.96 COL 12.29 COLON-ALIGNED WIDGET-ID 130
     Base_fill AT ROW 4.88 COL 78.72 COLON-ALIGNED WIDGET-ID 134
     Porcentaje_fill AT ROW 6.65 COL 78.57 COLON-ALIGNED WIDGET-ID 136
     Btn-Salvar AT ROW 3.08 COL 136 WIDGET-ID 42
     Btn_Done AT ROW 27.12 COL 136 WIDGET-ID 38
    ...
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         ...
         AT COL 1 ROW 1
         SIZE 144.72 BY 28.81 WIDGET-ID 100.
...
/* SETTINGS FOR BROWSE brwListas IN FRAME F-MAIN
   NO-ENABLE                                                            */
ASSIGN 
       Nombre_Cuenta:READ-ONLY IN FRAME F-MAIN        = TRUE.
...
/* Query rebuild information for BROWSE brwListas
     ...
     _FldNameList[1]   > bdcentral.cfg_listasSarlaft.concepto
"cfg_listasSarlaft.concepto" "Concepto" ? "character" ? ? ? 10 ? ? no ? no no "47.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > bdcentral.cfg_listasSarlaft.tipo
"cfg_listasSarlaft.tipo" "Tipo" ? "character" ? ? ? 10 ? ? no ? no no "35.43" yes no no "U" "" "" "DROP-DOWN-LIST" "," ? "Salarios, SAL,Honorarios, HON,Servicios, SRV,Arrendamientos, ARR,Redimientos financieros, RENFIN,Compras, CMP,Impuesto a las ventas, IVA,Industria y Comercio, INDCOM,Otras, OTR" 15 no 0 no no
     _FldNameList[3]   > bdcentral.cfg_listasSarlaft.base
"cfg_listasSarlaft.base" "Base" ? "decimal" ? ? ? 10 ? ? no ? no no "19.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > bdcentral.cfg_listasSarlaft.porcentaje
"cfg_listasSarlaft.porcentaje" "Porcentaje" ? "decimal" ? ? ? 10 ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE brwListas */
&ANALYZE-RESUME
...
DO:
  ...
  IF AVAIL(cfg_listasSarlaft) THEN DO:
     ASSIGN Concepto_fill:SCREEN-VALUE = STRING(cfg_listasSarlaft.concepto)
           Tipo_fill:SCREEN-VALUE = STRING(cfg_listasSarlaft.tipo)
           Base_fill:SCREEN-VALUE = STRING(cfg_listasSarlaft.base)
           Porcentaje_fill:SCREEN-VALUE = STRING(cfg_listasSarlaft.porcentaje)
           Cuenta_fill:SCREEN-VALUE = STRING(cfg_listasSarlaft.cuenta).
     FIND FIRST Cuentas WHERE Cuentas.Cuenta = cfg_listasSarlaft.cuenta NO-LOCK NO-ERROR.
     IF AVAIL(Cuentas) THEN DO:
         ASSIGN Nombre_Cuenta:SCREEN-VALUE = Cuentas.Nombre.
     END.
     ASSIGN Btn-Salvar:HIDDEN IN FRAME F-MAIN            = TRUE. 
     ASSIGN Btn-Guardar:HIDDEN IN FRAME F-MAIN           = FALSE.
  END.
...
DO:
    IF AVAIL(cfg_listasSarlaft) THEN DO:
        ...
        MESSAGE "¿Desea cambiar el registro seleccionado?" SKIP
         ...
         UPDATE W_RptaRet AS LOGICAL.
        IF NOT W_RptaRet THEN RETURN.
        ASSIGN cfg_listasSarlaft.concepto = STRING(Concepto_fill:SCREEN-VALUE)
           cfg_listasSarlaft.tipo = STRING(Tipo_fill:SCREEN-VALUE)
           cfg_listasSarlaft.base = dec(Base_fill:SCREEN-VALUE)
           cfg_listasSarlaft.porcentaje = dec(Porcentaje_fill:SCREEN-VALUE)
           cfg_listasSarlaft.cuenta = STRING(Cuenta_fill:SCREEN-VALUE).
...
&Scoped-define SELF-NAME Cuenta_fill
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL Cuenta_fill C-Win
ON LEAVE OF Cuenta_fill IN FRAME F-MAIN /* Número de cuenta contable */
DO:
    FIND FIRST Cuentas WHERE Cuentas.Cuenta = STRING(Cuenta_fill:SCREEN-VALUE)
    ...
    IF NOT AVAIL(Cuentas) THEN DO:
        ...
        FIND FIRST Cuentas WHERE Cuentas.Cuenta = W_Cta
                             AND Cuentas.Tipo = 2
                             AND Cuentas.Estado = 1 NO-LOCK NO-ERROR.
        IF AVAIL(Cuentas) THEN DO:
            ASSIGN Nombre_Cuenta:SCREEN-VALUE = Cuentas.Nombre
                   Cuenta_Fill:SCREEN-VALUE = Cuentas.Cuenta.
        END.
    END.
END.
...
------------------------------------------------------------------------------*/
  DISPLAY Nombre_Cuenta Cuenta_fill Tipo_fill Concepto_fill Base_fill 
          Porcentaje_fill 
      WITH FRAME F-MAIN IN WINDOW C-Win.
  ENABLE Nombre_Cuenta Cuenta_fill Btn_Deshacer Btn-Guardar Btn-Eliminar 
         Tipo_fill Concepto_fill Base_fill Porcentaje_fill Btn-Salvar Btn_Done 
         Btn-Editar RECT-325 
      WITH FRAME F-MAIN IN WINDOW C-Win.
...
DO WITH FRAME f-main:
     ASSIGN Concepto_fill:SCREEN-VALUE = ""
           ...
           Cuenta_fill:SCREEN-VALUE = ""
           Nombre_Cuenta:SCREEN-VALUE = "".
     ASSIGN Btn-Guardar:HIDDEN IN FRAME F-MAIN           = TRUE.
     ASSIGN Btn-Salvar:HIDDEN IN FRAME F-MAIN            = FALSE.
END.
...
DO WITH FRAME f-main:
        IF Concepto_fill:SCREEN-VALUE = "" OR Tipo_fill:SCREEN-VALUE = " " OR dec(Porcentaje_fill:SCREEN-VALUE) = 0 OR Cuenta_fill:SCREEN-VALUE = "" THEN DO:
        ...
        FIND FIRST Cuentas WHERE Cuentas.Cuenta = STRING(Cuenta_fill:SCREEN-VALUE)
                            ...
                            AND Cuentas.Estado EQ 1 NO-LOCK NO-ERROR.
        IF NOT AVAIL(Cuentas) THEN DO:
            MESSAGE "El número de cuenta no existe o no está activo. Verifique."
                VIEW-AS ALERT-BOX INFO BUTTONS OK.
            RETURN ERROR.
        END.        
END.
