...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Rec_XLibranza ASSIGN
         HIDDEN             = YES
         TITLE              = "Multiactiva - Distribución Recaudos Dirigidos - W-Rec_Xprima.r"
         HEIGHT             = 19.08
         WIDTH              = 112.43
         MAX-HEIGHT         = 26.69
         MAX-WIDTH          = 146.29
...
ON END-ERROR OF W-Rec_XLibranza /* Multiactiva - Distribución Recaudos Dirigidos - W-Rec_Xprima.r */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Rec_XLibranza W-Rec_XLibranza
ON WINDOW-CLOSE OF W-Rec_XLibranza /* Multiactiva - Distribución Recaudos Dirigidos - W-Rec_Xprima.r */
DO:
...
/* solo aportes */
  ...
  FOR EACH Ahorros WHERE Ahorros.Nit      EQ W_NitCte AND
      ...
      IF ahorros.tip_ahorro NE  4 THEN NEXT.
  ...
  /********************* MODIF. GCAMACHO ABR/09/08 *********************************/
      IF Ahorros.tip_ahorro EQ 4 AND Ahorros.Cod_ahorro EQ 5 AND Ahorros.estado EQ 1  THEN DO: /*ES Aporte Obligatorio*/
          FIND FIRST indicadores WHERE indicadores.indicador EQ 21 NO-LOCK NO-ERROR. /*buscamos valor salario minimo*/
          IF AVAILABLE indicadores THEN DO:
              IF Ahorros.Sdo_Disponible GE (Indicadores.Valor * 20) THEN DO: /*Aportes Obligatorios mayores a 20 SMMLV*/
                  UPDATE TPdctos.Cuota = 0.
                  FIND FIRST BAhorros WHERE BAhorros.nit EQ Ahorros.Nit AND
                      BAhorros.tip_ahorro EQ 2 AND BAhorros.Cod_ahorro EQ 221 AND 
                      BAhorros.estado EQ 1 NO-LOCK NO-ERROR. /*Buscamos si cliente tiene producto ahorro permanente*/
                  IF AVAILABLE BAhorros THEN DO:
                      IF YEAR(W_Fecha) GT YEAR(BAhorros.Fec_Apertura) OR
                          ((YEAR(W_Fecha) EQ YEAR(BAhorros.Fec_Apertura)) AND (MONTH(W_Fecha) GT MONTH(BAhorros.Fec_Apertura)))
                          THEN DO:
                          FIND FIRST pro_ahorros WHERE pro_ahorros.tip_ahorro EQ BAhorros.tip_ahorro AND 
                                pro_ahorros.cod_ahorro EQ BAhorros.Cod_ahorro NO-LOCK NO-ERROR.
                          CREATE TPdctos.
                          ASSIGN TPdctos.Prior    = Pro_Ahorros.Prioridad
                                 TPdctos.CodP     = BAhorros.Cod_Ahorro
                                 TPdctos.TP       = "A"
                                 TPdctos.Agen     = BAhorros.Agencia
                                 TPdctos.CtaP     = BAhorros.Cue_Ahorro
                                 TPdctos.FApert   = BAhorros.Fec_Apertura
                                 TPdctos.Cuota    = Ahorros.Cuota
                                 TPdctos.CuoOrig  = BAhorros.Cuota
                                 TPdctos.VrSdo    = BAhorros.Sdo_Dispon + BAhorros.Sdo_Canje
                                 TPdctos.VrDist   = 0
                                 TPdctos.NC       = 0
                                 TPdctos.NomP     = Pro_Ahorros.Nom_Produc.                      
                      END.
              ...
              ELSE DO:
                  FIND FIRST BAhorros WHERE BAhorros.nit EQ Ahorros.Nit AND
                      BAhorros.tip_ahorro EQ 2 AND BAhorros.Cod_ahorro EQ 221
                      AND BAhorros.estado EQ 1 NO-LOCK NO-ERROR. /*Buscamos si cliente tiene producto ahorro permanente*/
                  IF AVAILABLE BAhorros THEN DO:
                      IF YEAR(W_Fecha) GT YEAR(BAhorros.Fec_Apertura) OR
                          ((YEAR(W_Fecha) EQ YEAR(BAhorros.Fec_Apertura)) AND (MONTH(W_Fecha) GT MONTH(BAhorros.Fec_Apertura)))
                          THEN DO: /**/
                          FIND FIRST pro_ahorros WHERE pro_ahorros.tip_ahorro EQ BAhorros.tip_ahorro AND 
                                pro_ahorros.cod_ahorro EQ BAhorros.Cod_ahorro NO-LOCK NO-ERROR.
                          ASSIGN viValPmnte = TPdctos.Cuota - BAhorros.Cuota.
                          UPDATE TPdctos.Cuota    = viValPmnte.
  ...
  FOR EACH Creditos WHERE Creditos.Nit         EQ W_NitCte AND
       ...
       IF (creditos.cod_credito NE 545 AND 
           creditos.cod_credito NE 546 AND 
           creditos.cod_credito NE 547 AND 
           creditos.cod_credito NE 020 AND
           creditos.cod_credito NE 548 AND
           creditos.cod_credito NE 550 AND
           creditos.cod_credito NE 552 AND
           creditos.cod_credito NE 554 AND
           creditos.cod_credito NE 555 AND
           creditos.cod_credito NE 557 AND
           creditos.cod_credito NE 558 AND
           creditos.cod_credito NE 561 AND
           creditos.cod_credito NE 569 AND 
           creditos.cod_credito NE 856 AND
           creditos.cod_credito NE 857 AND
           creditos.cod_credito NE 858 AND
           creditos.cod_credito NE 859 AND
           creditos.cod_credito NE 860 AND 
           creditos.cod_credito NE 861) THEN NEXT.

       IF Creditos.Cuota         LE 0 THEN NEXT.
       IF Creditos.Sdo_capital   LE 0 THEN NEXT.
       IF Creditos.Fec_PagAnti   GT w_fecha2 THEN NEXT. /*ASG - 03/03/2008*/
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN W_Reporte    = "Reporte   : Distribución Preliminar de Recaudo  multi    Fecha del Informe: " +
                        STRING(W_Fecha,"99/99/9999") + "      Hora : " + STRING(TIME,"HH:MM:SS")
 ...
 IF W_SiContab THEN DO:
    W_Reporte    = "Reporte   : Distribución Contabilizada de Recaudo multi    Fecha del Informe: " +
                        STRING(W_Fecha,"99/99/9999") + "      Hora : " + STRING(TIME,"HH:MM:SS").
