...
DEFINE VAR tasaSolicitud AS DECIMAL.
...
DEFINE FRAME VisorSolicitud
     ...
     enviarMail AT ROW 18.73 COL 57 WIDGET-ID 16
     "Puede actualizar los datos y volver a generar PDF." VIEW-AS TEXT
          SIZE 47.86 BY .62 AT ROW 18.92 COL 4.57 WIDGET-ID 12
    WITH 1 DOWN KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 2 ROW 3.42
         SIZE 107 BY 19.92
...
/* SETTINGS FOR FRAME F_Cerradas
   NOT-VISIBLE                                                          */
/* BROWSE-TAB Br_Cerradas TEXT-27 F_Cerradas */
ASSIGN 
...
/* SETTINGS FOR FRAME F_ConHV
   NOT-VISIBLE                                                          */
/* BROWSE-TAB Br_ConHV TEXT-20 F_ConHV */
ASSIGN 
...
/* SETTINGS FOR FRAME F_Scoring
   NOT-VISIBLE                                                          */
/* BROWSE-TAB BR_Scoring TEXT-10 F_Scoring */
ASSIGN 
...
&Scoped-define FRAME-NAME VisorSolicitud
&Scoped-define SELF-NAME enviarMail
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL enviarMail wWin
ON CHOOSE OF enviarMail IN FRAME VisorSolicitud /* Enviar a e-mail */
DO:
  RUN mail.r(INPUT Clientes.email,
           ...
           INPUT Usuario.email,
           INPUT "", 
           INPUT "Formato de solicitud de crédito " + Solicitud.Nit,
           INPUT "<img width='200px' src='https://www.fodun.com.co/Files/Logo/logo-fodun-1.png' align='right'/><p>Bogot&aacute;, D.C., " + STRING(TODAY,"99/99/9999") + 
             ...
             Solicitud.Nit + "</p><p>Cordial Saludo,</p><p>FODUN</p><p style='text-align: center;'>www.fodun.com.co</p>",
...
OCXFile = SEARCH( "w-proceso_Solicitud_emailPDFsolicitud.wrx":U ).
IF OCXFile = ? THEN
...
END.
ELSE MESSAGE "w-proceso_Solicitud_emailPDFsolicitud.wrx":U SKIP(1)
...
------------------------------------------------------------------------------*/
  ...
  ENABLE BUTTON-233 Btn_IrProcesarInst enviarMail 
...
/*     ASSIGN Tas_efe                = ((tasa1 * Periodo) * 100)
        ...
        RUN NVEF IN W_ManFin (INPUT(DEC(w_tasanominal:SCREEN-VALUE) / 100),Periodo,OUTPUT tasaSolicitud).
        solicitud.tasa:SCREEN-VALUE = string(tasaSolicitud * 100).
  END.
END PROCEDURE.
