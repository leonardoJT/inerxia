/* 
    NOMBRE      : pMtrizEvntosSarlaft.p
    DESCRIPCION : Genera A Excel Matriz De Eventos
    LOG         : Creado 20 ago 2008, Ing Edilberto Mariño Moya
    ESTRUCTURA DEL MENSAJE DE ENTRADA:
    ENTRADA UNO: FECHA INICIAL
    ENTRADA DOS: FECHA FINAL
*/ 
  
DEF INPUT PARAMETER cMnsje AS CHAR NO-UNDO.

/* borreme */
/* DEF VAR cMnsje AS CHAR NO-UNDO.                 */
/* cMnsje = "01/01/2008" + CHR(1) + STRING(TODAY). */
/* fin borreme */
...
daFchaIni = DATE(ENTRY(1,cMnsje,CHR(1))).
daFchaFin = DATE(ENTRY(2,cMnsje,CHR(1))).
...
PROCEDURE pPrcsa:
    ...
    DO i = 1 TO NUM-ENTRIES(cPrbbldad,","):
        ...
        chExcel:Range(cCoor):FONT:bold = TRUE.
    END.
    FOR EACH saro NO-LOCK
        WHERE
            (IF daFchaIni = ? THEN TRUE ELSE saro.fec_des_even >= daFchaIni)
        AND (IF daFChaFin = ? THEN TRUE ELSE saro.fec_des_even <= daFchaFin),
        EACH pro_saro NO-LOCK
            WHERE
                pro_saro.cod_factor = integer(saro.cod_evento),
        EACH pro_sarlaft NO-LOCK
            WHERE
                pro_sarlaft.cod_factor = integer(pro_saro.cod_factor_sarlaft)
        BREAK
            BY pro_sarlaft.cod_probabilidad
            BY pro_sarlaft.cod_severidad
            BY saro.cod_evento:
        ACCUMULATE saro.cod_evento (COUNT BY saro.cod_evento).
        IF LAST-OF(pro_sarlaft.cod_severidad)
        THEN DO:
            ...
            ftoexcelVlor(cCoor,TRIM(
                         (IF c = ? THEN "" ELSE c) + "," + 
                         saro.cod_evento + "*" + string(ACCUM COUNT BY saro.cod_evento saro.cod_evento),",")).
        END.
        i  = maximum(0,LOOKUP(string(pro_sarlaft.cod_probabilidad),cPrbbldad,",")) + 1.
        j  = maximum(0,LOOKUP(string(pro_sarlaft.cod_severidad),cSvrdad,",")) + 1.
    ...
    ASSIGN i = 1 j = 1.
    cCoor = fcoor(i,j).
    ftoexcelVlor(cCoor,"MATRIZ DE EVENTOS").
    chExcel:Range(cCoor):FONT:bold = TRUE.
    i = 2.
    cCoor = fcoor(i,j).
    ftoexcelVlor(cCoor,"PERIODO:").
    chExcel:Range(cCoor):FONT:bold = TRUE.
    j = 2.
    cCoor = fcoor(i,j).
    ftoexcelVlor(cCoor,string(daFchaIni,"99/99/9999") + " - " + string(daFchaFin,"99/99/9999")).
END PROCEDURE.
