DEFINE VARIABLE W_Informe AS CHARACTER FORMAT "X(50)" INITIAL "P. Y G. COMPARATIVO".
...
PROCEDURE Mayorizar:
    ...
    ASSIGN TSCuentas.TS_SMes = TSCuentas.TS_SMes + SMes
           MMes = /*TSCuentas.TS_SMes*/ SMes
           MNt = Cuentas.Naturaleza.

    IF TSCuentas.TS_Nat NE "CR" THEN
        TotMes = TotMes - SMes.
    ELSE
        TotMes = TotMes + SMes.
...
PROCEDURE Tabla_Temporal:
    ...
    FOR EACH Sal_Cuenta WHERE Sal_Cuenta.Agencia GE W_Ag1
                          ...
                          AND Sal_Cuenta.Ano EQ YEAR(W_Fec1) NO-LOCK:
    ...
    IF YEAR(W_Fec2) NE YEAR(W_Fec1) THEN DO:
        FOR EACH Sal_Cuenta WHERE Sal_Cuenta.Agencia GE W_Ag1
                              ...
                              AND Sal_Cuenta.Cuenta GE W_Cuenta1
                              AND Sal_Cuenta.Cuenta LE W_Cuenta2
                              AND Sal_Cuenta.Ano EQ YEAR(W_Fec2) NO-LOCK:
            RUN Buscar_Cuentas (INPUT Sal_Cuenta.Cuenta).
            IF Cuentas.Id_Cuenta EQ 4 THEN DO:
                W_Com = YES.
...
END PROCEDURE.
  ...
  PROCEDURE ProcesoImprimir:
     {Incluido\RepEncabezado.i}
      ...
      W_Reporte   = "REPORTE   : EXCEDENTES Y/O PERDIDAS - Agencia: " + STRING(Cmb_Agencia:SCREEN-VALUE IN FRAME F_Valida,"X(25)")
      ...
      FOR EACH TSCuentas WHERE TSCuentas.TS_Nivel LE Cmb_Nivel
          ...
          IF TSCuentas.TS_Cuenta EQ "4" THEN
             TotCom = TotCom + TSCuentas.TS_SCom.
          ELSE IF TSCuentas.TS_Cuenta EQ "5" OR TSCuentas.TS_Cuenta EQ "6"
               OR TSCuentas.TS_Cuenta EQ "7" THEN 
                  TotCom = TotCom - TSCuentas.TS_SCom.
      ...
      DISPLAY SKIP(1)
              TotMes AT 44
              TotCom AT 61 
              TotMes - TotCom            AT 79  FORM "->>>,>>>,>>>,>>9"
             (TotMes - TotCom) / TotCom  AT 100 FORM "->>9.99"
