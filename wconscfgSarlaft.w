...
&Scoped-define FRAME-NAME F_FactSarlaft
...
DEFINE FRAME F_FactSarlaft
    ...
    WITH 1 DOWN KEEP-TAB-ORDER OVERLAY 
         ...
         SIZE 66 BY 21.62
         BGCOLOR 17 
         TITLE "Consulta Descripción Estadisticas" WIDGET-ID 100.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW wWin ASSIGN
         HIDDEN             = YES
         TITLE              = "Consulta Descripcion Estadisticas Sarlaft"
         HEIGHT             = 21.38
         WIDTH              = 62.86
         MAX-HEIGHT         = 28.81
         MAX-WIDTH          = 146.14
...
/* SETTINGS FOR FRAME F_FactSarlaft
   FRAME-NAME                                                           */
...
ON END-ERROR OF wWin /* Consulta Descripcion Estadisticas Sarlaft */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL wWin wWin
ON WINDOW-CLOSE OF wWin /* Consulta Descripcion Estadisticas Sarlaft */
DO:
...
ON CHOOSE OF Btn_Terminarcon IN FRAME F_FactSarlaft /* Terminar Consulta */
DO:
...
ON LEAVE OF W_Busca IN FRAME F_FactSarlaft
  ...
  CASE currentPage: 
    ...
    WHEN 0 THEN DO:
       RUN constructObject (
             ...
             INPUT  FRAME F_FactSarlaft:HANDLE ,
       ...
       RUN repositionObject IN h_dcfg_Sarlaft ( 1.00 , 56.00 ) NO-ERROR.
       /* Size in AB:  ( 1.88 , 7.72 ) */

       RUN constructObject (
             INPUT  'adm2/dynbrowser.w':U ,
             INPUT  FRAME F_FactSarlaft:HANDLE ,
             INPUT  'DisplayedFieldsCfg_Nombre,Cod_Nivel,NombreEnabledFieldsBrowseColumnTypesBrowseColumnItemsBrowseColumnItemPairsBrowseColumnInnerLinesBrowseColumnSortsBrowseColumnMaxCharsBrowseColumnAutoCompletionsBrowseColumnUniqueMatchesBrowseColumnDelimitersScrollRemotenoNumDown0CalcWidthnoMaxWidth5FetchOnReposToEndyesUseSortIndicatoryesSearchFieldDataSourceNamesdcfg_SarlaftUpdateTargetNames?LogicalObjectNameHideOnInitnoDisableOnInitnoObjectLayout':U ,
             OUTPUT h_dynbrowser-2 ).
       RUN repositionObject IN h_dynbrowser-2 ( 1.54 , 5.00 ) NO-ERROR.
       RUN resizeObject IN h_dynbrowser-2 ( 14.00 , 52.14 ) NO-ERROR.

       /* Links to SmartDataBrowser h_dynbrowser-2. */
       RUN addLink ( h_dcfg_Sarlaft , 'Data':U , h_dynbrowser-2 ).
       RUN addLink ( h_dynbrowser-2 , 'Update':U , h_dcfg_Sarlaft ).

       /* Adjust the tab order of the smart objects. */
       RUN adjustTabOrder ( h_dynbrowser-2 ,
             R_Busca:HANDLE IN FRAME F_FactSarlaft , 'BEFORE':U ).
    END. /* Page 0 */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Buscar wWin 
PROCEDURE Buscar :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
...
------------------------------------------------------------------------------*/
    IF LASTKEY = KEYCODE("ENTER") THEN DO:
         CASE R_Busca:
             ...
             DYNAMIC-FUNCTION('closeQuery':U IN h_dcfg_Sarlaft).
             DYNAMIC-FUNCTION('setQueryWhere':U IN h_dcfg_Sarlaft,
                 INPUT STRING(cfg_Sarlaft.Cod_Nivel,'999999') = W_Busca:SCREEN-VALUE IN FRAME {&FRAME-NAME}).  
             DYNAMIC-FUNCTION('openQuery':U IN h_dcfg_Sarlaft).
         END.
         WHEN 2 THEN DO:
             /*cerrando los query principal y abriendo uno de la seleccion */
             DYNAMIC-FUNCTION('closeQuery':U IN h_dcfg_Sarlaft).
             DYNAMIC-FUNCTION('setQueryWhere':U IN h_dcfg_Sarlaft,
                 INPUT Cfg_Sarlaft.Nombre = W_Busca:SCREEN-VALUE IN FRAME {&FRAME-NAME}).  
             DYNAMIC-FUNCTION('openQuery':U IN h_dcfg_Sarlaft).  
         END.
         END CASE.
...
------------------------------------------------------------------------------*/
  DISPLAY R_Busca W_Busca 
      WITH FRAME F_FactSarlaft IN WINDOW wWin.
  ENABLE RECT-322 R_Busca W_Busca Btn_Terminarcon 
      WITH FRAME F_FactSarlaft IN WINDOW wWin.
  {&OPEN-BROWSERS-IN-QUERY-F_FactSarlaft}
  VIEW wWin.
END PROCEDURE.
