...
IF T_ConsCtai NE "" THEN DO:
    FIND FIRST Cuentas WHERE Cuentas.Cuenta EQ T_ConsCtai NO-LOCK NO-ERROR.
...
IF T_ConsCta NE "" THEN DO:
    ...
    IF T_ConsCta EQ ? THEN DO:
        FIND FIRST Cuentas WHERE Cuentas.Cuenta EQ T_ConsCta NO-LOCK NO-ERROR NO-WAIT.
...
PROCEDURE Mayorizar:
    ...
    DO i = Cuentas.Nivel TO 1 BY -1:
        ...
        IF Cuentas.Naturaleza NE MNt THEN DO:
            TSCuentas.TS_SMes = TSCuentas.TS_SMes - MMes.

            IF SIni NE 0 THEN
                TSCuentas.TS_SCom = TSCuentas.TS_SCom - SIni.
        END.
        ELSE DO:
            TSCuentas.TS_SMes = TSCuentas.TS_SMes + MMes.
...
PROCEDURE Mayorizar_Comparado:
    ...
    RUN HallarSdo (INPUT MONTH(W_Fec2),
                   OUTPUT SIni,
                   OUTPUT SCom).
    ...
    DO i = Cuentas.Nivel TO 1 BY -1:
        ...
        IF Cuentas.Naturaleza NE MNt THEN
            TSCuentas.TS_SCom = TSCuentas.TS_SCom - MMes.
        ELSE
            TSCuentas.TS_SCom = TSCuentas.TS_SCom + MMes.
    END.
END PROCEDURE.
...
PROCEDURE Buscar_Cuentas:
    ...
    FIND FIRST Cuentas WHERE Cuentas.Cuenta EQ Cta NO-LOCK NO-ERROR.
END PROCEDURE.
...
PROCEDURE HallarSdo:
    ...
    DO i = 1 TO Smes BY 1:
        ...
        IF YEAR(W_Fec1) EQ YEAR(W_Fec2) AND MONTH(W_Fec2) EQ i AND W_Com = NO AND MONTH(W_Fec2) LT MONTH(W_Fec1) THEN
            SIni = SFin.
    END.
END PROCEDURE.
...
PROCEDURE Tabla_Temporal:  /* TT */
    ...
    FOR EACH Sal_Cuenta WHERE Sal_Cuenta.Agencia GE W_Ag1
                          ...
                          AND Sal_Cuenta.Cen_Costos GE W_CC1
                          AND Sal_Cuenta.Cen_Costos LE W_CC2
                          AND Sal_Cuenta.Cuenta GE W_Cuenta1
                          AND Sal_Cuenta.Cuenta LE W_Cuenta2
                          AND Sal_Cuenta.Ano EQ YEAR(W_Fec1) NO-LOCK BY Sal_Cuenta.Cuenta:
    ...
    IF YEAR(W_Fec2) NE YEAR(W_Fec1) THEN DO:
        FOR EACH Sal_Cuenta WHERE Sal_Cuenta.Agencia GE W_Ag1
                              ...
                              AND Sal_Cuenta.Cuenta GE W_Cuenta1
                              AND Sal_Cuenta.Cuenta LE W_Cuenta2
                              AND Sal_Cuenta.Ano EQ YEAR(W_Fec2) NO-LOCK BY Sal_Cuenta.Cuenta:
            RUN Buscar_Cuentas (INPUT Sal_Cuenta.Cuenta).
    ...
    IF AVAILABLE(Cuentas) THEN DO:
        ...
        ASSIGN TSCuentas.TS_Cuenta = Cuentas.Cuenta
               ...
               TSCuentas.TS_Porc = ((TM_SMes / TM_SCom) - 1) * 100.
        ...
        DO i = 4 TO 1 BY -1:
            ...
            IF Cuentas.Naturaleza NE "CR" THEN
...
PROCEDURE ProcesoImprimir:
    ...
    IF W_Fec1 NE W_Fec2 THEN
        W_Comparado = " - Comparado".

    W_Reporte = "REPORTE   : BALANCE GENERAL - Agencia: " + STRING(Cmb_Agencia:SCREEN-VALUE IN FRAME F_Valida,"X(25)") + " - "+ STRING(TIME,"hh:mm am") + W_Comparado.
    ...
    FOR EACH TSCuentas WHERE TSCuentas.TS_Nivel LE Cmb_Nivel BREAK BY SUBSTRING(TSCuentas.TS_Cuenta,1,1) 
                                                                   ...
                                                                   BY TSCuentas.TS_IdCta:
        ASSIGN TSCuentas.TS_Vari = TSCuentas.TS_SMes - TSCuentas.TS_SCom
               /*TSCuentas.TS_Porc = TSCuentas.TS_VARI / TSCuentas.TS_SCom.*/
               TSCuentas.TS_Porc = ((TSCuentas.TS_SMes / TSCuentas.TS_SCom) - 1) * 100.

        IF FIRST-OF(SUBSTRING(TSCuentas.TS_Cuenta,1,1)) AND TSCuentas.TS_IdCta NE 4 THEN
            DISPLAY SKIP(1).

        IF ts_smes EQ 0 AND ts_scom EQ 0 AND ts_vari EQ 0 THEN
            NEXT.

        DISPLAY TSCuentas.TS_Cuenta AT 1
                TSCuentas.TS_Nombre AT 16
                TSCuentas.TS_Nat AT 42
                TSCuentas.TS_SMes AT 51
                TSCuentas.TS_SCom AT 71
                TSCuentas.TS_Vari AT 94
                TSCuentas.TS_Porc AT 115
