...
FOR EACH anexos WHERE (SUBSTRING(anexos.cuenta,1,6) = "511028" OR
                       SUBSTRING(anexos.cuenta,1,8) = "51100801" OR
                       SUBSTRING(anexos.cuenta,1,8) = "51102901" OR
    ...
    IF FIRST-OF(anexos.nit) THEN DO:
        FIND FIRST F1001 WHERE F1001.nit = anexos.nit
                           AND F1001.concepto = "5004" NO-ERROR.
    ...
    IF pOrigen = "E" AND (SUBSTRING(anexos.cuenta,1,8) <> "51102801" AND
                          SUBSTRING(anexos.cuenta,1,10) <> "6140101154") THEN
        NEXT.
        ...
        FOR EACH mov_contable WHERE mov_contable.cuenta = anexos.cuenta
            ...
            IF NOT AVAILABLE docs THEN DO:
                ...
                FOR EACH bfrMovContable WHERE bfrMovContable.agencia = docs.agencia
                                          ...
                                          AND (SUBSTRING(bfrMovContable.cuenta,1,8) = "24352501" OR
                                               SUBSTRING(bfrMovContable.cuenta,1,8) = "24352502" OR
                                               SUBSTRING(bfrMovContable.cuenta,1,8) = "24352503" OR
                                               SUBSTRING(bfrMovContable.cuenta,1,8) = "24352504" OR
                                               SUBSTRING(bfrMovContable.cuenta,1,8) = "24352505" OR
                                               SUBSTRING(bfrMovContable.cuenta,1,8) = "24352506") NO-LOCK:
...
IF pOrigen = "E" THEN DO:
    FOR EACH F1001 WHERE F1001.concepto = "5004" NO-LOCK:
        totalReporte = totalReporte + F1001.pagosCostoDeduccion.
    END.

    FOR EACH sal_cuenta WHERE (SUBSTRING(sal_cuenta.cuenta,1,8) = "51102801" OR
                               SUBSTRING(sal_cuenta.cuenta,1,10) = "6140101154")
