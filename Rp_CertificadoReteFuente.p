...
DEFINE INPUT PARAMETER pMasivo AS INTEGER.

/* Variables */
/*DEFINE VAR pNit AS CHARACTER INITIAL "75147376".
DEFINE VAR pYear AS INTEGER INITIAL 2016.*/


/* Variables de procedimiento */
DEFINE VAR posY AS INTEGER INITIAL 870.
DEFINE VAR ruta AS CHARACTER.

DEFINE TEMP-TABLE lineas
    FIELD linea AS CHARACTER.

ruta = "Reportes\CertificadosRetencion\" + pNit.

INPUT FROM VALUE(ruta + ".txt").
REPEAT:
    CREATE lineas.
    IMPORT UNFORMAT lineas.linea.
END.
INPUT CLOSE.
...
FOR EACH lineas NO-LOCK:
    RUN pdf_set_font IN h_PDFinc ("Spdf","Courier-Oblique",9.5).
    RUN pdf_text_at  IN h_PDFinc ("Spdf", lineas.linea,pdf_LeftMargin("Spdf") - 45).
...
PROCEDURE PageFooter:
    ...
    RUN pdf_skip IN h_PDFinc ("Spdf").
    RUN pdf_skip IN h_PDFinc ("Spdf").
    RUN pdf_text_to IN h_PDFinc ("Spdf", "www.fodun.com.co", 105).
END.

PROCEDURE PageHeader:
    /* Marca de agua */
    /*RUN pdf_watermark IN h_PDFinc ("Spdf","CERTIFICADO DE RETENCIÓN EN LA FUENTE","Courier-Bold",20,.87,.87,.87,100,450).*/
    
    /* Place Logo but only on first page of Report */
    IF pdf_Page("Spdf") = 1 OR pdf_Page("Spdf") = 2 THEN DO:
        ...
        /*FOR EACH lineas NO-LOCK:
            RUN pdf_set_font IN h_PDFinc ("Spdf","Courier-Oblique",9.5).
            RUN pdf_text_xy IN h_PDFinc ("Spdf", lineas.linea,pdf_LeftMargin("Spdf"),posY).
            posY = posY - 16.
        END.*/
