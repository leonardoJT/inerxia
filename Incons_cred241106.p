...
OUTPUT TO C:\InfRed\Incons_QuedanCred291106.Txt.
FOR EACH creditos WHERE estado EQ 2 AND sistema EQ 1 AND sdo_capit GT 0 NO-LOCK
    ...
    ELSE DO:
       ...
       IF  (Creditos.Fec_Pago    LT TODAY 
       AND Creditos.Fec_Pago     LT PlanPagos.Fec_Vcto
       AND (Creditos.Cuo_Pagadas LT (Creditos.Plazo - CuoFalt) OR Creditos.Fec_Pago LT PlanPagos.Fec_Vcto))
       OR (Creditos.Val_Atraso GT 0 AND Creditos.Fec_Pago GE TODAY) 
       OR ErrTransc THEN DO:
          IF  Creditos.Cuo_Pagadas GE (Creditos.Plazo - CuoFalt)
          AND Creditos.Fec_Pago    EQ TODAY THEN.
          ELSE DO:
            ...
            /* FIND LAST PlanPagos WHERE PlanPagos.Agencia                   EQ Creditos.Agencia       
             ...
             IF  Creditos.Val_Atraso  GT 0 
             AND Creditos.Cuo_Pagadas GE (Creditos.Plazo - CuoFalt) THEN DO:
                 IF Creditos.Sdo_Proyect LE Creditos.Sdo_Capital AND PlanPagos.Nro_Cuota LE Creditos.Plazo THEN DO:
