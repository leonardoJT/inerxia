...
REPEAT TRANSACTION ON ERROR UNDO TranConta, LEAVE TranConta:
  /* Activo Fijo */
  FOR EACH act_fijo WHERE act_fijo.estado NE 3 NO-LOCK
...
PROCEDURE SalAnexos:
FOR EACH pro_activo NO-LOCK BY pro_activo.grupo:
    /* Incluir Varios*/
    ...
    FOR EACH Anexos WHERE 
        Anexos.Cuenta EQ pro_activo.cta_fuente
        NO-LOCK BREAK BY Anexos.agencia BY Anexos.Nit:
/*         Anexos.Agencia EQ TemActivo.TAgencia AND */
/*         Anexos.Cuenta  EQ TemActivo.TCuenta  AND */
/*         Anexos.nit     EQ TemActivo.TNit         */
        /*AND Anexos.Ano     EQ 2007 YEAR(TODAY)*/
        ...
        IF LAST-OF (Anexos.agencia) OR LAST-OF (Anexos.nit)THEN DO:
           FIND FIRST TemActivo WHERE
                TemActivo.TAgencia EQ Anexos.Agencia AND
                TemActivo.TNit     EQ Anexos.Nit     AND 
                TemActivo.TCuenta  EQ Anexos.Cuenta  NO-ERROR.    

           IF AVAILABLE(TemActivo) THEN
              UPDATE TemActivo.TContable = vdSdoFinal
                     TemActivo.TDiferen  = TemActivo.TCompra - TemActivo.TContable.
           ELSE 
...
FOR EACH TemActivo BY TemActivo.TAgencia BY TemActivo.TGrupo BY TemActivo.TNit BY TemActivo.TCuenta:
    FORM 
