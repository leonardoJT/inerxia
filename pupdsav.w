...
DEFINE FRAME Panel-Frame
     ...
     Btn-Update AT ROW 1.35 COL 11
     Btn-Reset AT ROW 1.35 COL 20.14
     Btn-Add AT ROW 1.35 COL 29.14
     Btn-Copy AT ROW 1.35 COL 38.14
     Btn-Delete AT ROW 1.35 COL 47.14
     Btn-Cancel AT ROW 1.35 COL 56.14
...
&ENDIF
  DO:
     ...
     DO WITH FRAME Panel-Frame:
        IF Btn-Save:LABEL = '&Actualizar' THEN 
        DO:
...
ON CHOOSE OF Btn-Update IN FRAME Panel-Frame /* Actualizar */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE InhabilitaTodosLosBotones C-WIn 
PROCEDURE InhabilitaTodosLosBotones :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
  ...
  DO WITH FRAME Panel-Frame:
    ...
    DO:
      ...
      /* thing that can be done to it is add-record.                      */
         &IF LOOKUP("Btn-Save":U, "{&ENABLED-OBJECTS}":U," ":U) NE 0 &THEN
      IF cPanelType = 'UPDATE':U THEN
         Btn-Save:LABEL = "&Actualizar":U.
      
      Btn-Save:SENSITIVE = Btn-Save:LABEL = "&Actualizar" AND 
