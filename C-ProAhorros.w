...
&Scoped-define FIELDS-IN-QUERY-Br_Productos Pro_Ahorros.Cod_Ahorro Pro_Ahorros.Nom_Producto   
&Scoped-define ENABLED-FIELDS-IN-QUERY-Br_Productos   
&Scoped-define SELF-NAME Br_Productos
&Scoped-define OPEN-QUERY-Br_Productos ASSIGN FRAME {&FRAME-NAME} T_ProductosAgencia R_Activo. IF T_ProductosAgencia THEN   OPEN QUERY Br_Productos     FOR EACH Pro_Ahorros WHERE Pro_Ahorros.Agencia EQ P_Agencia AND              Pro_Ahorros.Estado EQ R_Activo NO-LOCK INDEXED-REPOSITION. ELSE   OPEN QUERY Br_Productos     FOR EACH Pro_Ahorros WHERE Pro_Ahorros.Estado EQ R_Activo NO-LOCK INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-Br_Productos Pro_Ahorros
&Scoped-define FIRST-TABLE-IN-QUERY-Br_Productos Pro_Ahorros


/* Definitions for FRAME DEFAULT-FRAME                                  */

/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS F_Busca Br_Productos R_Busca R_Activo ~
T_ProductosAgencia BUTTON-90 BUTTON-91 RECT-127 
&Scoped-Define DISPLAYED-OBJECTS F_Busca R_Busca R_Activo ~
T_ProductosAgencia 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS Br_Productos C-Win _FREEFORM
  QUERY Br_Productos NO-LOCK DISPLAY
      Pro_Ahorros.Cod_Ahorro FORMAT "999":U
      Pro_Ahorros.Nom_Producto FORMAT "X(50)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME DEFAULT-FRAME
     F_Busca AT ROW 9.35 COL 28 COLON-ALIGNED NO-LABEL
     Br_Productos AT ROW 1.27 COL 2
     R_Busca AT ROW 9.35 COL 7 NO-LABEL
     R_Activo AT ROW 10.69 COL 41 NO-LABEL
     T_ProductosAgencia AT ROW 10.96 COL 3
     BUTTON-90 AT ROW 10.96 COL 69
     BUTTON-91 AT ROW 11.92 COL 3
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW C-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "SFG - Consulta de Productos de Ahorro"
         HEIGHT             = 12.15
         WIDTH              = 76.86
         MAX-HEIGHT         = 16
         MAX-WIDTH          = 80
...
/* SETTINGS FOR FRAME DEFAULT-FRAME
   Custom                                                               */
/* BROWSE-TAB Br_Productos F_Busca DEFAULT-FRAME */
...
IF T_ProductosAgencia THEN
  OPEN QUERY Br_Productos
    FOR EACH Pro_Ahorros WHERE Pro_Ahorros.Agencia EQ P_Agencia AND
             Pro_Ahorros.Estado EQ R_Activo NO-LOCK INDEXED-REPOSITION.
ELSE
  OPEN QUERY Br_Productos
    FOR EACH Pro_Ahorros WHERE Pro_Ahorros.Estado EQ R_Activo NO-LOCK INDEXED-REPOSITION.
     _END_FREEFORM
...
ON END-ERROR OF C-Win /* SFG - Consulta de Productos de Ahorro */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* SFG - Consulta de Productos de Ahorro */
DO:
...
DO:
  IF Br_Productos:NUM-SELECTED-ROWS > 0 THEN                 
     ASSIGN P_Cod      = Pro_Ahorros.Cod_Ahorro
            P_Nombre   = Pro_Ahorros.Nom_Producto.
...
DO:
  ...
  IF T_ProductosAgencia EQ YES THEN
    OPEN QUERY Br_Productos FOR EACH Pro_Ahorros WHERE 
         Pro_Ahorros.Estado  EQ R_Activo NO-LOCK INDEXED-REPOSITION.  
  ELSE
    OPEN QUERY Br_Productos FOR EACH Pro_Ahorros WHERE Pro_Ahorros.Estado EQ R_Activo NO-LOCK INDEXED-REPOSITION.  
END.
...
DO:
  ...
  DO:
    CASE R_Busca:
      WHEN 1 THEN
        OPEN QUERY Br_Productos FOR EACH Pro_Ahorros  WHERE 
                                        Pro_Ahorros.Cod_Ahorro EQ INTEGER(F_Busca) AND 
                                        Pro_Ahorros.Estado EQ R_Activo 
                                         NO-LOCK INDEXED-REPOSITION.
      WHEN 2 THEN
        OPEN QUERY Br_Productos FOR EACH Pro_Ahorros WHERE 
                                        Pro_Ahorros.Nom_Producto BEGINS F_Busca AND 
                                        Pro_Ahorros.Estado EQ R_Activo
                                        NO-LOCK INDEXED-REPOSITION.
    END CASE.
  END.
...
------------------------------------------------------------------------------*/
  DISPLAY F_Busca R_Busca R_Activo T_ProductosAgencia 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
  ENABLE F_Busca Br_Productos R_Busca R_Activo T_ProductosAgencia BUTTON-90 
         BUTTON-91 RECT-127 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
