...
/*       (mov_creditos.cod_credito = 570 OR mov_creditos.cod_credito = 870) AND */
         ...
         mov_creditos.fecha        LT wfecini 
          /*NO Ojo debe ser wfecfin */
         NO-LOCK:
    IF AVAILABLE(Mov_creditos) THEN DO:
       CREATE TemSdoAntCre.
       UPDATE TemSdoAntCre.agencia     = creditos.agencia
              TemSdoAntCre.num_credito = creditos.num_credito
...
DISPLAY "Bien..".
RETURN.
/* FOR EACH TemSdoAntCre WHERE       */
/*     TemSdoAntCre.sdo_capital = 0: */
/*     DELETE TemSdoAntCre.          */
/* END.                              */
...
/*    END.                                                */
   ...
   FIND FIRST TemSdoAntCre WHERE 
        TemSdoAntCre.agencia     EQ creditos.agencia     AND            
        TemSdoAntCre.num_credito EQ creditos.num_credito /*AND
        TemSdoAntCre.sdo_capital GT 0*/
       NO-LOCK NO-ERROR.
...
/*           ELSE wnro_auditoria = STRING(TemMovCre.num_documento). */
          IF TemMovCre.detalle = "Si" THEN
             RUN detalle_factura.
          wnro_auditoria = STRING(TemMovCre.nro_auditoria).
