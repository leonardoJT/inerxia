...
{INCLUIDO\VARIABLE.I "SHARED"}
  DEFINE VAR w_consecutivo AS INTEGER INITIAL 0.
...
DEFINE FRAME FImprimir
     ...
     TIncluir AT ROW 5.85 COL 6
     FecCorte AT ROW 6.65 COL 31 COLON-ALIGNED
     BUTTON-200 AT ROW 8 COL 5
     BUTTON-201 AT ROW 8 COL 31
    WITH 1 DOWN KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 53 ROW 6.12
         SIZE 47 BY 9.15
...
ASSIGN XXTABVALXX = FRAME FDetCon:MOVE-AFTER-TAB-ITEM (BUTTON-195:HANDLE IN FRAME FSipla)
       XXTABVALXX = FRAME FReporte:MOVE-BEFORE-TAB-ITEM (BUTTON-196:HANDLE IN FRAME FSipla)
       ...
       XXTABVALXX = FRAME FTransacciones:MOVE-BEFORE-TAB-ITEM (FRAME FManual:HANDLE)
       XXTABVALXX = FRAME FInfoHojaVida:MOVE-BEFORE-TAB-ITEM (FRAME FTransacciones:HANDLE)
       XXTABVALXX = FRAME FInfoCreditos:MOVE-BEFORE-TAB-ITEM (FRAME FInfoHojaVida:HANDLE)
       XXTABVALXX = FRAME FInfoCliente:MOVE-BEFORE-TAB-ITEM (FRAME FInfoCreditos:HANDLE)
       XXTABVALXX = FRAME FInfoAhorros:MOVE-BEFORE-TAB-ITEM (FRAME FInfoCliente:HANDLE)
       XXTABVALXX = FRAME FCerradas:MOVE-BEFORE-TAB-ITEM (FRAME FInfoAhorros:HANDLE)
       XXTABVALXX = FRAME FTraspaso:MOVE-BEFORE-TAB-ITEM (FRAME FCerradas:HANDLE)
       XXTABVALXX = FRAME FImprimir:MOVE-BEFORE-TAB-ITEM (FRAME FTraspaso:HANDLE)
       XXTABVALXX = FRAME FFiltros:MOVE-BEFORE-TAB-ITEM (FRAME FImprimir:HANDLE)
       XXTABVALXX = FRAME FConsulta:MOVE-BEFORE-TAB-ITEM (FRAME FReporte:HANDLE)
...
/* SETTINGS FOR FRAME FTransacciones
   NOT-VISIBLE                                                          */
/* BROWSE-TAB BTransacciones TEXT-10 FTransacciones */
...
DO:
  ...
  IF SELF:SCREEN-VALUE NE "3" AND SELF:SCREEN-VALUE NE "4" THEN
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Info_Todos wWin 
PROCEDURE Info_Todos :
...
ASSIGN FRAME F_Filtros Cmb_Emp.
IF SUBSTRING(Cmb_Emp,1,4) EQ "0000" THEN ASSIGN EmpIni = 1 EmpFin = 9999.
ELSE ASSIGN EmpIni = INTEGER(SUBSTRING(Cmb_Emp,1,4)) EmpFin = EmpIni.
...
{Incluido\RepEncabezado.i}
  ...
  CASE Cmb_BEmpresas:
    WHEN "Pagos" THEN W_EncColumna = "Cod.Empresa  Nit Empresa   Nom.Empresa".
    WHEN "Creditos Desembolsados" THEN W_EncColumna = "AGE NIT          NOMBRE                          FEC.DESEMBOLSO    MONTO CREDITO            CUOTA".
    WHEN "Creditos Cancelados" THEN W_EncColumna = "AGE NIT          NOMBRE                          FEC.CANCELACION   MONTO CREDITO            CUOTA".
  END CASE.

  VIEW FRAME F-Encabezado.
  ASSIGN j = 0 k = 0.
IF Cmb_BUsuarios EQ "Pagos" OR Cmb_BUsuarios EQ "no pagos" THEN DO:
    ...
    END.
END.
IF Cmb_BEmpresas EQ "Creditos Desembolsados" THEN DO: 
   FOR EACH Creditos WHERE 
            ...
            Creditos.Fec_Desembolso LE FecFin AND
            Creditos.Estado         NE 3 NO-LOCK:
       j = j + 1.
       RUN Progreso.
       FIND Clientes WHERE Clientes.Nit EQ Creditos.Nit NO-LOCK NO-ERROR.
       IF AVAILABLE Clientes THEN DO:
          CREATE TEmp.
          ASSIGN TEmp.Agencia        = Creditos.Agencia
                 TEmp.Cod_Empresa    = Clientes.Cod_Empresa
                 TEmp.Nit            = Clientes.Nit
                 TEmp.Nombre         = Clientes.Nombre + " " + Clientes.Apellido1 + " " + Clientes.Apellido2
                 TEmp.Fec_Desembolso = Creditos.Fec_Desembolso
                 TEmp.Fec_Cancela    = Creditos.Fec_CanceTotal
                 TEmp.Monto          = Creditos.Val_Desembolso
                 TEmp.Sdo_Capital    = Creditos.Sdo_Capital
                 TEmp.Cuota          = Creditos.Cuota.
       END.
   END.
   FOR EACH Temp WHERE
            TEmp.Cod_Empresa GE EmpIni AND
            TEmp.Cod_Empresa LE EmpFin BREAK BY TEmp.Cod_Empresa BY TEmp.Nit:
      IF FIRST-OF(TEmp.Cod_Empresa) THEN DO:
         WNEmp = STRING(TEmp.Cod_Empresa) + " - Empresa No Encontrada".
         FIND Empresas WHERE Empresas.Cod_Empresa EQ TEmp.Cod_Empresa NO-LOCK NO-ERROR.
         IF AVAILABLE Empresas THEN DO: 
            FIND Clientes WHERE Clientes.Nit EQ Empresas.Nit NO-LOCK NO-ERROR.
            IF AVAILABLE Clientes THEN
               WNEmp = STRING(Empresas.Cod_Empresa) + " - " + Clientes.Nombre + " " + Clientes.Apellido1 + " " + Clientes.Apellido2.
         END.
         DISPLAY SKIP(1) WNEmp WITH FRAME FNomEmpresa WIDTH 132 NO-LABELS USE-TEXT NO-BOX STREAM-IO.
      END.
      DISPLAY TEmp.Agencia TEmp.Nit TEmp.Nombre TEmp.Fec_Desembolso TEmp.Monto TEmp.Cuota   
      WITH FRAME FCreEmpresa WIDTH 132 NO-LABELS USE-TEXT NO-BOX STREAM-IO.
      ASSIGN TotReg = TotReg + 1
             TotMon = TotMon + TEmp.Monto
             TotCuo = TotCuo + Temp.Cuota.
      IF LAST-OF(TEmp.Cod_Empresa) THEN DO:
         DISPLAY "------------------------------------" AT 65 SKIP
                 TotMon AT 65 TotCuo WITH FRAME FTotEmp WIDTH 132 NO-LABELS NO-BOX USE-TEXT STREAM-IO.
         ASSIGN TTotMon = TTotMon + TotMon 
                TTotCuo = TTotCuo + TotCuo 
                TotMon = 0 TotCuo = 0.
      END.
   END.
   DISPLAY SKIP(1) "Total de Creditos Desembolsados" AT 1
           TTotMon AT 65 TTotCuo WITH FRAME FTTotEmp WIDTH 132 NO-LABELS NO-BOX USE-TEXT STREAM-IO.

END.

IF Cmb_BEmpresas EQ "Creditos Cancelados" THEN DO: 
   FOR EACH Creditos WHERE 
            Creditos.Agencia        GE AgeINi AND
            Creditos.Agencia        LE AgeFin AND
            Creditos.FOR_Pago       EQ 2      AND
      ...
      IF FIRST-OF(TEmp.Cod_Empresa) THEN DO:
         ...
         IF AVAILABLE Empresas THEN DO: 
            ...
            IF AVAILABLE Clientes THEN
               WNEmp = STRING(Empresas.Cod_Empresa) + " - " + Clientes.Nombre + " " + Clientes.Apellido1 + " " + Clientes.Apellido2.
         END.
         DISPLAY SKIP(1) WNEmp WITH FRAME CFNomEmpresa WIDTH 132 NO-LABELS USE-TEXT NO-BOX STREAM-IO.
      END.
      DISPLAY TEmp.Agencia TEmp.Nit TEmp.Nombre TEmp.Fec_Cancela TEmp.Monto TEmp.Cuota   
      WITH FRAME FCCreEmpresa WIDTH 132 NO-LABELS USE-TEXT NO-BOX STREAM-IO.
      ASSIGN TotReg = TotReg + 1
             TotMon = TotMon + TEmp.Monto
             TotCuo = TotCuo + Temp.Cuota.
      IF LAST-OF(TEmp.Cod_Empresa) THEN DO:
         DISPLAY "------------------------------------" AT 65 SKIP
                 TotMon AT 65 TotCuo WITH FRAME CFTotEmp WIDTH 132 NO-LABELS NO-BOX USE-TEXT STREAM-IO.
         ASSIGN TTotMon = TTotMon + TotMon 
                TTotCuo = TTotCuo + TotCuo 
                TotMon = 0 TotCuo = 0.
      END.
   END.
   DISPLAY SKIP(1) "Total de Creditos Cancelados" AT 1
           TTotMon AT 65 TTotCuo WITH FRAME FT2TotEmp WIDTH 132 NO-LABELS NO-BOX USE-TEXT STREAM-IO.
...
{Incluido\RepEncabezado.i}
  ...
  VIEW FRAME F-Encabezado.                                                       */
  ASSIGN w_consecutivo = 0  w_vlrTotal = 0.

  /*creacion de registros al plano de los que sobrepasaron el valor mensual (reg automaticos)*/
  FOR EACH TConsulta WHERE 
           TConsulta.Id_NUM AND 
           TConsulta.TipReg EQ "A" AND
           TConsulta.FecRep GE FInicial AND
           TConsulta.FecRep LE FFinal
           BREAK BY TConsulta.Nit:
      ...
      FIND Mov_InsSipla WHERE
           ...
           Mov_InsSipla.Instancia         EQ TConsulta.Instancia AND
           Mov_InsSipla.Nit               EQ TConsulta.Nit       AND
           Mov_InsSipla.Fecha_Transaccion EQ TConsulta.FecRep    AND
           Mov_InsSipla.Tipo_Registro     EQ "A" NO-ERROR.
      IF AVAILABLE Mov_InsSipla THEN
         IF Mov_InsSipla.Fec_RepUIAF NE ? THEN NEXT.
      
      RUN TransaccionesReporte (INPUT TConsulta.Nit, INPUT FInicial, INPUT FFinal, 
                                OUTPUT TConAho, OUTPUT TRetAho, OUTPUT TAboCre).
      IF TConAho GT Entidad.MaxOp_Efectivo_Mes THEN
         RUN Registro_UIAF (INPUT TConsulta.Nit, TConAho, Clientes.Agencia, "CA", FFinal).
      IF TRetAho GT Entidad.MaxOp_Efectivo_Mes THEN
         RUN Registro_UIAF (INPUT TConsulta.Nit, TRetAho, Clientes.Agencia, "RA", FFinal).
      IF TAboCre GT Entidad.MaxOp_Efectivo_Mes THEN
         RUN Registro_UIAF (INPUT TConsulta.Nit, TAboCre, Clientes.Agencia, "AC", FFinal).
  ...
  FOR EACH TConsulta WHERE 
           TConsulta.Id_NUD AND 
       NOT TConsulta.Id_NUM AND
           TConsulta.IdUIAF AND 
           TConsulta.TipReg EQ "A" AND
           TConsulta.FecRep GE FInicial AND
           TConsulta.FecRep LE FFinal
           BREAK BY TConsulta.Nit:
      ...
      IF AVAILABLE TReportadosM THEN NEXT.
      FIND Mov_InsSipla WHERE
           Mov_InsSipla.Agencia           EQ TConsulta.Agencia   AND
           Mov_InsSipla.Instancia         EQ TConsulta.Instancia AND
           Mov_InsSipla.Nit               EQ TConsulta.Nit       AND
      ...
      IF TConAho GT Entidad.MaxOp_Efectivo_Dia THEN
         RUN Registro_UIAF (INPUT TConsulta.Nit, TConAho, Clientes.Agencia, "CA", FInicial).
      IF TRetAho GT Entidad.MaxOp_Efectivo_Dia THEN
         RUN Registro_UIAF (INPUT TConsulta.Nit, TRetAho, Clientes.Agencia, "RA", FInicial).
      IF TAboCre GT Entidad.MaxOp_Efectivo_Dia THEN
         RUN Registro_UIAF (INPUT TConsulta.Nit, TAboCre, Clientes.Agencia, "AC", FInicial).
         ASSIGN Mov_InsSipla.Fec_RepUIAF = FecCorte
                Mov_InsSipla.Id_RepUIAF  = YES.
  END.
  FOR EACH TReportadosM: DELETE TReportadosM. END.

  FOR EACH TConsulta WHERE 
           TConsulta.Id_NUD AND 
       NOT TConsulta.Id_NUM AND
           TConsulta.IdUIAF AND 
           TConsulta.TipReg EQ "M" AND
           TConsulta.FecRep GE FInicial AND
           TConsulta.FecRep LE FFinal
           BREAK BY TConsulta.Nit:
      ASSIGN FInicial = TConsulta.FecRep
      ...
      IF FIRST-OF(TConsulta.Nit) THEN
         FIND Clientes WHERE Clientes.Nit EQ TConsulta.Nit NO-LOCK NO-ERROR.
      RUN Registro_UIAF (INPUT TConsulta.Nit, Mov_InsSipla.Valor_RegManual, Clientes.Agencia, "CA", Mov_InsSipla.Fecha_Transaccion).
      ASSIGN Mov_InsSipla.Fec_RepUIAF = FecCorte
             Mov_InsSipla.Id_RepUIAF  = YES.
  END.
  listado = W_pathspl + "\UIAF" + TRIM(STRING(YEAR(Feccorte))) + TRIM(STRING(MONTH(feccorte),"99")) + TRIM(STRING(DAY(feccorte),"99")) + ".TXT".
  OS-DELETE VALUE(listado).
  OUTPUT TO VALUE(listado).
  zcadena = "31023" + /* Consultar con Rodrigo */
            TRIM(STRING(YEAR(Feccorte)))    + TRIM(STRING(MONTH(feccorte),"99")) + TRIM(STRING(DAY(feccorte),"99")) +
            STRING(w_consecutivo,"99999999") + 
            "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX".
  PUT zcadena SKIP(0).
  FOR EACH UIAF:
     PUT registro SKIP(0).
  END.
  zcadena = "31023" + STRING(w_vlrtotal,"9999999999999999999") +
...
DEFINE VAR    wtipopdcto  AS CHARACTER FORMAT "X".
DEFINE VAR    wtipotra    AS CHARACTER FORMAT "X".
DEFINE VAR    wcta        AS CHARACTER FORMAT "X(20)".
DEFINE VAR    wtipoide    AS CHARACTER FORMAT "X".
...
CASE wtipo:
    WHEN "RA" /* Retiro Ahorro  */      THEN ASSIGN wtipopdcto = "2" wtipotra = "2".
    WHEN "CA" /* Consignacion Ahorro */ THEN ASSIGN wtipopdcto = "2" wtipotra = "1".
    WHEN "AC" /* Abono Credito */       THEN ASSIGN wtipopdcto = "3" wtipotra = "1".
END CASE.
...
CASE Tipo_Identificacion:
    WHEN "C.C" THEN wtipoide = "1".
    WHEN "C.E" THEN wtipoide = "2".
    WHEN "NIT" THEN wtipoide = "3".
    WHEN "T.I" THEN wtipoide = "4".
    WHEN "NUI" THEN wtipoide = "9".
END CASE.


IF clientes.tipo_ident EQ  "NIT" THEN                                                                                                   
   REPEAT i = 1 TO 15:
...
wregistro2 = TRIM(STRING(YEAR(wfecha))) + TRIM(STRING(MONTH(wfecha),"99")) + TRIM(STRING(DAY(wfecha),"99")) +
             STRING(wvalor,"99999999999999") + "1" + 
             STRING(TRIM(agencias.nombre),"X(15)") + 
             wtipopdcto + wtipotra +
             wcta + 
             wtipoide + znit_new + SUBSTRING(agencias.ciudad,1,5) +
             STRING(w_consecutivo,"99999999").
w_vlrtotal = w_vlrtotal + wvalor.
CREATE UIAF.
ASSIGN UIAF.consecutivo = w_consecutivo
       UIAF.registro    = wregistro2.
END PROCEDURE.
...
DEFINE INPUT  PARAMETER NITT LIKE Clientes.Nit.
...
DEFINE OUTPUT PARAMETER TAC LIKE Ahorros.Sdo_Disponible.
  ...
  FOR EACH Mov_Ahorros WHERE
           Mov_Ahorros.Nit   EQ Nitt AND
           Mov_Ahorros.Fecha GE FIni AND
           Mov_Ahorros.Fecha LE FFin NO-LOCK:
      FIND Operacion WHERE Operacion.Cod_Operacion EQ Mov_Ahorros.Cod_Operacion NO-LOCK NO-ERROR.
      IF AVAILABLE Operacion THEN DO:
         IF Operacion.Tipo_Operacion EQ 1 THEN DO:
            IF Operacion.Ctrl_EfeChe EQ 1 THEN
  ...
  END.
  FOR EACH Mov_Creditos WHERE
           Mov_Creditos.Nit   EQ Nitt AND
           Mov_Creditos.Fecha GE FIni AND
           Mov_Creditos.Fecha LE FFin NO-LOCK:
      FIND Operacion WHERE Operacion.Cod_Operacion EQ Mov_Creditos.Cod_Operacion AND
           Operacion.Tipo_Operacion EQ 1 NO-LOCK NO-ERROR.
      IF AVAILABLE Operacion THEN DO:
          IF Operacion.Ctrl_EfeChe EQ 1 THEN
