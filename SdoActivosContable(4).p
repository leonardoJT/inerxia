...
DEFINE VARIABLE vcfuente    AS CHARACTER FORMAT "X(4)" INITIAL "" NO-UNDO.
DEFINE VARIABLE vcdepacu    AS CHARACTER FORMAT "X(4)" INITIAL "" NO-UNDO.
...
REPEAT TRANSACTION ON ERROR UNDO TranConta, LEAVE TranConta:
  /* Activo Fijo */
  FOR EACH act_fijo WHERE act_fijo.agencia EQ 1 AND codigo EQ "TE01" /*act_fijo.estado NE 3*/ 
...
FOR EACH pro_activo NO-LOCK BY pro_activo.grupo:
    ...
    ELSE
       ASSIGN vcnomgrupo = "No Existe".
    ASSIGN vcfuente = SUBSTRING(pro_activo.cta_fuente,1,4)
           vcdepacu = SUBSTRING(pro_activo.CtaDepAcum,1,4).
...
/*         IF (Anexos.Cuenta BEGINS SUBSTRING(pro_activo.cta_fuente,1,4) OR    */
/*         Anexos.Cuenta BEGINS SUBSTRING(pro_activo.CtaDepAcum,1,4)) THEN DO: */
...
/*             MESSAGE vcfuente vcdepacu anexos.cuenta  */
/*                 VIEW-AS ALERT-BOX INFO BUTTONS OK.   */
...
/*                   MESSAGE "Entro.." /*TemActivo.Tagencia TemActivo.Tnit Temactivo.tgrupo anexos.cuenta vdSdoFinal*/  */
/*                        VIEW-AS ALERT-BOX INFO BUTTONS OK.                                                            */
