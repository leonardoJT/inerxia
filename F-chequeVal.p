...
DEFINE INPUT PARAMETER P_ImpVali AS INTEG FORM "9". /*0 Impresora, 1 Validadora*/
...
ASSIGN Ano     = SUBSTRING(STRING(YEAR (W_Fecha)),3,2)
       P_Val2  = P_Valor
       P_Nom2  = P_Nombre.
...
PROCEDURE Impresora:
  ...
  OUTPUT TO VALUE(Cheque) NO-ECHO PAGED PAGE-SIZE 40. /*22*/
  IF P_Valor GT 0 THEN
   DO:
    DISPLAY 
      Ano                        AT ROW 4 COL 36 FORMAT "X(2)" 
      STRING(MONTH(W_Fecha),">9")  AT ROW 4 COL 40 FORMAT "X(2)" 
      STRING(DAY(W_Fecha),">9")    AT ROW 4 COL 44 FORMAT "X(2)" 
...
PROCEDURE Validadora:
   OUTPUT TO /*Com1.*/ C:\MemoSF\cheque1.txt.   
   ...
   PUT SKIP(1) STRING(YEAR(W_Fecha) - 2000)   AT 46 FORMAT "X(2)"  
               STRING(MONTH(W_Fecha),">9")    AT 50 FORMAT "X(2)"    
               STRING(DAY(W_Fecha),">9")      AT 54 FORMAT "X(2)"      
               P_Valor                        AT 62 
               "***"                          AT 77.                          
