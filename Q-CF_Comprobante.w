...
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH Comprobantes WHERE ~{&KEY-PHRASE} NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-Query-Main Comprobantes
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main Comprobantes
...
/* Query rebuild information for QUERY Query-Main
     _TblList          = "bdcentral.Comprobantes"
...
------------------------------------------------------------------------------*/
  ...
  IF NOT AVAILABLE Tmp_Usuarios THEN DO:
     FIND FIRST Comprobantes WHERE Comprobantes.agencia EQ W_agencia NO-LOCK.
     ASSIGN W_Rowid = ROWID(Comprobantes).
     RUN Reposiciona-Query IN THIS-PROCEDURE (INPUT-OUTPUT W_Rowid).
  END.
END PROCEDURE.
