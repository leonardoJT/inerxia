...
FIND LAST Mov_Contable WHERE Mov_Contable.Agencia = W_Ofi1
                         AND Mov_Contable.Comprobante = W_Cbt1
                         AND Mov_Contable.Num_Documento >= W_Doc1
                         AND Mov_Contable.Num_Documento <= W_Doc2
...
FIND FIRST Usuarios WHERE Usuarios.Usuario = Mov_Contable.Usuario NO-LOCK NO-ERROR.
...
IF AVAILABLE(Agencias) THEN DO:
    ...
    FIND FIRST Entidad WHERE Entidad.Entidad = Agencia.Entidad NO-LOCK NO-ERROR.
...
IF W_NroDoc = 0 THEN DO:
    FOR EACH Mov_Contable WHERE Mov_Contable.Agencia = W_Ofi1
                            AND Mov_Contable.Comprobante = W_Cbt1
                            AND Mov_Contable.Num_Documento >= W_Doc1
                            AND Mov_Contable.Num_Documento <= W_Doc2
        ...
        FIND FIRST WCom WHERE WCom = Mov_Contable.Comentario NO-ERROR.
