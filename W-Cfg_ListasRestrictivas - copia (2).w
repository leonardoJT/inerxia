...
DEFINE VARIABLE Texto NO-UNDO LIKE cfg_listasSarlaft.contenido.
DEFINE VARIABLE TextoExtraido NO-UNDO LIKE cfg_listasSarlaft.contenido.
...
&Scoped-define FIELDS-IN-QUERY-brwListas cfg_listasSarlaft.Nombre ~
cfg_listasSarlaft.Tipo_Lista cfg_listasSarlaft.Activo ~
cfg_listasSarlaft.Administrador_lista cfg_listasSarlaft.Contenido_lista ~
...
&Scoped-Define ENABLED-OBJECTS brwListas Cont_Archivo Btn_Archivo ~
Radio_Activo Archivo Nombre_fill Btn_Deshacer Btn-Guardar Btn-Eliminar ~
Tipo_fill Btn-Salvar Btn_Done Btn-Editar RECT-325 
&Scoped-Define DISPLAYED-OBJECTS Cont_Archivo Radio_Activo Archivo ~
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brwListas C-Win _STRUCTURED
  QUERY brwListas SHARE-LOCK NO-WAIT DISPLAY
      cfg_listasSarlaft.Nombre FORMAT "x(15)":U
      cfg_listasSarlaft.Tipo_Lista FORMAT "x(8)":U
      cfg_listasSarlaft.Activo FORMAT "yes/no":U
      cfg_listasSarlaft.Administrador_lista FORMAT "x(64)":U WIDTH 20.43
      cfg_listasSarlaft.Contenido_lista FORMAT "x(9999)":U WIDTH 44.43
      cfg_listasSarlaft.Fecha_modificacion FORMAT "99/99/99":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 124 BY 12.65 FIT-LAST-COLUMN.
...
DEFINE FRAME F-MAIN
     ...
     Cont_Archivo AT ROW 6.38 COL 86 COLON-ALIGNED WIDGET-ID 160
     Btn_Archivo AT ROW 4.62 COL 74 WIDGET-ID 158
     Radio_Activo AT ROW 3.15 COL 73 NO-LABEL WIDGET-ID 154
...
/* Query rebuild information for BROWSE brwListas
     ...
     _FldNameList[1]   = bdcentral.cfg_listasSarlaft.Nombre
     _FldNameList[2]   = bdcentral.cfg_listasSarlaft.Tipo_Lista
     _FldNameList[3]   = bdcentral.cfg_listasSarlaft.Activo
     _FldNameList[4]   > bdcentral.cfg_listasSarlaft.Administrador_lista
"cfg_listasSarlaft.Administrador_lista" ? ? "character" ? ? ? ? ? ? no ? no no "20.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > bdcentral.cfg_listasSarlaft.Contenido_lista
"cfg_listasSarlaft.Contenido_lista" ? ? "character" ? ? ? ? ? ? no ? no no "44.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
  ...
  IF AVAIL(cfg_listasSarlaft) THEN DO:
     ASSIGN Nombre_fill:SCREEN-VALUE = STRING(cfg_listasSarlaft.nombre)
            Tipo_fill:SCREEN-VALUE = STRING(cfg_listasSarlaft.tipo)
            /*Radio_Activo:SCREEN-VALUE = logical(cfg_listasSarlaft.activo)*/
...
DO:
    IF AVAIL(cfg_listasSarlaft) THEN DO:
        ...
        MESSAGE "¿Desea cambiar el registro seleccionado?" SKIP
         ...
         UPDATE W_RptaRet AS LOGICAL.
        IF NOT W_RptaRet THEN RETURN.
        ASSIGN cfg_listasSarlaft.nombre = STRING(nombre_fill:SCREEN-VALUE)
                cfg_listasSarlaft.tipo = STRING(Tipo_fill:SCREEN-VALUE)
                cfg_listasSarlaft.activo = LOGICAL(radio_activo:SCREEN-VALUE)
        ...
        FIND FIRST Usuarios WHERE Usuarios.Usuario = W_Usuario NO-LOCK NO-ERROR.
                IF AVAILABLE Usuarios THEN
                    ASSIGN cfg_listasSarlaft.administrador_lista = Usuarios.nombre.

        IF archivo:SCREEN-VALUE <> "" THEN DO:
            ASSIGN textoExtraido = ""
                texto = "".
            INPUT FROM VALUE(Archivo:SCREEN-VALUE).
                IMPORT UNFORMATTED Texto NO-ERROR.
                IF ERROR-STATUS:ERROR THEN DO:
                   MESSAGE "El archivo tiene problemas. Revise por favor" VIEW-AS ALERT-BOX.
                   LEAVE.
                END.
                ASSIGN textoExtraido = textoExtraido + Texto + "~n".
                ...
                ASSIGN textoExtraido = textoExtraido + Texto + "~n".
            INPUT CLOSE.
            ASSIGN cfg_listasSarlaft.contenido = textoExtraido.
        END.
...
DO: 
    ...
    INPUT FROM VALUE(Archivo:SCREEN-VALUE).
        ...
        ASSIGN cfg_listasSarlaft.nombre = STRING(Nombre_fill:SCREEN-VALUE)
            cfg_listasSarlaft.tipo = STRING(Tipo_fill:SCREEN-VALUE)
            cfg_listasSarlaft.activo = LOGICAL(Radio_Activo:SCREEN-VALUE)
            cfg_listasSarlaft.contenido = textoExtraido
            cfg_listasSarlaft.fecha_modificacion = TODAY.
...
------------------------------------------------------------------------------*/
  DISPLAY Cont_Archivo Radio_Activo Archivo Nombre_fill Tipo_fill 
      WITH FRAME F-MAIN IN WINDOW C-Win.
  ENABLE brwListas Cont_Archivo Btn_Archivo Radio_Activo Archivo Nombre_fill 
