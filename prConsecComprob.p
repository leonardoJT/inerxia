/*
Proposito   : Listar Todos Los Consecutivos de los Comprobantes Contables, 
Date        : Dic/17/2007
By          : Félix Vargas
*/
...
DEFINE VARIABLE viConAnt AS INTEGER NO-UNDO.
DEFINE VARIABLE viConAct AS INTEGER NO-UNDO.
DEFINE VARIABLE vcComp   AS CHARACTER   NO-UNDO. /*Nombre del comprobante*/
...
PROCEDURE Tabla_Temporal:
  ...
  END.
  ELSE
    IF viCpt1 = viCpt2 AND viAge1 = viAge2 AND pdt01 = pdt02 THEN 
       ...
       END.
    ELSE 
      IF viCpt1 = viCpt2 AND viAge1 = viAge2 THEN 
         FOR EACH Mov_Contable /*field(agencia comprobante fec_contable num_documento cuenta db cr)*/
...
/*             WITH FRAME FAge NO-BOX NO-LABEL WIDTH 132. */
        ...
        FORM
            TSDoc.TS_Age    COLUMN-LABEL "Age"          FORMAT "999"
...
/*         END.                                                                      */
        IF FIRST-OF(TSDoc.TS_Com) THEN DO:
           ...
           IF AVAILABLE Comprobantes THEN DO:
                ASSIGN vcComp = (STRING(Comprobantes.Comprobante,"999")) + " - " + Comprobantes.Nombre.
...
/*              DOWN 1 WITH FRAME FAge.                                                  */
        ...
        IF viConAnt NE TSDoc.TS_Doc THEN DO:
            /* Nuevo*/
            IF (viConAnt + 1) LT TSDoc.TS_Doc THEN DO:
               REPEAT WHILE viConAnt LT TSDoc.TS_Doc:
                  ASSIGN  viConAnt = viConAnt + 1.
                  IF viConAnt = TSDoc.TS_Doc THEN
                     LEAVE.
/*                   MESSAGE TSDoc.TS_Com viConAnt TSDoc.TS_Doc  */
