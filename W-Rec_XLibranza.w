...
DEFINE VAR W_OpAboAho AS INTEGER INITIAL 010301001. /*Consignaciones para Ahorros*/
...
DO:
  ...
  IF TPdctos.TP = "A" THEN exc_tp = 1.
  ELSE exc_tp = 2.
  MESSAGE  Rec_Nomina.Nit
              TPdctos.TP      
              TPdctos.CodP
              TPdctos.CtaP    
     ...
     CASE choice:   
      WHEN TRUE THEN DO:
          FIND FIRST exc_abono WHERE exc_abono.nit = Rec_nomina.nit AND
                                 exc_abono.producto = exc_tp        AND
                                 exc_abono.codigo   = TPdctos.CodP AND
                                 exc_abono.cuenta   = DECIMAL(TPdctos.CtaP) NO-ERROR.
 ...
 ------------------------------------------------------------------------------*/
  IF TPdctos.TP EQ "A" THEN DO:   /*Abona Ahorros*/
     ...
     IF ahorros.Tip_ahorro = 4 THEN DO:
        ...
        ASSIGN clientes.tipo_vinculo = 1
               /*clientes.fec_ingreso = ahorros.fec_apertura*/
               clientes.cod_retiro  = ?
               clientes.fec_retiro  = ?.
...
FOR EACH Rec_Nomina WHERE Rec_Nomina.Cod_Empr            EQ W_CodEmp AND
                          Rec_Nomina.Fec_Contabilizacion EQ ? NO-LOCK
                          BREAK BY Rec_Nomina.Nro_pago:
...
/*------------------------------------------------------------------------------
  Purpose: Distribucion del recaudo de acuerdo a las prioridades de los produtos
  Modificado: Gcamacho - Abr/09/08 - distribucion de aporte voluntario en 
                y aporte voluntario X%
                ahorro permanente 50-X%
                para cuando aporte obligatorio supera los 20 SMMLV
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE viValPmnte AS INTEGER     NO-UNDO.
  ...
  */
  ...
  /* Nuevo 07/Feb/2008*/
  ASSIGN w_fecha1 = DATE(INTEGER(w_sem),1,viano).
  ASSIGN w_fecha2 = DATE(INTEGER(w_sem),1,viano) - 1.
  ...
  ELSE 
     IF INTEGER(w_sem) = 2 THEN
        ASSIGN w_fecha3 = w_fecha1 + DAY(DATE(3,1,viano) - 1).
     ELSE 
  ...
  FOR EACH Ahorros WHERE Ahorros.Nit      EQ W_NitCte AND
                         (Ahorros.FOR_Pago EQ 2 OR Ahorros.FOR_Pago EQ 4) AND
                         Ahorros.Estado   EQ 1        AND 
                         
                         TRUE NO-LOCK BY Ahorros.Cod_Ahorro:
      IF AHORROS.CUOTA LE 0  THEN NEXT.
      /* excluye los marcados */
      FIND FIRST exc_abono WHERE exc_abono.nit = ahorros.nit AND
                                 exc_abono.producto = 1 AND
                                 exc_abono.codigo   = ahorros.cod_ahorro AND
                                 exc_abono.cuenta   = DECIMAL(ahorros.cue_ahorro) NO-LOCK NO-ERROR.
      IF AVAILABLE(exc_abono) THEN NEXT.
      /***********/
      ...
      IF ahorros.Per_Deduccion EQ 4 AND v_forma_pago EQ 3 THEN
       ...
       /* Adicionado Nelson Espinosa 15-12-2008 */ 
/*         IF ahorros.Per_Deduccion EQ 3 THEN DO:      */
/*                 TPdctos.NC = 1.                     */
/*                 TPdctos.Cuota = TPdctos.Cuota * 2.  */
/*         END.                                        */
/*         ELSE IF ahorros.Per_Deduccion EQ 4 THEN DO: */
/*                 TPdctos.NC = 0.                     */
/*         END.                                        */
  END.                     

  FOR EACH Creditos WHERE Creditos.Nit         EQ W_NitCte AND
                          (Creditos.FOR_Pago EQ 2 OR Creditos.FOR_Pago EQ 4) AND
       ...
       IF creditos.Fec_PagAnti  GE date(01,06,2009) THEN NEXT.
       /* excluye marcados */
       FIND FIRST exc_abono WHERE exc_abono.nit = creditos.nit AND
                                 exc_abono.producto = 2 AND
                                 exc_abono.codigo   = creditos.cod_credito AND
                                 exc_abono.cuenta   = creditos.num_credito NO-LOCK NO-ERROR.
      IF AVAILABLE(exc_abono) THEN NEXT.
      /*************/
       /* IF creditos.fec_pago GT (w_fecha + 5) THEN NEXT. */   
...
/*         IF ERROR-STATUS:ERROR THEN DO:                */
/*            MESSAGE "Error en Verifica_remanente" SKIP */
/*                 "Nit " W_NitCte                       */
/*                VIEW-AS ALERT-BOX INFO BUTTONS OK.     */
/*            RETURN ERROR.                              */
/*         END.                                          */

     FOR EACH Pro_Ahorros WHERE pro_ahorros.cod_ahorro EQ 3 AND 
...
------------------------------------------------------------------------------*/
  ...
  SYSTEM-DIALOG GET-FILE Archivo
     ...
     MUST-EXIST
     USE-FILENAME.
  MESSAGE   W_CodEmp VIEW-AS ALERT-BOX INFORMATION.
  ...
  REPEAT:
   ...
   ASSIGN Tmp.Ced = TRIM(SUBSTRING(Datos,1,12))
            Tmp.Ced = trim(STRING(INT(Tmp.Ced)))
          Tmp.Vr  = DEC(SUBSTRING(Datos,14,11)).
...
/* REPEAT TRANSACTION ON ERROR UNDO valrema,LEAVE valrema:  */
    ...
    IF w_grarema = 0 AND w_totrema = 0 THEN DO:    /* No Existe Registro*/
       FIND Pro_Ahorros WHERE 
            ...
            Pro_Ahorros.Cod_ahorro EQ 3 EXCLUSIVE-LOCK NO-ERROR.
