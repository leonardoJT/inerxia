...
/* fin incluido Pantalla parametros */
  ...
  SESSION:SET-WAIT-STATE("GENERAL").
    ...
    ELSE DO:
       FOR EACH agencias NO-LOCK:
         FOR EACH Sal_Cuenta WHERE Sal_cuenta.Agencia EQ Agencias.agencia and
                                   Sal_Cuenta.Ano  EQ XANO NO-LOCK:
           /*Sal_Cuenta.Cuenta  GE W_Cuenta1 AND
             Sal_Cuenta.Cuenta  LE W_Cuenta2 */
           RUN Buscar_Cuentas (INPUT Sal_Cuenta.Cuenta).
           RUN Mayorizar.
         END.
       END.
  ...
  PROCEDURE ProcesoImprimir:
     {Incluido\RepEncabezado.i}
      ...
      FOR EACH TSCuentas WHERE TS_Cuenta NE " " AND
                               TSCuentas.TS_Nivel LE Cmb_Nivel 
