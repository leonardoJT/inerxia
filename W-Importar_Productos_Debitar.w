...
Cmb_Cptes Rs_DbCr W_CtaContab W_CtaContra BUTTON-3 Preview Btn_Done 
...
DEFINE FRAME F-Main
     ...
     BUTTON-3 AT ROW 12.27 COL 17.14
     Preview AT ROW 13.58 COL 17.14
     Btn_Conta AT ROW 14.88 COL 17.14
     Btn_Done AT ROW 16.27 COL 69.14
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-323 RECT-324 Cmb_agencia Cmb_TProducto Cmb_Cptes Rs_DbCr 
         W_CtaContab W_CtaContra BUTTON-3 Preview Btn_Done 
      WITH FRAME F-Main IN WINDOW W-Importar_Productos.
...
REPEAT:
 ...
 /*  IMPORT UNFORMATTED datos NO-ERROR.
   ...
   /*ASSIGN Tmp.CAge = INTEGER(SUBSTRING(datos,1,3))
          CNit     = SUBSTRING(datos,4,11)
          CCuo     = decimal(SUBSTRING(datos,15,10)).*/
...
END.
INPUT CLOSE.
MESSAGE xtot VIEW-AS ALERT-BOX.
ASSIGN xtot = 0.
/*CAge/CNit/CCuo*/
...
FOR EACH Tmp WHERE
  ...
  FIND FIRST Ahorros WHERE
       ...
       ahorros.sdo_disponible GT 0 NO-LOCK NO-ERROR.
...
OUTPUT CLOSE.
MESSAGE "dos " xtot VIEW-AS ALERT-BOX.
