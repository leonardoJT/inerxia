...
DO:
    ...
    DEFINE VAR vNumIdentificacion AS CHARACTER.
    DEFINE VAR vCodCredito AS INTEGER.
    DEFINE VAR vNumCredito AS INTEGER.
    DEFINE VAR vTNA AS DECIMAL.
    ...
    IF LENGTH(wTarjetaDB:SCREEN-VALUE) <> 16 AND SUBSTRING(wTarjetaDB:SCREEN-VALUE,1,4) <> '5907' AND SUBSTRING(wTarjetaDB:SCREEN-VALUE,1,4) <> '4177' THEN DO:
        MESSAGE wTarjetaDB:SCREEN-VALUE "no es un número de tarjeta válido para FODUN." SKIP
                "Es indipensable ingresar un número de tarjeta válido. Revise por favor!"
            VIEW-AS ALERT-BOX INFO BUTTONS OK.
    ...
    IF AVAILABLE creditos THEN DO:
        ...
        IF pAutorizacion = TRUE THEN DO:
            ...
            Hoja_Vida.Observacion = "Cupo rotativo autorizado por " + pUsuarioAutoriza.
        END.
        ELSE DO:
    ...
    IF NOT AVAILABLE(creditos) THEN DO:
        ...
        IF AVAILABLE(indicadores) THEN
            vTNA =  (((EXP( (indicadores.tasa / 100) + 1,1 / 12)) - 1 )  * 100) * 12.
        ELSE DO:
        ...
        ASSIGN vAgencia = creditos.agencia
               vNumIdentificacion = creditos.nit
               vCodCredito = creditos.cod_credito
               vNumCredito = creditos.num_credito.

        RELEASE creditos.

        RUN w-planpagosCupoRotativo.r (INPUT vAgencia,
                                       INPUT vNumIdentificacion,
                                       INPUT vCodCredito,
                                       INPUT vNumCredito).
