...
BUTTON-155 BUTTON-121 BUTTON-120 Btn_Ejecutar T3 T1 Rt1 Rt3 T2 T4 Rt2 Rt4 ~
BUTTON-149 T5 T6 Tg_SdoCaja E1 BtnDone BUTTON-158 
...
------------------------------------------------------------------------------*/
  ...
  ENABLE Img_MesF Img_MesI RECT-282 RECT-283 RECT-284 RECT-285 RECT-287 
         ...
         BUTTON-121 BUTTON-120 Btn_Ejecutar T3 T1 Rt1 Rt3 T2 T4 Rt2 Rt4 
         BUTTON-149 T5 T6 Tg_SdoCaja E1 BtnDone BUTTON-158 
      WITH FRAME FTes IN WINDOW wWin.
...
OUTPUT TO value(Listado) PAGED PAGE-SIZE 65 APPEND .
  ...
  FOR EACH Tmp WHERE NO-LOCK BREAK BY TMP.TTIP  BY SUBSTRING(TMP.TCOD,1,2) BY tmp.TCod :
      ...
      IF FIRST-OF (SUBSTRING(TMP.TCOD,1,2)) THEN DO:
         IF SUBSTRING(TMP.TCOD,1,2) = "11" THEN  DISPL "Saldos disponible Caja y Bancos "  AT 1 SKIP.
         IF SUBSTRING(TMP.TCOD,1,2) = "13" THEN  DISPL "Saldos Inversiones "  AT 1 SKIP.
         IF SUBSTRING(TMP.TCOD,1,2) = "21" THEN  DISPL "Saldos Despositos "  AT 1 SKIP.
         totdis = 0.
      END.

     /* IF FIRST-OF (TMP.TTIP) THEN do:
          DISP "Apalancamiento " SKIP.
      END.*/
