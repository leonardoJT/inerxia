...
DEFINE VARIABLE Cmb_BAhorros AS CHARACTER FORMAT "X(50)":U INITIAL "Básico" 
     ...
     LIST-ITEMS "Básico","Fechas de Apertura","Fechas de Cancelación","Fechas de Vencimiento","Canje","Depositos","Clasificacion CDATS","TAC Vencidos","A Termino","Aportes","Contractual","Contractuales 3 Vdas","Productos","Vctos-Liquidac.Proximas A Termino","Cuentas de Empresas Asociadas","Tarjetas Debito","A término con Interés x Pagar" 
     DROP-DOWN-LIST
     SIZE 40 BY 1
     BGCOLOR 15  NO-UNDO.
...
DEFINE VARIABLE Cmb_BClientes AS CHARACTER FORMAT "X(256)":U INITIAL "Básico" 
     ...
     LIST-ITEMS "Básico","Actualización de Información","Cumpleaños","Afiliaciones","Retiros","Clientes/Terceros/Proveedores/Empleados","Asociados Informe General","Asociados Sin Productos","Solo Asociados","Solo Clientes No Asociados","Habiles","No Habiles","Motivos de Vinculación","Actividades","Profesiones","Empresas Asociadas","Inconsistencias en Ubicación","Relaciones","Libro Oficial de Asociados" 
     DROP-DOWN-LIST
     SIZE 40 BY 1
     BGCOLOR 15  NO-UNDO.
...
DEFINE VARIABLE Cmb_BCreditos AS CHARACTER FORMAT "X(50)":U INITIAL "Cartera" 
     ...
     LIST-ITEMS "Cartera","Resumen Creditos","Detallado-Dias Mora","Desembolsos","Por Instancias","Vencimiento","Créditos en Abogado","Cancelados","Castigados","Fondo Mutual","Fondo Mutual General","Por Empresa - Canc. y Desembolsos","Datos Demograficos","Acuerdos Pago No Cumplidos","Acuerdos Pago Cumplidos","Indicador de Solidez","Créditos Congelados" 
     DROP-DOWN-LIST
     SIZE 40 BY 1
     BGCOLOR 15  NO-UNDO.
...
DEFINE VARIABLE Cmb_Tipo AS CHARACTER FORMAT "X(30)":U INITIAL "Clientes" 
     ...
     VIEW-AS COMBO-BOX INNER-LINES 15
     LIST-ITEMS "Clientes","Ahorros","Créditos","Solicitudes","Garantias","Usuarios","Empresas","Pdctos X Empresa","Crecimiento Agencias","Crecimiento Totales","Activos Fijos","Retención en la Fuente" 
     DROP-DOWN-LIST
     SIZE 21.72 BY 1
     BGCOLOR 15  NO-UNDO.
...
DO:
    ...
    IF cmb_BAhorros:SCREEN-VALUE = "Básico" THEN
...
DO:
    ...
    CASE SELF:SCREEN-VALUE:
        ...
        WHEN "Créditos Congelados" THEN DO:
...
DO:
    ...
    IF cmb_tipo:SCREEN-VALUE IN FRAME F_Basicos = "Créditos" THEN DO:
        IF cmb_bcreditos:SCREEN-VALUE = "Detallado-Dias Mora" THEN DO:
            ASSIGN ValIni:SENSITIVE = YES
                   ValFin:SENSITIVE = YES
                   ValIni:LABEL = "Días Mora Inicial"
                   ValFin:LABEL = "Días Mora Final".
        END.
    END.
END.
...
DO:
    ...
    CASE Cmb_Tipo:
        ...
        WHEN "Créditos" THEN DO:
            ...
            IF Cmb_BCreditos:SCREEN-VALUE = "Cartera" OR Cmb_BCreditos:SCREEN-VALUE = "Créditos congelados" THEN
                button-120:SENSITIVE IN FRAME F_Filtros = FALSE.
        ...
        WHEN "Retención en la Fuente" THEN DO:
            HIDE {&List-1} IN FRAME F_Basicos.
...
DO:
    ...
    CASE Cmb_Tipo:
        ...
        WHEN "Créditos" THEN DO:
            IF Cmb_BCreditos:SCREEN-VALUE IN FRAME F_Basicos = "Simular Refinanciación" THEN DO:
                ...
                IF AVAILABLE(Creditos) THEN
                    RUN W-Prorec_Refinancia.R (INPUT "Créditos",
                                               INPUT W_NitW,
                                               INPUT W_Pro,
        ...
        WHEN "Retención en la Fuente" THEN
            RUN Inf_ReteFuente.
    END CASE.
...
CASE Cmb_BAhorros:
    ...
    WHEN "A Termino" OR
    WHEN "A término con Interés x Pagar" THEN DO:
...
FOR EACH Ahorros WHERE Ahorros.Agencia GE AgeIni
    ...
    IF LAST-OF(Ahorros.Agencia) THEN
        IF Cmb_BAhorros EQ "Depositos" OR Cmb_BAhorros EQ "TAC Vencidos" OR  Cmb_BAhorros EQ "A Termino" OR cmb_BAhorros = "A término con Interés x Pagar" OR Cmb_BAhorros EQ "Contractual" THEN
            DISPLAY SKIP(1)
                    "Tot Reg Ag: "           AT 1
...
CASE Cmb_BClientes:
    WHEN "Básico" THEN W_EncColumna = WEnc1 + " FEC.APERTR  MONTO.APERTURA  SDO.DISPONIBLE".
...
END.
ELSE DO:
    IF cmb_BAhorros:SCREEN-VALUE = "Básico" THEN DO:
...
CASE Cmb_BClientes:
    ...
    WHEN "Cumpleaños" THEN
        W_EncColumna = WEnc1 + "FEC.CUMPLEAÑOS   TEL.REDIDENCIA   TEL.COMERCIAL ".
    ...
    WHEN "Clientes/Terceros/Proveedores/Empleados" THEN
        W_EncColumna = "AG CÉDULA/NIT     NOMBRES Y APELLIDOS                                 DIRECCIÓN                                           TELÉFONO         E-MAIL                          VÍNCULO".
...
OUTPUT TO value(Listado) NO-ECHO PAGED PAGE-SIZE 65.
    ...
    FOR EACH ttAhorros NO-LOCK BREAK BY ttAhorros.Agencia
        ...
        CASE Cmb_BAhorros:
            ...
            WHEN "A Termino" OR
            WHEN "A término con Interés x Pagar" THEN DO:
                IF Cmb_BAhorros = "A término con Interés x Pagar" THEN
                    intXpagar = 1.
                ELSE
                    intXpagar = 0.
...
CASE Cmb_BClientes:
    WHEN "Básico" THEN
        W_EncColumna = "AG | NIT             | NOMBRE               | APELLIDO1       | APELLIDO2       | DIRECCIÓN RESIDENCIA           | TEL_RESID       | DIRECCIÓN LABORAL              | TEL_COMERCIAL   | EMAIL                                    | EDAD".
    ...
    WHEN "Cumpleaños" THEN
        W_EncColumna = "AGE CÉDULA         NOMBRE(S)             PRIMER APELLIDO  SEGUNDO APELLIDO FEC_NAC    EDAD DIRECCIÓN DE RESIDENCIA        TEL_RESID       DIRECCIÓN LABORAL              TEL_LABORAL     CORREO ELECTRÓNICO".
...
CASE Cmb_BCreditos:SCREEN-VALUE IN FRAME F_Basicos:
    ...
    WHEN "Cartera" THEN
        ASSIGN W_Reporte = "REPORTE   : Cartera de Créditos: " + " - FECHA: " + STRING(TODAY) + " - " + STRING(TIME,"hh:mm am")
               W_EncColumna = "AGE NIT          PDT ASOCIADO                                 #CREDI     TASA PLAZO F-DESEMB       MONTO     SDO_CAPITAL   INT_CORRIENTE    INT_DIFCOBRO        INT_MORA    VR.CUOTA FEC-NAC    FOR_PAGO       PAGARE         PER_PAGO      CxP  ATR CALIFIC       PROV_K       PROV_INT CELULAR         EDAD (AÑOS)".
    ...
    WHEN "Vencimiento" THEN DO:
        ...
        W_EncColumna = "AG CÉDULA/NIT   PRIMER APELLIDO  SEGUNDO APELLIDO NOMBRE                         COD #CRÉDITO   TASA PLZ FEC_PAGO   SDO_CAPITAL DÍAS SDO_AHORROS    CUOTA      FOR_PAGO     DIRECCIÓN LABORAL              TEL_LABORAL  DIRECCIÓN RESIDENCIA           TEL_RESID    E-MAIL".
    END.
    ...
    WHEN "Detallado-Dias Mora" THEN
        ASSIGN W_Reporte = "REPORTE   : Detallado-Dias Mora: " + " - FECHA: " + STRING(TODAY) + " - " + STRING(TIME,"hh:mm am") + "  - Creditos: " + FPN
               W_EncColumna = "AG NIT          LÍNEA DE CRÉDITO            NUM-CRED NOMBRE ASOCIADO                          TASA PLAZO FEC_DESEMB  MONTO_DESEM SDO_cAPITAL  INTERESES ATRASO     CUOTA VAL_ATRASO  DIRECCIÓN RESIDENCIA    TEL_RESIDEN   DIRECCIÓN LABORAL       TEL_LABORAL  CELULAR    CORREO ELECTRÓNICO".
    ...
    WHEN "Desembolsos" THEN
        ASSIGN W_Reporte = "REPORTE   : Acta Desembolso Créditos: " + " - FECHA: " + STRING(W_Fecha) + " - " + STRING(TIME,"hh:mm am") + "      Desde " + STRING(FecIni) + " Hasta " + STRING(FecFin)
               W_EncColumna = "AG CED/NIT      NOMBRE ASOCIADO                          E-MAIL                              LÍNEA DE CRÉDITO          #SOLIC PAGARE           MONTO   NETO GIRADO PLAZO PERIODICIDAD   T.N.A           CUOTA FEC_DESEMB FEC_PAGO   USUSARIO_DESEMBOLSÓ       DESTINO DEL CRÉDITO".
    ...
    WHEN "Vctos para Débitar" THEN DO:
        IF fecini <> fecfin THEN
            W_Reporte = "REPORTE   : Vctos.para Débitar: " + " - FECHA: " + STRING(W_Fecha) + " - " + STRING(TIME,"hh:mm am") + " DEL " + STRING(FECINI) + " AL " + STRING(FECFIN).
        ELSE
            W_Reporte = "REPORTE   : Vctos.para Débitar: " + " - FECHA: " + STRING(W_Fecha) + " - " + STRING(TIME,"hh:mm am") + " AL " + STRING(FECFIN).

        W_EncColumna = "AGE CED/NIT     PCTO  #CREDITO  PROX_PAGO  CUOTA/SDO_CANCELAR  S_DISP.AHORROS D.Atraso FEC.ULT.MVTO COMENTARIO".
    END.

    WHEN "Créditos en Abogado" THEN
    ...
    WHEN "Libranzas(Empresa,Dias-Mora)" THEN
        W_Reporte   = "REPORTE   : Créditos por Libranzas: " + " - FECHA: " + STRING(TODAY) + " - " + STRING(TIME,"hh:mm am").
    ...
    WHEN "Créditos congelados" THEN DO:
        W_Reporte = "REPORTE   : Cartera de Créditos: " + " - FECHA: " + STRING(TODAY) + " - " + STRING(TIME,"hh:mm am").
        W_EncColumna = "AGE NIT          PDT ASOCIADO                                 #CREDI     TASA PLAZO F-DESEMB       MONTO     SDO_CAPITAL   INT_CORRIENTE    INT_DIFCOBRO        INT_MORA    VR.CUOTA FEC-NAC    FOR_PAGO       PAGARE         PER_PAGO      CxP  ATR CALIFIC       PROV_K       PROV_INT CELULAR         EDAD (AÑOS)".
    END.
...
CASE Cmb_BCreditos:SCREEN-VALUE:
    ...
    WHEN "Vctos para Débitar" THEN RUN Informes_VctosADebitar.
    ...
    WHEN "Créditos en Abogado" THEN RUN Informes_Creditos_Abogado.
    ...
    WHEN "Créditos congelados" THEN RUN Informes_Creditos_Congelados. /* oakley */
END CASE.
...
FOR EACH ttCreditos NO-LOCK BREAK BY ttCreditos.Agencia
    ...
    IF AVAILABLE Clientes THEN DO:
        ...
        CASE ttCreditos.FOR_pago:
            ...
            WHEN 2 THEN vFormaDePago = 'Nómina'.
        END CASE.
...
FOR EACH Creditos WHERE Creditos.Agencia >= AgeIni
    ...
    ELSE
        IF Dias_Vdos > 720 THEN
            ASSIGN DM = 720
                   NM = "Superior a 2 años hasta 1080 días".
        ELSE
            IF Dias_Vdos > 540 THEN
                ASSIGN DM = 540
                       NM = "Superior a 18 meses hasta 720 días".
            ELSE
                IF Dias_Vdos > 360 THEN
                    ASSIGN DM = 360
                           NM = "Superior a un año hasta 540 días".
                ELSE
                    IF Dias_Vdos > 180 THEN
                        ASSIGN DM = 180
                               NM = "Superior a 6 meses hasta 360 días".
                    ELSE
                        IF Dias_Vdos > 120 THEN
                            ASSIGN DM = 120
                                   NM = "Desde 121 días hasta 180 días".
                        ELSE
                            IF Dias_Vdos > 90 THEN
                                ASSIGN DM = 90
                                       NM = "Desde 91 días Hasta 120 días".
                            ELSE
                                IF Dias_Vdos > 60 THEN
                                    ASSIGN DM = 60
                                           NM = "Desde 61 días hasta 90 días".
                                ELSE
                                    IF Dias_Vdos > 30 THEN
                                        ASSIGN DM = 30
                                               NM = "Desde 31 días hasta 60 días".
                                    ELSE
                                        ASSIGN DM = 0
                                               NM = "Desde 0 días hasta 30 días".
END.
...
FOR EACH mov_creditos WHERE (mov_creditos.cod_operacion = 020102001 OR
    ...
    /*IF LAST-OF(Creditos.Agencia) THEN DO:*/
        DISPLAY "--------------------------------------------------------------------------------------------" SKIP
                "Total Agencia Número Desembolsos :" numDesembolsos[1]
...
DISPLAY SKIP(1)
        "--------------------------------------------------------------------------------------------" SKIP
        "Total General Número Desembolsos :" numDesembolsos[2]
...
W_Reporte   = "REPORTE   : Retención en la Fuente: " + cmb_TProducto:SCREEN-VALUE IN FRAME F_Filtros.
W_EncColumna = "CÉDULA      NOMBRE Y APELLIDOS                           BASE  CUENTA RETEFUENTE                      RETENCIÓN".
...
FOR EACH mov_contable WHERE mov_contable.agencia >= ageIni AND mov_contable.agencia <= ageFin AND mov_contable.fec_contable >= fecIni AND mov_contable.fec_contable <= fecFin
    ...
    IF (SUBSTRING(mov_contable.cuenta,1,2) = "26" OR
       ...
       SUBSTRING(mov_contable.cuenta,1,8) <> "51105214" THEN DO:
        FIND FIRST bases WHERE INDEX(mov_contable.nit,bases.nit) = 1
                           AND bases.concepto = "DEMÁS COSTOS" NO-ERROR.
        IF NOT AVAILABLE bases THEN DO:
            CREATE bases.
            ASSIGN bases.Nit = mov_contable.nit
                   bases.concepto = "DEMÁS COSTOS".
        END.
    ...
    IF SUBSTRING(mov_contable.cuenta,1,4) = "2445" THEN DO:
        FIND FIRST retefuente WHERE INDEX(mov_contable.nit,retefuente.nit) = 1
                                      AND retefuente.concepto = "DEMÁS COSTOS"
                                      AND retefuente.cuenta = cuentas.cuenta NO-ERROR.
        IF NOT AVAILABLE retefuente THEN DO:
            CREATE retefuente.
            retefuente.nit = mov_contable.nit.
            retefuente.concepto = "DEMÁS COSTOS".
            retefuente.cuenta = cuentas.cuenta.
            retefuente.nombre = cuentas.nombre.
        END.
