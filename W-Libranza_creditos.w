...
DEFINE VAR vAgenciaCliente AS INTEGER.
DEFINE VAR valor_aDistribuir AS DECIMAL.
...
DEFINE VAR TDif AS DECIMAL.
DEFINE VAR W_ContabDistSi AS DECIMAL.
DEFINE VAR W_ContabDistNo AS DECIMAL.
DEFINE VAR TImp AS DECIMAL.
...
DEFINE TEMP-TABLE tmpConsolidado
...
DEFINE FRAME F_Proc
     RADIO-SET-1 AT ROW 13.65 COL 74 NO-LABEL WIDGET-ID 14
     TOGGLE-1 AT ROW 13.92 COL 111 WIDGET-ID 10
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Rec_XLibranza ASSIGN
         HIDDEN             = YES
         TITLE              = "Distribución  Libranza creditos"
         HEIGHT             = 18.92
         WIDTH              = 112.43
         MAX-HEIGHT         = 26.69
         MAX-WIDTH          = 146.29
...
ON END-ERROR OF W-Rec_XLibranza /* Distribución  Libranza creditos */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Rec_XLibranza W-Rec_XLibranza
ON WINDOW-CLOSE OF W-Rec_XLibranza /* Distribución  Libranza creditos */
DO:
...
&Scoped-define SELF-NAME Br_Pdctos
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL Br_Pdctos W-Rec_XLibranza
ON MOUSE-SELECT-DBLCLICK OF Br_Pdctos IN FRAME F_Proc /* Distribución del Recaudo Seleccionado */
DO:
    /*DEFINE VARIABLE exc_tp AS INTEGER.

    IF TPdctos.TP = "A" THEN
        exc_tp = 1.
    ELSE
        exc_tp = 2.

    MESSAGE  Rec_Nomina.Nit TPdctos.TP TPdctos.CodP TPdctos.CtaP
        VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO-CANCEL TITLE "" UPDATE choice AS LOGICAL.

    CASE choice:
        WHEN TRUE THEN DO:
            FIND FIRST exc_abono WHERE exc_abono.nit = Rec_nomina.nit
                                   AND exc_abono.producto = exc_tp
                                   AND exc_abono.codigo = TPdctos.CodP
                                   AND exc_abono.cuenta = DECIMAL(TPdctos.CtaP) NO-ERROR.
            IF NOT AVAILABLE(EXC_ABONO) THEN DO:
                CREATE Exc_Abono.
                ASSIGN exc_abono.nit = rec_nomina.nit
                       exc_abono.producto = exc_tp
                       exc_abono.codigo = tpdctos.codp
                       exc_abono.cuenta = decimal(tpdctos.ctap).
            END.
        END.
    END CASE.*/
END.
    ...
    IF w_sem:SCREEN-VALUE NE "" THEN DO:
        ...
        FOR EACH Rec_Nomina WHERE Rec_Nomina.Cod_Empresa EQ W_CodEmp
            ...
            IF Rec_Nomina.Val_Deduc GT 0 AND Rec_Nomina.Num_Cuotas NE 99 THEN DO:
              ...
              ASSIGN W_TotNoD              = W_TotNoD + valor_aDistribuir
                     W_TotNoD:SCREEN-VALUE = STRING(W_TotNoD).
           END.
       END.
       /* Nuevo */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL RADIO-SET-1 W-Rec_XLibranza
ON MOUSE-SELECT-DBLCLICK OF RADIO-SET-1 IN FRAME F_Proc
DO:
  FOR EACH exc_abono:
      DELETE exc_abono.
  END.
...
DO:
   ...
   IF NOT AVAIL(Clientes) THEN DO:                                                                    
      ...
      RUN C-Clientes.R (INPUT  1,W_Agencia,                                                           
                        OUTPUT W_NitCte, OUTPUT W_NomCte, OUTPUT W_NomCte, OUTPUT vAgenciaCliente).       
...
DO:
  ASSIGN W_Sem.
  IF INTEGER(W_Sem) NE vimesini AND INTEGER(w_sem) NE vimesfin THEN DO:
       MESSAGE "El Mes a Procesar Debe Ser: " SKIP 
               "               " vimesini  " ó " vimesfin 
       VIEW-AS ALERT-BOX.
       RETURN NO-APPLY.
  END.
  IF INTEGER(W_Sem) = 12 AND MONTH(vdfecpago) = 1 THEN
     ASSIGN viano = YEAR(vdfecpago) - 1.
END.
...
IF TPdctos.TP EQ "A" THEN DO:   /*Abona Ahorros*/
    ...
    IF NOT AVAIL(TempCtas) THEN DO:
        MESSAGE "Falta Configuración con Producto_Ahorro:" Ahorros.Cod_Ahorro "para la Agencia:" Ahorros.Agencia
            VIEW-AS ALERT-BOX ERROR.
...
END.
ELSE DO:
    FIND FIRST Creditos WHERE Creditos.Nit EQ W_NitCte
    ...
    IF AVAIL(Creditos) THEN DO:
        /*MESSAGE creditos.cod_credito SKIP
                creditos.nit SKIP
                Creditos.Num_Credito SKIP
                TPdctos.VrDist SKIP
                Comprobantes.Comprobante SKIP
                Comprobantes.Secuencia SKIP
                0 SKIP
                rec_nomina.num_cuotas SKIP
                w_fecha
            VIEW-AS ALERT-BOX INFO BUTTONS OK.*/

        RUN p-pagoCredito.r (INPUT TRUE,
                             INPUT Creditos.Cod_Credito,
                             INPUT Creditos.Nit,
                             INPUT Creditos.Num_Credito,
                             INPUT TPdctos.VrDist,
                             INPUT Comprobantes.Comprobante,
                             INPUT Comprobantes.Secuencia,
                             INPUT 0,
                             INPUT rec_nomina.num_cuotas,
                             INPUT w_fecha,
                             INPUT FALSE,
                             OUTPUT P_Poliza,
                             OUTPUT P_Honora,
        ...
        IF TPdctos.VrDist <> P_Poliza + P_Honora + P_Costas + P_IAntic + P_IAntic + p_IMorDifC + P_IMora + P_IDifCob + P_ICte + P_IAntic + P_Capit + P_VlrNoDist + P_IAntic THEN
            MESSAGE creditos.nit TPdctos.VrDist - P_Poliza + P_Honora + P_Costas + P_IAntic + P_IAntic + p_IMorDifC + P_IMora + P_IDifCob + P_ICte + P_IAntic + P_Capit + P_VlrNoDist + P_IAntic
                VIEW-AS ALERT-BOX INFO BUTTONS OK.

        IF ERROR-STATUS:ERROR THEN
            RETURN ERROR.

        FIND FIRST tmpConsolidado WHERE tmpConsolidado.nit = creditos.nit NO-ERROR.
        IF NOT AVAILABLE tmpConsolidado THEN DO:
            CREATE tmpConsolidado.
            tmpConsolidado.nit = creditos.nit.
            tmpConsolidado.val = P_Poliza + P_Honora + P_Costas + P_IAntic + P_IAntic + p_IMorDifC + P_IMora + P_IDifCob + P_ICte + P_IAntic + P_Capit + P_VlrNoDist + P_IAntic.
        END.
        ELSE DO:
            tmpConsolidado.val = tmpConsolidado.val + P_Poliza + P_Honora + P_Costas + P_IAntic + P_IAntic + p_IMorDifC + P_IMora + P_IDifCob + P_ICte + P_IAntic + P_Capit + P_VlrNoDist + P_IAntic.
        END.
     END.
     ELSE DO:
     ...
     IF P_VlrNoDist GT 0 OR P_VlrNoDist LT 0 THEN DO:
        MESSAGE "El proceso de distribución del pago retornó valor no distribuido para" SKIP
...
FIND FIRST Cuentas WHERE Cuentas.Tipo EQ 2
                     AND Cuentas.Cuenta EQ "1904159930"
                     AND Cuentas.Estado EQ 1 NO-LOCK NO-ERROR.
FOR EACH Rec_Nomina WHERE Rec_Nomina.Cod_Empresa EQ W_CodEmp
                      AND Rec_Nomina.Nro_Pago EQ W_NroPago
                      AND Rec_Nomina.Num_Cuotas EQ 99 BY Rec_Nomina.Nit.
    FOR EACH Clientes WHERE Clientes.nit EQ Rec_Nomina.Nit NO-LOCK:
        IF Clientes.Agencia NE W_Agencia THEN DO:
            FIND FIRST tmp-exasocSyA WHERE tmp-exasocSyA.cod_age EQ Clientes.Agencia
                                       AND tmp-exasocSyA.Nit EQ Rec_Nomina.Nit NO-ERROR.
            IF NOT AVAIL(tmp-exasocSyA) THEN DO:
                CREATE tmp-exasocSyA.
                ASSIGN tmp-exasocSyA.cod_age = Clientes.Agencia
                       tmp-exasocSyA.nit = Clientes.Nit
                       tmp-exasocSyA.CtaPro = Cuentas.Cuenta
                       tmp-exasocSyA.valrec = Rec_Nomina.Val_Deducido.
            END.
            ELSE
                tmp-exasocSyA.valrec = tmp-exasocSyA.valrec + Rec_Nomina.Val_Deducido.
        END.
    END.
END.
...
FOR EACH tmp-exasocSyA WHERE tmp-exasocSyA.valrec > 0 BREAK BY tmp-exasocSyA.cod_age:
    IF FIRST-OF(tmp-exasocSyA.cod_age) THEN
        w_totrecsya = 0.

    w_totrecsya = w_totrecsya + tmp-exasocSyA.valrec.
    ...
    ASSIGN Mov_Contable.Agencia        = tmp-exasocSyA.cod_age
           ...
           Mov_Contable.Db             = tmp-exasocSyA.valrec.
...
REPEAT TRANSACTION ON ERROR UNDO TranConta, LEAVE TranConta:
    ...
    IF W_ContabDistSi + W_ContabDistNo > 0 THEN DO:
        CREATE Mov_Contable.
        ASSIGN Mov_Contable.Agencia = W_Agencia
               Mov_Contable.Cuenta = Cuentas.Cuenta
    ...
    OUTPUT TO C:\Info_Fodun\pruebaAbono.txt.
    FOR EACH tmpConsolidado:
        EXPORT tmpConsolidado.
    END.
    OUTPUT CLOSE.
    ...
    FOR EACH Mov_Contable WHERE Mov_Contable.Comprobante EQ Comprobantes.Comprobante
                            ...
                            AND Mov_Contable.Fec_Contable EQ W_Fecha NO-LOCK BREAK BY Mov_Contable.Agencia:
...
DEFINE VAR pSeguroVida AS DECIMAL.
DEFINE VAR pSeguroDeudor AS DECIMAL.
...
FOR EACH Creditos WHERE Creditos.Nit EQ W_NitCte
    ...
    FIND FIRST exc_abono WHERE exc_abono.nit = creditos.nit
                           ...
                           AND exc_abono.codigo = creditos.cod_credito
                           AND exc_abono.cuenta = creditos.num_credito NO-LOCK NO-ERROR.
    IF AVAILABLE(exc_abono) THEN
        NEXT.
    ...
    ASSIGN TPdctos.Prior = Pro_Creditos.Prioridad
           ...
           TPdctos.VrSdo = Creditos.Honorarios + Creditos.Costas + Creditos.Polizas + Creditos.Int_MorCobrar + Creditos.Int_MoraDifCob + Creditos.Int_DifCobro + Creditos.Int_Corrientes +
...
valor_aDistribuir = Rec_Nomina.Val_Deduc.

DO j = 1 TO Rec_Nomina.Num_Cuotas:
    FOR EACH TPdctos BY TPdctos.Prior BY TPdctos.FApert:
        IF valor_aDistribuir GE TPdctos.Cuota THEN
            ASSIGN TPdctos.VrDist = TPdctos.VrDist + TPdctos.Cuota
                   TPdctos.NC = TPdctos.NC + 1
                   valor_aDistribuir = valor_aDistribuir - TPdctos.Cuota.
        ELSE
            ASSIGN TPdctos.VrDist = TPdctos.VrDist + valor_aDistribuir
                   TPdctos.NC = TPdctos.NC + 1
                   valor_aDistribuir = 0.
        ...
        IF TPdctos.TP EQ "C" THEN DO:
            IF TPdctos.VrSdo - TPdctos.VrDist LE 0 THEN
                TPdctos.Cuota = 0.
            ELSE
                IF TPdctos.Cuota GT 0 AND TPdctos.Cuota GT (TPdctos.VrSdo - TPdctos.VrDist) THEN
                    TPdctos.Cuota = (TPdctos.VrSdo - TPdctos.VrDist).
        END.

        IF valor_aDistribuir LE 0 THEN
            LEAVE.
    END.
...
IF valor_aDistribuir GT 0 THEN DO:
    IF NOT W_SiContab THEN
        RUN Verifica_Remanente NO-ERROR.
         
    FOR EACH Pro_Ahorros WHERE pro_ahorros.cod_ahorro EQ 8
                           AND Pro_Ahorros.Estado EQ 1 NO-LOCK BY pro_ahorros.tip_ahorro:
        FIND FIRST Ahorros WHERE Ahorros.Nit EQ W_NitCte
                             AND Ahorros.Cod_Ahorro EQ 8
        ...
        IF AVAIL(Ahorros) THEN DO:
            ...
            ASSIGN TPdctos.Prior = Pro_Ahorros.Prioridad
                   ...
                   TPdctos.VrDist = TPdctos.VrDist + valor_aDistribuir
        ...
        ELSE DO:
            ...
            IF NOT AVAILABLE(Tmp_SinRema) THEN DO:
                ...
                ASSIGN Tmp_SinRema.agencia = rec_nomina.agencia
                       ...
                       Tmp_SinRema.valdist = valor_aDistribuir.
            END.
        END.
    END.
END. /* Fin de valor_aDistribuir GT 0*/

IF NOT W_Inf THEN DO:
    IF valor_aDistribuir GT 0 THEN
    ...
    FOR EACH TPdctos:
        ...
        IF TPdctos.TP EQ "C" THEN DO:
            RUN p-pagoCredito.r (INPUT FALSE,
                                 INPUT TPdctos.CodP,
                                 INPUT W_NitCte,
                                 INPUT TPdctos.CtaP,
                                 INPUT TPdctos.VrDist,
                                 ...
                                 INPUT w_fecha,
                                 INPUT NO,
                                 OUTPUT TPdctos.Abonos[8],
                                 OUTPUT TPdctos.Abonos[7],
                                 OUTPUT TPdctos.Abonos[6],
                                 OUTPUT pSeguroVida,
                                 OUTPUT pSeguroDeudor,
                                 OUTPUT pMoraDifCobro,
                                 OUTPUT TPdctos.Abonos[5],
                                 OUTPUT TPdctos.Abonos[4],
                                 OUTPUT TPdctos.Abonos[3],
                                 OUTPUT TPdctos.Abonos[2],
                                 OUTPUT TPdctos.Abonos[1],
                                 OUTPUT P_VlrNoDist,
                                 OUTPUT pError).
        END.
    END.
...
END.
ELSE DO:
    IF W_SiContab AND valor_aDistribuir GT 0 THEN
        W_ContabDistNo = W_ContabDistNo + valor_aDistribuir.
...
------------------------------------------------------------------------------*/
  DISPLAY RADIO-SET-1 TOGGLE-1 W_CmbEmp W_CmbOfi Cmb_pagos Tg_Nuevo Rs_Opcrea 
          W_NitCte W_VrRec W_NCuotas W_NroPago W_NomCte txt_contrapartida W_sem 
          W_TotRec W_TotNoD Tg_SemP W_Cont Msaje W_Dist W_PorD 
      WITH FRAME F_Proc IN WINDOW W-Rec_XLibranza.
  ENABLE RADIO-SET-1 TOGGLE-1 Btn_BorraPagos W_CmbEmp Cmb_pagos Tg_Nuevo 
         W_NitCte W_VrRec W_NCuotas BUTTON-5 Br_Rec txt_contrapartida W_sem 
         Btn_Imp Btn_Preliminar Btn_Contabilizar Tg_SemP Btn_Done Br_Pdctos 
         Btn_CreCancelado btn-Cuotas RECT-314 RECT-316 
      WITH FRAME F_Proc IN WINDOW W-Rec_XLibranza.
  ...
  FOR EACH Clientes WHERE Clientes.Cod_Empresa EQ W_CodEmp
    ...
    /*  FOR EACH Ahorros WHERE Ahorros.Nit       EQ Clientes.Nit
      ...
      FOR EACH Creditos WHERE Creditos.Nit         EQ Clientes.Nit
               ...
               ASSIGN T_VrRec = T_VrRec + round(Creditos.Cuota,0).  
...
/* REPEAT TRANSACTION ON ERROR UNDO valrema,LEAVE valrema:  */
    ...
    IF w_grarema = 0 AND w_totrema = 0 THEN DO:    /* No Existe Registro*/
       FIND Pro_Ahorros WHERE 
            ...
            Pro_Ahorros.Cod_ahorro EQ 8 EXCLUSIVE-LOCK NO-ERROR.
       ...
       DO WHILE LOCKED(pro_Ahorro):
          ...
          IF wespera THEN
              FIND Pro_Ahorros WHERE 
                   ...
                   Pro_Ahorros.Cod_ahorro EQ 8 EXCLUSIVE-LOCK NO-ERROR.
          ELSE DO:
              MESSAGE "error por registro pro_ahorro lookeado " SKIP
                    "Nit " Ahorros.Nit
                  VIEW-AS ALERT-BOX INFO BUTTONS OK.
              RETURN ERROR.
          END.
