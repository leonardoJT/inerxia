  ...
  DEFINE VAR XANO LIKE SAL_CUENTA.ANO.
  ...
  XANO = YEAR(W_Fec1).
   IF w_ag1 = w_ag2 THEN  DO: 
     FOR EACH Sal_Cuenta WHERE
             Sal_Cuenta.Agencia     EQ W_Ag1     AND
             Sal_Cuenta.Cen_Costos  EQ 999     AND
             Sal_Cuenta.Ano         EQ XANO  NO-LOCK:
         IF Sal_Cuenta.Cuenta      GE W_Cuenta1 AND
            Sal_Cuenta.Cuenta      LE W_Cuenta2 THEN DO: 
...
/*      IF SUBSTRING(Cmb_Agencia:SCREEN-VALUE IN FRAME F_Valida,1,3)  = "000" THEN 
 ...
 /*   VIEW FRAME F-Encabezado.
      ...
      ASSIGN Ct = Ct + 1
             IEx.NLinea = Ct
             IEx.Linea  = "AGENCIA" + Cma + "CUENTA" + Cma + "NOMBRE" + Cma + 
      ...
      FOR EACH TSCuentas WHERE TSCuentas.TS_Nivel LE Cmb_Nivel AND TS_Cuenta NE " "
         ...
         OR TSCuentas.TS_CR  NE 0 OR TSCuentas.TS_Fin NE 0 THEN DO:
            ...
            DISPLAY   
              ...
              ASSIGN Ct = Ct + 1
                     IEx.NLinea = Ct
                     IEx.Linea  = STRING(TSCuentas.TS_Agencia) + Cma +
