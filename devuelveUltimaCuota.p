...
DEFINE INPUT PARAMETER p_numSolicitud AS INTEGER.
...
/* RUN rutina. p_fecha - p_fecha1 - W_NroDias

IF vCuota < p_cuota THEN DO:
    DO WHILE vCuota < p_cuota:
        p_cuota = p_cuota - 100.
        RUN rutina.
    END.
...
PROCEDURE P-AmortCuotaFija:
    ...
    FIND FIRST Extras WHERE extras.fec_vcto > fechaAnterior
                        AND extras.fec_vcto <= fecPago
                        AND extras.fec_vcto > ADD-INTERVAL(fecPago,-1,"months")
                        AND Extras.Nit = P_Nit
                        AND Extras.Num_Solicitud = p_numSolicitud NO-LOCK NO-ERROR.
    IF AVAILABLE extras THEN
        vCuota = p_cuota + Extras.Vr_CuoExtra.
