...
DEFINE VARIABLE W_orden AS CHARACTER FORMAT "X(256)":U 
     ...
     LIST-ITEM-PAIRS "Código","cdgo_ger",
                     "Descripción","descr"
     DROP-DOWN-LIST
     SIZE 24.14 BY 1
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL wWin wWin
ON WINDOW-CLOSE OF wWin /* SFG - Mnto. */
DO:
...
DO:
    ...
    DYNAMIC-FUNCTION('setRebuildOnRepos':U IN h_d??,
                     INPUT NO /* LOGICAL */).  

    RUN fetchLast IN h_d??.
    ASSIGN viCnt = DYNAMIC-FUNCTION('getLastRowNum':U IN h_d??).
...
DO:
  DYNAMIC-FUNCTION('setQuerySort':U IN h_d??, INPUT SELF:SCREEN-VALUE).
  DYNAMIC-FUNCTION('OpenQuery':U IN h_d??).
END.
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN W_orden:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "cdgo_ger".
  RUN cancelObject IN h_w??. /*filtro*/
