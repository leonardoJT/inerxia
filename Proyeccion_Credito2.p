...
PROCEDURE P-Amortizacion:
    ...
    DEFINE FRAME F-Encabezado HEADER
        ...
        "Simulación - amortizaciones de crédito"   AT 28 SKIP(1)
        "---------------------------------------------------------------------------------------------------" AT 1
    ...
    IF p_codPro = 108 OR p_codPro = 113 OR p_codPro = 114 THEN DO:
        /*tea_Chr = TRIM(STRING(tea * 100,">>9.99") + "% ").*/
    ...
    IF W_TotInt <> 0 THEN DO:
        FOR EACH W_TabAmor WITH FRAME F-Amortizacion:
            DISP W_Periodo W_TabAmor.W_Fecha W_CuoEx W_TabAmor.W_Cuota W_AboCap W_AboInt W_SdoCap W_TabAmor.descripcion
...
PROCEDURE P-AmortCuotaFija:
    ...
    cuotaPeriodo = P_Cuota.

    FIND FIRST Temp_Extra WHERE Temp_Extra.nro_cuota = w_perTra NO-LOCK NO-ERROR.
    IF AVAILABLE Temp_Extra THEN DO:
        W_TabAmor.W_CuoEx = Temp_Extra.Vr_CuoExtra.
        cuotaPeriodo = cuotaPeriodo + Temp_Extra.Vr_CuoExtra.
    END.
    ...
    IF W_SdoCapTra > 0 THEN DO:
        ...
        IF W_TabAmor.W_AboInt > cuotaPeriodo THEN
            W_TabAmor.W_aboInt = cuotaPeriodo.

        interesPendiente = interesPendiente - W_TabAmor.W_AboInt.
    END.

    W_TabAmor.W_Cuota = cuotaPeriodo.
