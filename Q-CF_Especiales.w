...
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH PRO_ESPECIALES WHERE ~{&KEY-PHRASE} NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-Query-Main PRO_ESPECIALES
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main PRO_ESPECIALES
...
/* Query rebuild information for QUERY Query-Main
     _TblList          = "ESPECIAL.PRO_ESPECIALES"
...
------------------------------------------------------------------------------*/
  ...
  IF W_SuperUsu THEN
     FIND FIRST PRO_ESPECIALES WHERE PRO_ESPECIALES.Estado EQ 1 NO-LOCK NO-ERROR.
  ELSE
     FIND FIRST PRO_ESPECIALES WHERE PRO_ESPECIALES.Agencia EQ W_Agencia
                                 AND PRO_ESPECIALES.Estado EQ 1 NO-LOCK NO-ERROR.
  ASSIGN W_Rowid = ROWID(PRO_ESPECIALES).
  
  RUN Reposicion-Query (INPUT-OUTPUT W_Rowid).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Reposicion-Query q-tables 
PROCEDURE Reposicion-Query :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
