...
WNit Btn_Sdos BNit btnPdf BRet btnMasivoAsociados BtnDone 
...
DEFINE FRAME FCer
     ...
     btnMasivoAsociados AT ROW 18.15 COL 95.72 WIDGET-ID 18
     BtnDone AT ROW 20.27 COL 95.86
...
ON CHOOSE OF btnMasivoAsociados IN FRAME FCer /* Masivo Asociados */
DO:
    ...
    FOR EACH clientes WHERE clientes.nit >= "19117326" NO-LOCK:
        ...
        RUN A_Archivo.
        RUN Rp_CertificadoReteFuente.r(INPUT TCer.nit,
                                       INPUT WAno,
                                       INPUT 2) NO-ERROR.
    END.
END.
...
OUTPUT TO VALUE("Reportes\CertificadosRetencion\" + TNit.nit + ".txt").
DISPLAY SKIP(15)
...
FOR EACH TCer WHERE TCer.Nit = TNit.Nit
                AND TCer.Cba = "HONORARIOS Y/O SERVICIOS" NO-LOCK BREAK BY TCer.nit:
...
FOR EACH TCer WHERE TCer.Nit = TNit.Nit
                AND TCer.Cba = "COMPRAS" NO-LOCK BREAK BY TCer.nit:
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-1 RECT-318 RECT-319 WAno Btn_Imprimir WNit Btn_Sdos BNit btnPdf 
         BRet btnMasivoAsociados BtnDone 
      WITH FRAME FCer IN WINDOW wWin.
        ...
        FOR EACH mov_contable WHERE mov_contable.cuenta = anexos.cuenta
            ...
            IF NOT AVAILABLE docs THEN DO:
                ...
                FOR EACH bfrMovContable WHERE bfrMovContable.agencia = mov_contable.agencia
                                          ...
                                          AND bfrMovContable.nit = mov_contable.nit
                                          AND bfrMovContable.fec_contable = mov_contable.fec_contable
                                          AND SUBSTRING(bfrMovContable.cuenta,1,4) = "2448"
                                          AND bfrMovContable.comentario <> "Cierre Anexos Anual" NO-LOCK:
                    TCer.Ret = TCer.Ret + bfrMovContable.cr - bfrMovContable.db.
                END.
            END.
