 ...
 DEFINE VAR W_CuoEs            AS CHARACTER FORMAT "X(80)".
 DEFINE VARIABLE W_DirCli AS CHARACTER FORMAT "X(80)".
 DEFINE VARIABLE W_TelCli AS CHARACTER FORMAT "X(20)".
 DEFINE VARIABLE W_ForPagQ AS CHARACTER INITIAL " "  FORMAT "x(15)" NO-UNDO.
 DEFINE VARIABLE W_ForPagM AS CHARACTER INITIAL " "  NO-UNDO.
 DEFINE VARIABLE W_TipPagC AS CHARACTER INITIAL " "  NO-UNDO.
 DEFINE VARIABLE W_TipPagN AS CHARACTER INITIAL " "  NO-UNDO.
 DEFINE VARIABLE W_TipPagA AS CHARACTER INITIAL " "  NO-UNDO.
 DEFINE VARIABLE W_NuevaFecha AS DATE        NO-UNDO.
 ...
 DEFINE VARIABLE W_Prueba AS CHARACTER INITIAL "A"  NO-UNDO.
 ...
 *   END CASE.*/
  ...
  IF AVAILABLE Ahorros THEN DO:
      ...
      RUN MontoEsc.r (INPUT ROUND(Ahorros.cuota,0),INPUT 0,OUTPUT W_CuoEs).
      RUN PartirValor IN W_Manija (INPUT W_MonEs,INPUT 60,OUTPUT W_MonEs1,OUTPUT W_MonEs2,OUTPUT W_MonEs3).



      IF ahorros.Per_Deduccion = 3 THEN W_ForPagQ = "QUINCENAL".
      IF ahorros.Per_Deduccion = 4 THEN W_ForPagQ = "MENSUAL".
      
      IF ahorros.For_Pago = 1 THEN W_TipPagC = "CAJA".
      IF ahorros.For_Pago = 2 THEN W_TipPagC = "NOMINA".
      IF ahorros.For_Pago = 3 THEN W_TipPagC = "DEBITO AUT".
...
/******************************************************************************/
  ...
  ASSIGN FecApeD = SUBSTRING(STRING(Ahorros.Fec_Apertura),1,2) 
         ...
         */
         ...
         W_NuevaFecha = Ahorros.Fec_Apertura + Ahorros.Plazo

         FecVenD = SUBSTRING(STRING(W_NuevaFecha),1,2)
         FecVenM = SUBSTRING(STRING(W_NuevaFecha),4,5)
         FecVenA = "20" + SUBSTRING(STRING(W_NuevaFecha),7,10)
...
/*  DESHABILITADO PARA PODER IMPRIMIR 05/11/2008 WILLIAM MARTINEZ
...
*/       
    ...
    ASSIGN W_CuoEs = CAPS(W_CuoEs)
           NomCiudad = CAPS(NomCiudad).
      ...
      DISPLAY
          ...
          TRIM(W_Cue)               AT ROW 3.5    COL 93
          NomCiudad                 AT ROW 9      COL 8
          TRIM(W_Cue)               AT ROW 9      COL 39
          FecApeA                   AT ROW 9      COL 89
          FecApeM                   AT ROW 9      COL 96          
          FecApeD                   AT ROW 9      COL 100
                    
          W_NomCli                  AT ROW 12     COL 8
          W_nit                     AT ROW 12     COL 86
          W_DirCli                  AT ROW 14     COL 8
          W_TelCli                  AT ROW 14     COL 86
          
/*
          /* BENEFICIARIO */
          W_NomBen                  AT ROW 16.5   COL 8
          W_NitBen                  AT ROW 16.5   COL 86
          W_DirBen                  AT ROW 19     COL 8
          W_TelBen                  AT ROW 19     COL 86
*/

          STRING(ahorros.cuota)     AT ROW 22     COL 15
          W_CuoEs                   AT ROW 22     COL 35

          W_TipPagC                 AT ROW 24.5   COL 62
          W_ForPagQ                 AT ROW 24.5   COL 24    /* frecuencia */
          Ahorros.Tasa              AT ROW 27     COL 23 FORMAT ">>>.9999"

          FecVenA                   AT ROW 30.5   COL 89
          FecVenM                   AT ROW 30.5   COL 96          
          FecVenD                   AT ROW 30.5   COL 100
