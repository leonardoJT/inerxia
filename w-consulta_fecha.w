...
SELECT-CmposSlcciondos SELECT-Rpstrio BUTTON-166 Btn_Ejecutar Btn_Imp ~
...
DEFINE FRAME fMain
     ...
     SELECT-CmposSlcciondos AT ROW 3.96 COL 54 NO-LABEL WIDGET-ID 8
     SELECT-Rpstrio AT ROW 4 COL 3 NO-LABEL WIDGET-ID 4
     BUTTON-166 AT ROW 4.77 COL 94 WIDGET-ID 38
     Btn_Ejecutar AT ROW 6.12 COL 94 WIDGET-ID 36
     Btn_Imp AT ROW 7.73 COL 97 WIDGET-ID 54
     ...
     BUTTON-14 AT ROW 15.81 COL 97 WIDGET-ID 46
     BUTTON-11 AT ROW 18.23 COL 99 WIDGET-ID 42
     ...
     "Campos Seleccionados Repositorio:" VIEW-AS TEXT
          SIZE 32 BY .62 TOOLTIP "Campos Disponibles Repositorio" AT ROW 3.15 COL 54 WIDGET-ID 10
          FGCOLOR 7 
     ...
     RECT-2 AT ROW 8.85 COL 35 WIDGET-ID 32
     RECT-290 AT ROW 4.23 COL 93 WIDGET-ID 40
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW wWin ASSIGN
         HIDDEN             = YES
         TITLE              = "Consulta Repositorio Por Fechas"
         HEIGHT             = 19.65
         WIDTH              = 113.14
         MAX-HEIGHT         = 28.81
         MAX-WIDTH          = 146.29
...
ON END-ERROR OF wWin /* Consulta Repositorio Por Fechas */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL wWin wWin
ON WINDOW-CLOSE OF wWin /* Consulta Repositorio Por Fechas */
DO:
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-1 RECT-2 RECT-290 faFchai faFchaf SELECT-CmposSlcciondos 
         SELECT-Rpstrio BUTTON-166 Btn_Ejecutar Btn_Imp BUTTON-tdos-drcha 
...
/*         IF ifla > 100                                                                          */
/*         THEN do:                                                                               */
/*             MESSAGE "PARA PRUEBAS EN PROGRAMACION, SOLAMENTE SE ENVIAN LAS PRIMERAS 100 FILAS" */
/*                 VIEW-AS ALERT-BOX.                                                             */
/*             LEAVE.                                                                             */
/*         END.                                                                                   */
    END.
...
------------------------------------------------------------------------------*/
    ...
    clsta = "Período,fecha,Año,ano,Mes,mes,Día,dia,Nit,nit,Número Crédito,NumeroCredito,".
    clsta = clsta + "Secuencia,SecuencialMes,Tipo Novedad,TipoNovedad,".
    FOR EACH conf_repositorio NO-LOCK
        ...
        ELSE  conf_repositorio.campoorigen) + "," + conf_repositorio.campodestino + ",".  
    END.
    clsta = TRIM(clsta,",").
    DO WITH FRAME {&FRAME-NAME}:
