...
/* oakley */
   ...
   /*Variables del Prog.*/
   DEFI VAR W_OpAboAho  LIKE Operacion.Cod_Operacion. /*Los abonos a Créditos están en prog.AboCredito.P*/
...
END.
ELSE DO:
    FIND Creditos WHERE Creditos.Nit = W_NitCte
                    AND Creditos.Cod_Credito = TPdctos.CodP
                    AND Creditos.Num_Credito = INTEG(TPdctos.CtaP)
    ...
    IF P_VlrNoDist > 0 OR P_VlrNoDist < 0 THEN DO:
    ...
    FIND FIRST TempCtas WHERE TempCtas.Agen = Creditos.Agencia
                          AND TempCtas.TipP = "C"
                          AND TempCtas.Pto = Creditos.Cod_Credito NO-LOCK NO-ERROR.
    IF NOT AVAILABLE(TempCtas) THEN DO:
...
DEFINE VAR j AS INTEGER FORMAT "99".
...
IF NOT W_Inf THEN
    FIND LAST Rec_Nomina WHERE Rec_Nomina.Cod_Empresa = W_CodEmp
                           ...
                           AND Rec_Nomina.Num_Cuotas <> 99 NO-LOCK NO-ERROR.
...
FOR EACH Ahorros WHERE Ahorros.Nit = W_NitCte
    ...
    IF ahorros.cuota <= 0 THEN
        NEXT.

    /* excluye los marcados */
    FIND FIRST exc_abono WHERE exc_abono.nit = ahorros.nit
                           AND exc_abono.producto = 1
                           AND exc_abono.codigo = ahorros.cod_ahorro
                           AND exc_abono.cuenta = DECIMAL(ahorros.cue_ahorro) NO-LOCK NO-ERROR.
...
FOR EACH Creditos WHERE Creditos.Nit = W_NitCte
    ...
    IF Creditos.Cuota <= 0 THEN
        NEXT.

    IF Creditos.Sdo_capital <= 0 THEN
        NEXT.

    IF creditos.Fec_PagAnti >= date(01,06,2009) THEN
        NEXT.
    ...
    ASSIGN TPdctos.Prior = Pro_Creditos.Prioridad
           ...
           TPdctos.VrSdo = Creditos.Honorarios + Creditos.Costas + Creditos.Polizas + Creditos.Int_MorCobrar + Creditos.Int_MoraDifCob + Creditos.Int_DifCobro +
                           Creditos.Int_Corrientes + Creditos.Sdo_Capital - Creditos.Int_Anticipado
           TPdctos.VrDist = 0
           TPdctos.NC = 0
           TPdctos.NomP = Pro_Creditos.Nom_Produc
           TPdctos.Vint = Creditos.Int_corrientes.

    IF TPdctos.Cuota > TPdctos.VrSdo THEN
...
DO j = 1 TO Rec_Nomina.Num_Cuotas:
    FOR EACH TPdctos BY TPdctos.Prior
                     BY TPdctos.FApert:
        IF W_VrADist >= TPdctos.Cuota THEN
        ...
        IF TPdctos.TP = "C" THEN DO:
            IF TPdctos.VrSdo - TPdctos.VrDist <= 0 THEN
                TPdctos.Cuota = 0.
            ELSE
                IF TPdctos.Cuota > 0 AND TPdctos.Cuota > (TPdctos.VrSdo - TPdctos.VrDist) THEN
