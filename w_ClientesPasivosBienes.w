...
DO:
  RUN Calculos_bienes.
    PUBLISH "CierraClientesPasivosBienes".
...
------------------------------------------------------------------------------*/
  ...
  CASE currentPage: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN repositionObject IN h_dact_pasivos1 ( 5.04 , 78.00 ) NO-ERROR.
       /* Size in AB:  ( 3.50 , 13.00 ) */
       ...
       RUN repositionObject IN h_dact_pasivos3 ( 5.31 , 6.00 ) NO-ERROR.
       /* Size in AB:  ( 3.77 , 16.00 ) */
...
------------------------------------------------------------------------------*/
    ...
    DYNAMIC-FUNCTION('setQueryWhere':U IN h_dact_pasivos1,INPUT "nit = '" + p_nit + "' AND Cod_Tipo = 4").
    DYNAMIC-FUNCTION('openQuery':U IN h_dact_pasivos1).
    
    DYNAMIC-FUNCTION('closeQuery':U IN h_dact_pasivos2).
    DYNAMIC-FUNCTION('setQueryWhere':U IN h_dact_pasivos2,INPUT "nit = '" + p_nit + "' AND Cod_Tipo = 2").
    DYNAMIC-FUNCTION('openQuery':U IN h_dact_pasivos2).
    
    DYNAMIC-FUNCTION('closeQuery':U IN h_dact_pasivos3).
    DYNAMIC-FUNCTION('setQueryWhere':U IN h_dact_pasivos3,INPUT "nit = '" + p_nit + "' AND Cod_Tipo = 1").
    DYNAMIC-FUNCTION('openQuery':U IN h_dact_pasivos3).
    
    DYNAMIC-FUNCTION('closeQuery':U IN h_dact_pasivos4).
    DYNAMIC-FUNCTION('setQueryWhere':U IN h_dact_pasivos4,INPUT "nit = '" + p_nit + "' AND Cod_Tipo = 3").
    DYNAMIC-FUNCTION('openQuery':U IN h_dact_pasivos4).
    
    
    DYNAMIC-FUNCTION('setBrowseColumnLabels':U IN h_dynbrowser1,INPUT "'Entidad'" ).
    RUN selectPage(4).
    RUN selectPage(3).
    RUN selectPage(2).
    RUN selectPage(1).
    RUN pEscndeDrccion IN h_vclientesbienes-2.
END PROCEDURE.
