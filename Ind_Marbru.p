...
FOR EACH Cuentas WHERE SUBSTRING(Cuentas.Cuenta,1,1,'CHARACTER') = '4' AND Cuentas.Tipo = 2:
...
END.
W_TotPor = 0.
FOR EACH Cuentas WHERE SUBSTRING(Cuentas.Cuenta,1,1,'CHARACTER') = '5' AND Cuentas.Tipo = 2:
 RUN SaldosCuenta.p(INPUT Cuentas.Cuenta, INPUT  W_Ano, INPUT W_Age, INPUT W_Age1, INPUT Cuentas.Naturaleza, INPUT W_Mes, OUTPUT W_TotPor).
 W_TotAct = W_TotAct - W_TotPor.
END.
W_TotPor = 0.
FOR EACH Cuentas WHERE SUBSTRING(Cuentas.Cuenta,1,1,'CHARACTER') = '6' AND Cuentas.Tipo = 2:
 RUN SaldosCuenta.p(INPUT Cuentas.Cuenta, INPUT  W_Ano, INPUT W_Age, INPUT W_Age1, INPUT Cuentas.Naturaleza, INPUT W_Mes, OUTPUT W_TotPor).
 W_TotAct = W_TotAct - W_TotPor.
END.
W_TotPor = 0.
