...
DEFINE VARIABLE h_dpro_saro AS HANDLE NO-UNDO.
...
OR RETURN OF W_Busca DO:
  ...
  DYNAMIC-FUNCTION('closeQuery':U IN h_dpro_saro).
  DYNAMIC-FUNCTION('setQueryWhere':U IN h_dpro_saro,
     INPUT STRING(pro_saro.cod_Factor,'999999') = W_Busca:SCREEN-VALUE).  
  DYNAMIC-FUNCTION('openQuery':U IN h_dpro_saro).
  ...
  CASE currentPage: 
    ...
    WHEN 0 THEN DO:
       RUN constructObject (
             ...
             OUTPUT h_dpro_saro ).
       RUN repositionObject IN h_dpro_saro ( 1.27 , 54.00 ) NO-ERROR.
       ...
       /* Links to SmartDataObject h_dpro_saro. */
       RUN addLink ( h_dpro_saro , 'Data':U , THIS-PROCEDURE ).

       /* Links to SmartDataBrowser h_dynbrowser. */
       RUN addLink ( h_dpro_saro , 'Data':U , h_dynbrowser ).
       RUN addLink ( h_dynbrowser , 'Update':U , h_dpro_saro ).
