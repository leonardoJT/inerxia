...
DEFINE TEMP-TABLE Tmp
  ...
  FIELD CCueAho  AS CHARACTER FORMAT "X(8)"
  FIELD CcodAho  AS INTEGER   FORMAT "99"
  FIELD Cnombre  AS CHARACTER FORMAT "X(38)".
...
DEFINE VAR w_Cpte LIKE comprobantes.comprobante.
DEFINE VARIABLE procname AS CHARACTER NO-UNDO.
DEFINE VARIABLE OKpressed AS LOGICAL INITIAL TRUE.

DEFINE VAR CtaAnex_Det    LIKE Cuentas.Cuenta.     
...
&Scoped-Define ENABLED-OBJECTS Cmb_agencia Cmb_TProducto Cmb_Cptes Rs_DbCr ~
W_CtaContab W_CtaContra T-GMF w_comentario BUTTON-3 Preview Btn_Done ~
RECT-323 RECT-324 
&Scoped-Define DISPLAYED-OBJECTS Cmb_agencia Cmb_TProducto Cmb_Productos ~
Cmb_Cptes Rs_DbCr W_CtaContab NomCta1 W_CtaContra NomCta2 T-GMF ~
w_comentario 
...
DEFINE VARIABLE w_comentario AS CHARACTER FORMAT "X(70)":U 
     LABEL "Descripción" 
     VIEW-AS FILL-IN 
     SIZE 60 BY .81 TOOLTIP "Descripción corta que define la operación Débito Automático"
     BGCOLOR 15  NO-UNDO.
...
DEFINE VARIABLE T-GMF AS LOGICAL INITIAL no 
     LABEL "" 
     VIEW-AS TOGGLE-BOX
     SIZE 3 BY .81 TOOLTIP "Si desea cobrar GMF, indiquelo" NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     Cmb_Cptes AT ROW 5.5 COL 20.72 COLON-ALIGNED
     Rs_DbCr AT ROW 6.81 COL 23 NO-LABEL
     W_CtaContab AT ROW 7.73 COL 20.72 COLON-ALIGNED
     NomCta1 AT ROW 7.77 COL 35.86 COLON-ALIGNED NO-LABEL
     W_CtaContra AT ROW 8.81 COL 20.86 COLON-ALIGNED
     NomCta2 AT ROW 8.85 COL 36 COLON-ALIGNED NO-LABEL
     T-GMF AT ROW 9.92 COL 23
     w_comentario AT ROW 10.96 COL 21 COLON-ALIGNED
     BUTTON-3 AT ROW 12.85 COL 25.29
     Btn_Conta AT ROW 14.19 COL 25.43
     Preview AT ROW 15.54 COL 25.43
     Btn_Done AT ROW 17 COL 69.14
     RECT-323 AT ROW 6.65 COL 22.43
     RECT-324 AT ROW 1.54 COL 4
     "Descripción Dèbito" VIEW-AS TEXT
          SIZE 41 BY .62 AT ROW 1.27 COL 7
          FGCOLOR 7 
     "Naturaleza" VIEW-AS TEXT
          SIZE 10 BY .62 AT ROW 6.73 COL 11.14
     "Cobrar GMF" VIEW-AS TEXT
          SIZE 11.14 BY 1.08 AT ROW 9.77 COL 10.72
...
DO:
  ASSIGN W_Cpte = INTEG(SUBSTRING(Cmb_Cptes:SCREEN-VALUE,1,2))
         cmb_cptes.
END.
...
------------------------------------------------------------------------------*/
  DISPLAY Cmb_agencia Cmb_TProducto Cmb_Productos Cmb_Cptes Rs_DbCr W_CtaContab 
          NomCta1 W_CtaContra NomCta2 T-GMF w_comentario 
      WITH FRAME F-Main IN WINDOW W-Importar_Productos.
  ENABLE Cmb_agencia Cmb_TProducto Cmb_Cptes Rs_DbCr W_CtaContab W_CtaContra 
         T-GMF w_comentario BUTTON-3 Preview Btn_Done RECT-323 RECT-324 
      WITH FRAME F-Main IN WINDOW W-Importar_Productos.
...
/*debito*/
  ...
  ASSIGN TmpMov.Agencia         = INTEGER(SUBSTRING(Cmb_Agencia:SCREEN-VALUE IN FRAME F-Main,1,3))
         ...
         TmpMov.Doc_Referencia  = Tmp.CcueAho /* STRING(W_Doc) */
         TmpMov.Nit             = TRIM(Tmp.CNit)
         TmpMov.Comentario      = TRIM(w_comentario:SCREEN-VALUE) + " (Db)"
  ...
  ASSIGN TmpMov.Agencia         = INTEGER(SUBSTRING(Cmb_Agencia:SCREEN-VALUE IN FRAME F-Main,1,3))
         ...
         TmpMov.Doc_Referencia  = Tmp.CcueAho  /* STRING(W_Doc)*/
         TmpMov.Nit             = TRIM(Tmp.CNit)
         TmpMov.Comentario      = TRIM(w_comentario:SCREEN-VALUE) + " (Cr)"
         TmpMov.Usuario         = W_Usuario. 
         TmpMov.CR              = Tmp.CCuo.

/*
  IF INTEGER(Tmp.CCuo * 4 / 1000) GT 0 THEN
  DO:
    /*debito GMF */                                                                                                
    CREATE TmpMov.                                                                                            
    ASSIGN TmpMov.Agencia         = INTEGER(SUBSTRING(Cmb_Agencia:SCREEN-VALUE IN FRAME F-Main,1,3))          
           ...
           TmpMov.Doc_Referencia  = Tmp.CcueAho /* STRING(W_Doc) */                                           
           TmpMov.Nit             = TRIM(Tmp.CNit)                                                            
           TmpMov.Comentario      = "Debito VIVIR (Db)"                                                       
           TmpMov.Usuario         = W_Usuario.                                                                
           TmpMov.DB              = INTEGER(Tmp.CCuo * 4 / 1000).                                                                 
    IF ahorros.exento_3xm THEN   /* Para los que estan exentos */                                            
       ASSIGN TmpMov.Cuenta       = W_CtaDbGMF.                                                                  
    ELSE                                                                                                     
       ASSIGN TmpMov.Cuenta       = W_CtaDeb.   /* GMF a la cuenta del producto */                                                               
    ...
    ASSIGN TmpMov.Agencia         = INTEGER(SUBSTRING(Cmb_Agencia:SCREEN-VALUE IN FRAME F-Main,1,3))          
           ...
           TmpMov.Doc_Referencia  = Tmp.CcueAho  /* STRING(W_Doc)*/                                           
           TmpMov.Nit             = TRIM(Tmp.CNit)                                                            
           TmpMov.Comentario      = "Debito VIVIR (Cr)"                                                       
           TmpMov.Usuario         = W_Usuario.                                                                
           TmpMov.CR              = INTEGER(Tmp.CCuo * 4 / 1000). 
    ...
    ASSIGN TmpAho.Agencia         = INTEGER(SUBSTRING(Cmb_Agencia:SCREEN-VALUE IN FRAME F-Main,1,3))                
           ...
           TmpAho.Cod_Operacion   = 010102001                                                                       
           TmpAho.Cpte            = INTEGER(SUBSTRING(Cmb_Cptes,1,2))                                               
           TmpAho.Cue_Ahorros     = Tmp.CcueAho /*ahorros.cue_ahorros*/                                             
           TmpAho.Descrip         = "GMF de la Importacion VIVIR"                                                   
           ...
           TmpAho.Val_Efectivo    = INTEGER(Tmp.CCuo * 4 / 1000)                                                                        
...
*/                                                                                                             
  ...
  ASSIGN TmpAho.Agencia         = INTEGER(SUBSTRING(Cmb_Agencia:SCREEN-VALUE IN FRAME F-Main,1,3)) 
         ...
         TmpAho.Cpte            = INTEGER(SUBSTRING(Cmb_Cptes,1,2))
         TmpAho.Cue_Ahorros     = Tmp.CcueAho /*ahorros.cue_ahorros*/
         TmpAho.Descrip         = TRIM(w_comentario:SCREEN-VALUE)
         TmpAho.Fecha           = W_Fecha
         TmpAho.Hora            = TIME
         TmpAho.Nit             = Ahorros.nit
         TmpAho.Num_Documento   = STRING(W_Doc)
...
DO TRANSACTION:
 DEFINE VAR w-4xmil AS DECIMAL INITIAL 0.
 ...
 FOR EACH TmpAho:
   ...
   IF TmpAho.Cod_Ahorro EQ INTEGER(SUBSTR(Cmb_Productos:SCREEN-VALUE IN FRAME F-Main,1,3)) THEN DO:
      FIND Ahorros WHERE
           ...
           Ahorros.Cue_ahorros EQ TmpAho.Cue_Ahorros AND
           Ahorros.Nit         EQ TmpAho.Nit EXCLUSIVE-LOCK NO-ERROR.
      IF AVAILABLE(Ahorros) THEN DO:
         IF TmpAho.Val_Efectivo GT 0 THEN DO:
            IF T-GMF THEN DO:
              RUN RutGMF.R (INPUT TRUE,W_Agencia,Ahorros.Agencia,1,Ahorros.Cod_Ahorro,Ahorros.Nit,  
                            INPUT Ahorros.Cue_Ahorro,"010102001",TmpAho.Val_Efectivo,INTEGER(SUBSTRING(Cmb_Cptes,1,2)),                   
                            INPUT STRING(W_Doc),TRIM(w_comentario:SCREEN-VALUE),0,2,                                   
                            OUTPUT w-4xmil) NO-ERROR.                                                    
              IF ERROR-STATUS:ERROR THEN DO:                                                                
         ...
         ELSE
         DO: 
           MESSAGE "Valor en Cero para el cargue Nit " Ahorros.nit  SKIP
                   "con Vlr " TmpAho.Val_Efectivo " Con valor Cero" .
           W_OkTmp = NO.
         END.

         ASSIGN Ahorros.Sdo_Disponible = Ahorros.Sdo_Disponible - TmpAho.Val_Efectivo /* - INTEGER(TmpAho.Val_Efectivo * 4 / 1000 */
         ...
         ASSIGN Mov_Ahorros.Agencia         = Ahorros.agencia 
                ...
                Mov_Ahorros.Descrip         = "Debito VIVIR"
      ...
      END.
      ELSE DO:
          MESSAGE "El cliente identificado con nit: " TmpAho.Nit SKIP
...
REPEAT:
   ...
   END.
   create tmp.
   ASSIGN Tmp.CAge = INTEGER(SUBSTRING(datos,1,3))
          CNit     = SUBSTRING(datos,4,12)
          CCuo     = decimal(SUBSTRING(datos,16,9))
          CCueAho  = SUBSTRING(datos,25,8)
          CcodAho  = integer(SUBSTRING(datos,33,2))
          Cnombre  = SUBSTRING(datos,4,38).
   ...
   IF Tmp.CAge NE INTEGER(SUBSTRING(Cmb_Agencia:SCREEN-VALUE IN FRAME F-Main,1,3)) THEN DO:
      ...
      RETURN ERROR.
   END.
   IF Tmp.CCodAho NE INTEGER(SUBSTRING(Cmb_Productos:SCREEN-VALUE IN FRAME F-Main,1,3)) THEN DO:
      MESSAGE "La información que se trata de subir pertenece al Pdcto de Ahorro: " Tmp.CcodAho SKIP
              "El  codigo  de ahorro seleccionada NO es igual a  la  del  archivo" SKIP(2)
              "Se cancela el proceso de subida de datos!" VIEW-AS ALERT-BOX.
      FOR EACH Tmp: DELETE Tmp. END.
      RETURN ERROR.
   END.
...
DEFINE VAR w_sdomin    AS DECIMAL   FORMAT "zzz,zzz,zz9" INITIAL 0.00.
DEFINE VAR W_tope4xmil AS DECIMAL   FORMAT "zzz,zzz,zz9" INITIAL 0.00.
FIND pro_ahorros WHERE pro_ahorros.tip_ahorro EQ INTEGER(SUBSTR(Cmb_TProducto:SCREEN-VALUE IN FRAME F-Main,1,1))
                   AND pro_ahorros.cod_ahorro EQ INTEGER(SUBSTR(Cmb_Productos:SCREEN-VALUE IN FRAME F-Main,1,3)) NO-LOCK NO-ERROR.
IF AVAILABLE(pro_ahorros) THEN
   ASSIGN w_sdomin    = Pro_Ahorros.val_sdominimo
          W_tope4xmil = Pro_Ahorros.VrTope_ExentoEE.
...
FOR EACH Tmp WHERE
  ...
  FIND Ahorros WHERE
       ...
       Ahorros.cue_ahorros EQ Tmp.CcueAho AND
  ...
  IF Tmp.CCuo GT 0 THEN
    IF NOT ahorros.exento_3xm THEN
       w-4xmil =  INTEGER(Tmp.CCuo * 4 / 1000).
    ELSE
       w-4xmil = 0.
  ELSE
  DO: 
  ...
  IF ( (Ahorros.sdo_disponible - Tmp.CCuo - w-4xmil ) LE w_sdomin) THEN DO:
      DISPLAY "Incons. No tiene fondos para el DB - Nit " Tmp.CNit  " Cuota: " Tmp.CCuo "  Saldo Act " sdo_disponible WITH FRAME j2 WITH WIDTH 120 NO-LABELS USE-TEXT.
      W_OkTmp = NO.
  END.
