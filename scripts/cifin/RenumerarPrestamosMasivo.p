...
DEFINE TEMP-TABLE ttcifin
    FIELD nit AS CHARACTER
    FIELD numCredito AS INTEGER.

DEFINE VAR newnumber AS CHARACTER.

INPUT FROM d:\Leonardo\Cifin\cifin.csv.
REPEAT:
    CREATE ttcifin.
    IMPORT DELIMITER ";" ttcifin.
    /*DISPLAY ttcifin.*/
END.
INPUT CLOSE.

FOR EACH ttcifin NO-LOCK:
    FIND FIRST creditos WHERE creditos.nit = ttcifin.nit
                          AND creditos.num_credito = ttcifin.numCredito
    ...
    IF AVAILABLE creditos THEN DO:
        ...
        IF creditos.cod_credito = 123 THEN DO:
            FIND FIRST tarjetas WHERE tarjetas.nit = creditos.nit
                                  AND tarjetas.num_credito = creditos.num_credito NO-ERROR.
            IF AVAILABLE tarjetas THEN
                tarjetas.num_credito = INTEGER(newNumber).
        END.
