...
DEFINE VAR fec_corte AS DATE INITIAL 08/31/2019.
...
OUTPUT TO c:\INFO_fodun\RiesgoLiquidez.csv.
...
FOR EACH sal_cuenta WHERE (SUBSTRING(sal_cuenta.cuenta,1,2) = "16" OR SUBSTRING(sal_cuenta.cuenta,1,4) = "1403")
                      AND sal_cuenta.ano = YEAR(fec_corte) NO-LOCK BREAK BY sal_cuenta.cuenta:
    IF FIRST-OF(sal_cuenta.cuenta) THEN
        FIND FIRST cuentas WHERE cuentas.cuenta = sal_cuenta.cuenta NO-LOCK NO-ERROR.
...
FOR EACH sal_cuenta WHERE (SUBSTRING(sal_cuenta.cuenta,1,4) = "1650" OR
                           SUBSTRING(sal_cuenta.cuenta,1,4) = "1645" OR
                           SUBSTRING(sal_cuenta.cuenta,1,4) = "1630")
                      AND sal_cuenta.ano = YEAR(fec_corte) NO-LOCK BREAK BY sal_cuenta.cuenta:
    IF FIRST-OF(sal_cuenta.cuenta) THEN
        FIND FIRST cuentas WHERE cuentas.cuenta = sal_cuenta.cuenta NO-LOCK NO-ERROR.
...
FOR EACH sal_cuenta WHERE (SUBSTRING(sal_cuenta.cuenta,1,2) = "19" OR SUBSTRING(sal_cuenta.cuenta,1,4) = "1226")
                      AND sal_cuenta.ano = YEAR(fec_corte) NO-LOCK BREAK BY sal_cuenta.cuenta:
    IF FIRST-OF(sal_cuenta.cuenta) THEN
        FIND FIRST cuentas WHERE cuentas.cuenta = sal_cuenta.cuenta NO-LOCK NO-ERROR.
