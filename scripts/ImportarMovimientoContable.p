...
DEFINE TEMP-TABLE movContable
    ...
    FIELD comprobante AS CHARACTER
    FIELD num_documento AS INTEGER
    FIELD suma AS DECIMAL FORMAT "->>>,>>>,>>>"
...
INPUT FROM "c:\INFO_Fodun\Leonardo\MovimientosMedellin.csv".
...
REPEAT:
    ...
    IF DECIMAL(movContable.cuenta) MODULO 100 = 0 THEN
        movContable.cuenta = STRING(DECIMAL(movContable.cuenta) / 100).
    ...
    IF NOT AVAILABLE cuentas THEN DO:
        CASE movContable.cuenta:
            ...
            WHEN "6140103110" THEN DO:
                movContable.cuenta = "6140101110".
                movContable.cen_costos = 003.
            END.

            WHEN "6140103102" THEN DO:
                movContable.cuenta = "6140101102".
                movContable.cen_costos = 003.
            END.

            WHEN "6140103114" THEN DO:
                movContable.cuenta = "6140101114".
                movContable.cen_costos = 003.
            END.

            WHEN "6140104195" THEN DO:
                movContable.cuenta = "6140101195".
                movContable.cen_costos = 004.
            END.

            WHEN "6140103122" THEN DO:
...
FOR EACH movContable WHERE movContable.db + movContable.cr <> 0 AND movContable.cuenta <> "" BREAK BY movContable.cuenta:
    ...
    /* Fecha */
    movCont.fecha = DATE(INTEGER(SUBSTRING(movContable.fecha,5,2)),
                         INTEGER(SUBSTRING(movContable.fecha,7,2)),
                         INTEGER(SUBSTRING(movContable.fecha,1,4))) NO-ERROR.

    /* Comprobante */
    CASE movContable.comprobante:
        WHEN "CAJA" THEN movCont.comprobante = 1.
        WHEN "EGRE" THEN movCont.comprobante = 2.
        WHEN "NBAN" THEN movCont.comprobante = 8.
        OTHERWISE movCont.comprobante = 3.
    END CASE.
    ...
    IF movCont.nit <> "" THEN DO:
        ...
        FIND FIRST anexos WHERE anexos.nit = movCont.nit
                            ...
                            AND anexos.cen_costos = movContable.cen_costos
        ...
        IF NOT AVAILABLE anexos THEN DO:
            ...
            anexos.cen_costos = movContable.cen_costos.
            anexos.agencia = ag.
            anexos.ano = 2011.
        END.

        anexos.db[2] = anexos.db[2] + movCont.db.
        anexos.cr[2] = anexos.cr[2] + movCont.cr.

        /* Anexos13 */
        FIND FIRST anexos13 WHERE anexos13.nit = movCont.nit
                              AND anexos13.cuenta = movCont.cuenta
                              AND anexos13.cen_costos = movContable.cen_costos
        ...
        IF NOT AVAILABLE anexos13 THEN DO:
            ...
            anexos13.cen_costos = movContable.cen_costos.
            anexos13.agencia = ag.
            anexos13.ano = 2011.
        END.

        anexos13.db[2] = anexos13.db[2] + movCont.db.
        anexos13.cr[2] = anexos13.cr[2] + movCont.cr.
    END.
    ...
    /* cen_costos */
    movCont.cen_costos = movContable.cen_costos.
...
FOR EACH movCont NO-LOCK:
    ...
    mov_contable.usuario = mov_contable.usuario.
