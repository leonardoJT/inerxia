...
OUTPUT TO c:\INFO_fodun\veri_sal_cta.csv.
ASSIGN wtitulo = "registro;agencia;Cuenta;Cen_Costos;Db[4]-Cuenta;wdb-Mvto;cr[4]-Cuenta;wcr-Movto".
        DISPLAY wtitulo
...
FOR EACH Sal_Cuenta WHERE Sal_Cuenta.Ano = 2011 NO-LOCK BY Sal_Cuenta.agencia 
    ...
    FOR EACH    Mov_Contable WHERE Mov_Contable.agencia EQ Sal_Cuenta.agencia AND
                ...
                year(Mov_Contable.Fec_Contable) EQ 2011 AND  MONTH(Mov_Contable.Fec_Contable) EQ 4 NO-LOCK:
        ASSIGN wdb = wdb + Mov_Contable.Db 
               wcr = wcr + Mov_Contable.Cr.
    END.
    IF ( Sal_Cuenta.Db[4] NE WDB ) OR  ( Sal_Cuenta.Cr[4] NE WCR)   THEN  DO:
        ...
        ASSIGN wsalida = STRING(wre,"99999999") + ";" + 
               ...
               string(Sal_Cuenta.Db[4], "999999999999999.99") + ";" +
               string(wdb, "999999999999999.99") + ";" +
               string(Sal_Cuenta.cr[4], "999999999999999.99") + ";" +
