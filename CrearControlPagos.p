...
PROCEDURE P-Amortizacion:
    ...
    IF AVAIL(Pro_Creditos) AND Pro_Creditos.Id_PerGracia AND Dia_Gracia GT 0 THEN
        ASSIGN INT_MGracia = ROUND((((vMonto * P_Tasa) / W_NroDias) * Dia_Gracia) / vPlazo,0).
    ...
    ASSIGN W_FecTra = fechaIni
           ...
           W_TabAmor.W_SdoCap = vMonto
           W_CuoTra = vCuota
           W_SdoCapTra = vMonto.

    DO W_PerTra = 0 TO vPlazo BY 1:
        ...
        IF P_Sistema EQ 1 OR P_Sistema EQ 2 THEN
            W_TabAmor.W_Cuota = vCuota.
        ELSE
            IF P_Sistema EQ 3 THEN
                W_TabAmor.W_Cuota = vMonto * P_Tasa.

        ASSIGN W_TabAmor.W_Periodo = W_PerTra.

        IF (W_PerTra = 0) THEN DO:
            ASSIGN W_TabAmor.W_Cuota = 0
                   W_TabAmor.W_Fecha = vFecPrimerPago.

            IF P_TipInt = 2 THEN
                ASSIGN W_TabAmor.W_AboInt = vMonto * P_Tasa.

            fechaAnterior = vFecPrimerPago.
        END.
        ELSE DO:
            ...
            ELSE
                IF P_Sistema EQ 2 THEN DO:
                    ...
                    IF P_TipInt = 1 THEN
                        ASSIGN W_TabAmor.W_AboInt = vCuota - vMonto.

                    ASSIGN W_TabAmor.W_AboCap = vMonto
                           W_TabAmor.W_Fecha = W_FecTra
                ...
                ELSE
                    IF P_Sistema EQ 3 THEN DO:
                        ...
                        IF vPlazo = W_PerTra THEN DO:
                            IF P_TipInt = 1 THEN
                                ASSIGN W_TabAmor.W_Cuota = W_TabAmor.W_Cuota + vMonto.
                            ELSE
                                ASSIGN W_TabAmor.W_Cuota = vMonto W_TabAmor.W_AboInt = 0.
...
PROCEDURE P-AmortCuotaConstante:
    W_TabAmor.W_AboCap = (vMonto / vPlazo).

    IF P_TipInt = 1 THEN
        RUN HCCC IN W_ManFin (INPUT vMonto,
                              INPUT vPlazo,
                              INPUT W_PerTra,
                              INPUT P_Tasa,
    ...
    IF W_PerTra = vPlazo AND W_SdoCapTra <> 0 THEN DO:
        IF W_SdoCapTra < 0 THEN
...
PROCEDURE P-AmortCuotaFija:
    ...
    IF AVAILABLE(Extras) THEN
        W_CuoExT = vCuota + Extras.Vr_CuoExtra.
    ...
    IF P_TipInt = 1 AND W_SdoCapTra GT 0 THEN DO:
        ...
        IF W_TabAmor.W_AboInt > W_CuoExT THEN
            W_TabAmor.W_AboInt = W_CuoExT.
    ...
    IF W_PerTra = vPlazo AND W_TabAmor.W_SdoCap <> 0 THEN DO:
...
PROCEDURE P-AmortizacionGradiente:
    ...
    IF (P_Sistema = 7) THEN DO:
        IF (P_PerDed = 1 OR P_PerDed = 2 OR P_PerDed = 3) THEN DO:
            W_Dia = ((YEAR(W_FecTra) - YEAR(vFecPrimerPago)) * 360 + (MONTH(W_FecTra) - MONTH(vFecPrimerPago)) * 30).
...
PROCEDURE P-AmortPeriodoGracia:
    IF P_Gracia = W_PerTra THEN DO:
        RUN HFDP IN W_ManFin (INPUT vMonto,
                              INPUT P_Tasa,
                              INPUT P_Gracia,
                              OUTPUT W_SdoCapTra).
    ...
    RUN HCCF IN W_ManFin (INPUT vMonto,
                          INPUT vCuota,
...
PROCEDURE Crear_Plan:
    ...
    CREATE PlanPagos.
    ASSIGN PlanPagos.Agencia = vAgencia
           PlanPAgos.Nit = P_Nit
           ...
           PlanPagos.Monto = vMonto
           PlanPagos.Id_PdoMes = 0.

    CREATE CONTROL_pagos.
    ASSIGN control_pagos.Agencia = vAgencia
