...
&Scoped-Define ENABLED-OBJECTS Img_MesF Img_MesI RECT-282 RECT-283 RECT-284 ~
RECT-285 RECT-287 RECT-288 RECT-289 RECT-290 RECT-293 RECT-294 BUTTON-153 ~
BUTTON-155 BUTTON-121 BUTTON-120 Btn_Ejecutar BUTTON-149 E1 BtnDone ~
BUTTON-158 
&Scoped-Define DISPLAYED-OBJECTS W_DiaIni AnoIni W_DiaFin AnoFin E1 
...
/* SETTINGS FOR FILL-IN AnoIni IN FRAME FTes
   NO-ENABLE                                                            */
/* SETTINGS FOR RADIO-SET Rt1 IN FRAME FTes
   NO-DISPLAY NO-ENABLE                                                 */
ASSIGN 
       Rt1:HIDDEN IN FRAME FTes           = TRUE.

/* SETTINGS FOR RADIO-SET Rt2 IN FRAME FTes
   NO-DISPLAY NO-ENABLE                                                 */
ASSIGN 
       Rt2:HIDDEN IN FRAME FTes           = TRUE.

/* SETTINGS FOR RADIO-SET Rt3 IN FRAME FTes
   NO-DISPLAY NO-ENABLE                                                 */
ASSIGN 
       Rt3:HIDDEN IN FRAME FTes           = TRUE.

/* SETTINGS FOR RADIO-SET Rt4 IN FRAME FTes
   NO-DISPLAY NO-ENABLE                                                 */
ASSIGN 
       Rt4:HIDDEN IN FRAME FTes           = TRUE.

/* SETTINGS FOR TOGGLE-BOX T1 IN FRAME FTes
   NO-DISPLAY NO-ENABLE                                                 */
ASSIGN 
       T1:HIDDEN IN FRAME FTes           = TRUE.

/* SETTINGS FOR TOGGLE-BOX T2 IN FRAME FTes
   NO-DISPLAY NO-ENABLE                                                 */
ASSIGN 
       T2:HIDDEN IN FRAME FTes           = TRUE.

/* SETTINGS FOR TOGGLE-BOX T3 IN FRAME FTes
   NO-DISPLAY NO-ENABLE                                                 */
ASSIGN 
       T3:HIDDEN IN FRAME FTes           = TRUE.

/* SETTINGS FOR TOGGLE-BOX T4 IN FRAME FTes
   NO-DISPLAY NO-ENABLE                                                 */
ASSIGN 
       T4:HIDDEN IN FRAME FTes           = TRUE.

/* SETTINGS FOR TOGGLE-BOX T5 IN FRAME FTes
   NO-DISPLAY NO-ENABLE                                                 */
ASSIGN 
       T5:HIDDEN IN FRAME FTes           = TRUE.

/* SETTINGS FOR TOGGLE-BOX T6 IN FRAME FTes
   NO-DISPLAY NO-ENABLE                                                 */
ASSIGN 
       T6:HIDDEN IN FRAME FTes           = TRUE.

/* SETTINGS FOR TOGGLE-BOX Tg_SdoCaja IN FRAME FTes
   NO-DISPLAY NO-ENABLE                                                 */
ASSIGN 
       Tg_SdoCaja:HIDDEN IN FRAME FTes           = TRUE.
...
------------------------------------------------------------------------------*/
  DISPLAY W_DiaIni AnoIni W_DiaFin AnoFin E1 
      WITH FRAME FTes IN WINDOW wWin.
  ENABLE Img_MesF Img_MesI RECT-282 RECT-283 RECT-284 RECT-285 RECT-287 
         RECT-288 RECT-289 RECT-290 RECT-293 RECT-294 BUTTON-153 BUTTON-155 
         BUTTON-121 BUTTON-120 Btn_Ejecutar BUTTON-149 E1 BtnDone BUTTON-158 
      WITH FRAME FTes IN WINDOW wWin.
...
DEFINE VAR TOTSDO LIKE SAL_CUENTA.SAL_INICIAL.
...
FOR EACH Cuentas 
   FIELDS(cuentas.naturaleza Cuentas.Cuenta Cuentas.Nombre Cuentas.Cod_FlujoEfec Cuentas.Car_Efectivo Cuentas.Estado) 
   WHERE (substring(Cuentas.cuenta,1,2)  EQ "11" OR 
          substring(Cuentas.cuenta,1,2)  EQ "13" OR
          substring(Cuentas.cuenta,1,2)  EQ "21" OR
          substring(Cuentas.cuenta,1,2)  EQ "14" OR
          substring(Cuentas.cuenta,1,2)  EQ "31" OR
          substring(Cuentas.cuenta,1,2)  EQ "32" OR
          substring(Cuentas.cuenta,1,2)  EQ "33" ) NO-LOCK BY Cuentas.Cuenta:
      Encabezado = Encabezado + " " + STRING(Cuentas.Nombre,"X(15)") + " ".
      FOR EACH SAL_CUENTA WHERE SAL_CUENTA.CUENTA = CUENTAS.CUENTA and
                                 sal_cuenta.ano   = year(w_fecha) NO-LOCK:
       j = j + 1.
       RUN Progreso.
       ASSIGN TOTsdo = sal_cuenta.sal_inicial.
       DO ix = 1 TO MONTH(w_fecha):
          IF cuentas.naturaleza = "DB" THEN
             ASSIGN TOTsdo = TOTsdo + db[ix] - CR[ix].
          ELSE 
             ASSIGN TOTsdo = TOTsdo - db[ix] + CR[ix].
       END.
       FIND Tmp WHERE Tmp.TCod EQ cuentas.cuenta NO-ERROR.
       IF NOT AVAILABLE Tmp THEN DO:
          CREATE Tmp.
          ASSIGN Tmp.TCod = Cuentas.Cuenta
                 Tmp.TNco = Cuentas.Nombre.
          IF substring(Cuentas.cuenta,1,2)  EQ "11" THEN TMP.TTIP = 1.
          IF substring(Cuentas.cuenta,1,2)  EQ "13" THEN TMP.TTIP = 1.
          IF substring(Cuentas.cuenta,1,2)  EQ "21" THEN TMP.TTIP = 2.
          IF substring(Cuentas.cuenta,1,2)  EQ "14" THEN TMP.TTIP = 3.
          IF substring(Cuentas.cuenta,1,2)  EQ "31" THEN TMP.TTIP = 3.
          IF substring(Cuentas.cuenta,1,2)  EQ "32" THEN TMP.TTIP = 3.
          IF substring(Cuentas.cuenta,1,2)  EQ "33" THEN TMP.TTIP = 3.
       END.
       ASSIGN Tmp.TVal = Tmp.TVal + totsdo.
   END.
END.
...
ASSIGN J = 0 k = 0.
OUTPUT TO value(Listado) PAGED PAGE-SIZE 65 APPEND .
  DISPLAY "COOPERATIVA FINANCIERA JURISCOOP" AT 1 SKIP(1)
          "INFORME DEL DISPONIBLE"       AT 1 SKIP(1)
          "Fecha de Corte : "  FecFin    AT 1 NO-LABEL
          "--------------------------------"  AT 1 
  ...
  ASSIGN TotBan = 0 TotDis = 0.
         DISPLAY 
           SKIP
           "CUENTA         NOMBRE CUENTA                             DISPONIBLE          SALDO"   
           SKIP
           WITH FRAME TFec WIDTH 300 NO-LABELS.
  
  FOR EACH Tmp WHERE NO-LOCK BREAK BY TMP.TTIP  BY SUBSTRING(TMP.TCOD,1,2) BY tmp.TCod :
       
      j = j + 1.
      RUN Progreso.
      
      IF FIRST-OF (SUBSTRING(TMP.TCOD,1,2)) THEN DO:
         IF SUBSTRING(TMP.TCOD,1,2) = "11" THEN  DISPL "Saldos disponible Caja y Bancos "  AT 1 .
         IF SUBSTRING(TMP.TCOD,1,2) = "13" THEN  DISPL "Saldos Inversiones "  AT 1 .
         IF SUBSTRING(TMP.TCOD,1,2) = "21" THEN  DISPL "Saldos Despositos "  AT 1 .
         IF SUBSTRING(TMP.TCOD,1,2) = "14" THEN DO: 
             DISPL "Saldos CARTERA "  AT 1 SKIP.
             IF SUBSTRING(TMP.TCOD,1,4) GT "1480" THEN TMP.TVAL = TMP.TVAL * -1.
         END.
         IF SUBSTRING(TMP.TCOD,1,2) = "31" THEN  DISPL "Saldos APORTES "  AT 1 .
         IF SUBSTRING(TMP.TCOD,1,2) = "32"  THEN  DISPL "Saldos RESERVAS "  AT 1 .
         totdis = 0.
      END.

      
      totdis = totdis + tmp.tval.
      DISPLAY Tmp.tcod tmp.tnco tmp.tval totdis
