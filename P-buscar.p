...
/*{C:\DLC\src\adecomm\adestds.i} */
{incluido\adestds.i}

IF NOT initialized_adestds
THEN RUN adecomm\_adeload.r.    
...
RUN CreateFrameAttr ( INPUT p_Editor:FRAME , {&PW_Find_Text} ).
RUN CreateFrameAttr ( INPUT p_Editor:FRAME , {&PW_Replace_Text} ).
RUN InitAttr ( INPUT p_Editor ).       
...
DO:
  REPEAT ON STOP UNDO, RETRY:
    ...
    CREATE WIDGET-POOL "_ADE_Procedure_Window" PERSISTENT NO-ERROR.
    RUN adecomm\_pwgetwh.r ( INPUT SELF , OUTPUT pw_Window ).
    
    RUN adecomm\_pwgeteh.r ( INPUT pw_Window , OUTPUT pw_Editor ).
