...
DEFINE VAR TPdt AS INTEGER.
DEFINE VAR NuevaCta AS LOGICAL.
DEFINE VAR Puntero AS ROWID.
DEFINE VAR PunteroP AS ROWID.
...
DEFINE VAR W_CueAho AS CHARACTER.
DEFINE VAR W_tasa AS DECIMAL.
DEFINE VAR W_Puntos AS INTEGER.
DEFINE VAR W_Error AS LOGICAL.
DEFINE VAR W_Autorizo AS CHARACTER.
DEFINE VAR W_Total AS DECIMAL.
DEFINE VAR W_Cbte AS INTEGER.
DEFINE VAR W_CtaOpe AS CHARACTER.
DEFINE VAR W_CtaCor AS CHARACTER.
DEFINE VAR W_CodDed AS CHARACTER.
DEFINE VAR W_Comision AS CHARACTER.
DEFINE VAR W_DedNom AS CHARACTER FORMAT "X(20)".
DEFINE VAR W_DedCta AS CHARACTER.
DEFINE VAR W_DedCla AS INTEGER.
DEFINE VAR W_DedVal AS DECIMAL.
DEFINE VAR W_DedValImp AS DECIMAL.
...
"Vencido", 2
     ...
     Ahorros.Estado AT ROW 19.88 COL 54.86 NO-LABEL
          VIEW-AS RADIO-SET VERTICAL
          RADIO-BUTTONS 
                    "Activa", 1,
"Inactiva", 2
...
DO:
  ...
  IF Pro_Ahorros.Id_Tasa EQ 1 THEN DO:
     IF DEC(AT_Monto:SCREEN-VALUE) EQ 0 OR DEC(AT_Plazo:SCREEN-VALUE) EQ 0 THEN DO:
        MESSAGE "El Monto y el Plazo deben ser mayores a cero. Rectifique-1" SKIP "Monto:"
                 DEC(AT_Monto:SCREEN-VALUE) SKIP "Plazo:" DEC(AT_Plazo:SCREEN-VALUE) VIEW-AS ALERT-BOX ERROR.
        RETURN NO-APPLY.
     END. 
        RUN Hallar_Tasa (INPUT Pro_Ahorros.Indicador, INPUT DEC(AT_Monto:SCREEN-VALUE),
                      INPUT DEC(AT_Plazo:SCREEN-VALUE), OUTPUT W_Tasa, OUTPUT W_Puntos).
         ASSIGN Tasa_Efectiva:SCREEN-VALUE IN FRAME F_Ahorros = STRING(w_tasa).
      /*  MESSAGE AT_Monto:SCREEN-VALUE "--" AT_plazo "--" W_tasa "--" W_puntos VIEW-AS ALERT-BOX INFO BUTTON OK.*/
     /*
     RUN Hallar_Tasa (INPUT Pro_Ahorros.Indicador, INPUT DEC(AT_Monto:SCREEN-VALUE),
                      INPUT DEC(AT_Plazo:SCREEN-VALUE), OUTPUT W_Tasa, OUTPUT W_Puntos).     */                    
...
DO:
  ...
  Cmb_TipoProductos:SCREEN-VALUE =Cmb_TipoProductos:ENTRY(1).
     ...
     IF AVAIL Pro_Ahorros THEN DO:
        ...
        ASSIGN Wk_MontoApertura = 0 Wk_Cuota = 0.
...
DO:
    ...
    IF Ahorros.Nit:SCREEN-VALUE = "" THEN DO:
    ...
    IF Ahorros.Cue_Ahorros:SCREEN-VALUE = "" AND Ahorros.Cue_Ahorros:SENSITIVE = YES THEN DO:
    ...
    IF DEC(Ahorros.Tasa:SCREEN-VALUE) LE 0 AND SUBSTR(Cmb_TipoProductos:SCREEN-VALUE,1,1)
            NE "4" AND SUBSTR(Cmb_TipoProductos:SCREEN-VALUE,1,1) NE "1" THEN DO:
        IF Ahorros.Cod_Ahorro LT 24 THEN DO:
           MESSAGE "No se puede matricular una cuenta de Ahorros" SKIP
                "si no se tiene configurada la Tasa. Rectifique!" VIEW-AS ALERT-BOX ERROR.
           RETURN NO-APPLY.
        END.
     END.
     IF DEC(Tasa_Efectiva:SCREEN-VALUE) LE 0 AND SUBSTR(Cmb_TipoProductos:SCREEN-VALUE,1,1)
             NE "4" AND SUBSTR(Cmb_TipoProductos:SCREEN-VALUE,1,1) NE "1" THEN DO: 
        IF Ahorros.Cod_Ahorro LT 24 THEN DO:
          MESSAGE "No se puede matricular una cuenta de Ahorros" SKIP
                  "si no se tiene configurada la Tasa. Rectifique!" VIEW-AS ALERT-BOX ERROR.
     ...
     IF WEdad < 6570 THEN DO:
        IF Ahorros.Id_Tutor:SCREEN-VALUE EQ " " THEN DO:
           MESSAGE "El Titular de la Cuenta es Menor de Edad y no se ha Seleccionado Representante Legal" skip(1)
                   "Fecha de Nacimiento: " Clientes.Fec_Nacimiento skip(1) "Edad: " WEdad VIEW-AS ALERT-BOX.
           RETURN NO-APPLY.
        END.
        IF Ahorros.Id_Tutor:SCREEN-VALUE = "Padre" OR Ahorros.Id_Tutor:SCREEN-VALUE = "Madre" THEN DO:
           FIND FIRST Relaciones WHERE Relaciones.Nit = Clientes.Nit AND Relaciones.Cod_Relacion = 3 AND
                Relaciones.descripcion = Ahorros.Id_Tutor:SCREEN-VALUE AND Relaciones.Estado = 1 NO-LOCK NO-ERROR.
        ...
        ELSE DO:
           ...
           IF NOT AVAIL Relaciones THEN DO:
              MESSAGE "No se Encontro la Relacion Representante Legal Para el Titular de la Cuenta" VIEW-AS ALERT-BOX.
              RETURN NO-APPLY.
           END.
           FIND Rela_Clientes WHERE Rela_Clientes.Nit = Relaciones.Nit_Relacion NO-LOCK NO-ERROR.
           IF NOT AVAIL Rela_Clientes THEN DO:
              MESSAGE "No se ha Encontrado Representante Legal del Titular en la Tabla CLIENTES" VIEW-AS ALERT-BOX.
              RETURN NO-APPLY.
           END.
        END.
     END.
    ...
    run verificaciones. 
     IF SwError THEN DO:
        ...
        IF AVAIL TAhorros
           THEN ASSIGN Ahorros.Exento_3xm:SCREEN-VALUE           = STRING(TAhorros.Exento_3xm)
           ...
           ELSE ASSIGN Ahorros.Exento_3xm:SCREEN-VALUE           = "No"
        ...
        ENABLE Ahorros.Exento_3xm Ahorros.Id_Subsidio_Entidad Ahorros.Ind_Tipo_Subsidio
               Ahorros.Id_Tesoro_Nacional Ahorros.Id_Mesada_Pensional WITH FRAME F_Ahorros.
        RETURN NO-APPLY.
     END.   
     DISABLE Cmb_TipoProductos Cmb_Productos Ahorros.Nit Ahorros.Cue_Ahorros Ahorros.Sdo_MinCta Ahorros.Num_Formato.
      ...
      IF SUBSTR(Cmb_TipoProductos:SCREEN-VALUE,1,1) EQ "1" OR SUBSTR(Cmb_TipoProductos:SCREEN-VALUE,1,1) EQ "4" THEN DO:
       IF DEC(AV_Monto:SCREEN-VALUE) EQ 0 AND Wk_MontoApertura GT 0 AND NOT AVAIL(Ahorros) THEN DO:
           MESSAGE "No se puede matricular una cuenta de Ahorros" SKIP
                   ...
                   "El monto mínimo de apertura debe ser: $" STRING(Wk_MontoApertura,">>,>>>,>>9") VIEW-AS ALERT-BOX ERROR.
           APPLY "entry" TO AV_Monto IN FRAME F_Alavista.
           RETURN NO-APPLY.
        END.
        ELSE
        IF NOT AVAIL(Ahorros) THEN DO:
        /*      MESSAGE "cerca?3"  VIEW-AS ALERT-BOX INFO BUTTON OK.*/
        ...
        IF AV_Cuota:SCREEN-VALUE EQ ? OR AV_Cuota:SCREEN-VALUE EQ "?" THEN AV_Cuota:SCREEN-VALUE = "0".
        IF DEC(AV_Cuota:SCREEN-VALUE) LE 0 AND INTEG(AP_FormaPago:SCREEN-VALUE) GT 1
            AND (SUBSTR(Cmb_TipoProductos:SCREEN-VALUE,1,1) EQ "4" OR SUBSTR(Cmb_TipoProductos:SCREEN-VALUE,1,1) EQ "2")             
            THEN DO:
           MESSAGE "No se puede matricular esta cuenta de Ahorros" SKIP
                   ...
                   "El valor mínimo de cuota es: $" STRING(Wk_Cuota,">>,>>>,>>9") VIEW-AS ALERT-BOX ERROR.
           APPLY "entry" TO AV_Cuota IN FRAME F_Alavista.
           RETURN NO-APPLY.
        END.
        IF DEC(AV_Cuota:SCREEN-VALUE) EQ 0 AND Wk_Cuota GT 0 
            AND (SUBSTR(Cmb_TipoProductos:SCREEN-VALUE,1,1) EQ "4" OR SUBSTR(Cmb_TipoProductos:SCREEN-VALUE,1,1) EQ "2") 
            THEN DO:
           MESSAGE "No se puede matricular esta cuenta de Ahorros" SKIP
        ...
        END.
        ELSE DO:
           IF DEC(AV_Cuota:SCREEN-VALUE) LT Wk_Cuota AND (SUBSTR(Cmb_TipoProductos:SCREEN-VALUE,1,1) EQ "4"
                OR SUBSTR(Cmb_TipoProductos:SCREEN-VALUE,1,1) EQ "2")  THEN DO:
              MESSAGE "El valor mínimo de la cuota digitada es menor al" SKIP
                      "valor mínimo de cuota estipulado para este producto" SKIP
                      "el cual es: $" STRING(Wk_Cuota,">>,>>>,>>9") VIEW-AS ALERT-BOX ERROR.
              APPLY "entry" TO AV_Cuota IN FRAME F_Alavista.
              RETURN NO-APPLY.
           END.
        END.
        IF AP_FormaPago:SCREEN-VALUE EQ "3" AND (AP_AgenciaDebito:SCREEN-VALUE  EQ ""  OR 
           AP_ProductoDebito:SCREEN-VALUE EQ "" OR AP_CuentaDebito:SCREEN-VALUE EQ "") THEN DO:
           MESSAGE "No se puede matricular esta cuenta de Ahorros" SKIP
                   "por que no se ha especificado la cuenta de Ahorros." SKIP
                   "de donde se hará el débito automático de la cuota" SKIP(1)
                   "A continuación podrá escoger la cuenta débito!" VIEW-AS ALERT-BOX ERROR.
  ...
  DO WITH FRAME F_Atermino:
     IF SUBSTR(Cmb_TipoProductos:SCREEN-VALUE,1,1) EQ "3" THEN DO: 
        IF Cmb_PerLiquidacion:SCREEN-VALUE = "01 - Diario" THEN DO:
           MESSAGE "Producto A-Termino no se permite liquidacion Diaria." VIEW-AS ALERT-BOX ERROR.
           RETURN NO-APPLY.
        END.
        IF DEC(AT_Monto:SCREEN-VALUE) EQ 0 THEN DO:
           MESSAGE "No se puede matricular una cuenta de Ahorros" SKIP
                   "si no se ha digitado el monto de Apertura. Rectifique!" VIEW-AS ALERT-BOX ERROR.
           APPLY 'entry' TO AT_Monto.
           RETURN NO-APPLY.
        END.
        IF DEC(AT_Plazo:SCREEN-VALUE) EQ 0 THEN DO:
           MESSAGE "No se puede matricular una cuenta de Ahorros" SKIP
                   "si no se ha digitado el plazo. Rectifique!" VIEW-AS ALERT-BOX ERROR.
           APPLY 'entry' TO AT_Plazo.
           RETURN NO-APPLY.
        END.
        IF AT_Destino_Intereses:SCREEN-VALUE NE "3" AND (AT_AgenciaDestino:SCREEN-VALUE EQ "" OR 
           AT_ProductoDestino:SCREEN-VALUE EQ "" OR AT_CuentaDestino:SCREEN-VALUE EQ "") THEN DO:
           MESSAGE "No se puede matricular una cuenta de Ahorros" SKIP
                   "Si no se ha especificado la cuenta de Ahorros." SKIP
                   "o el crédito al cual se abonaran los intereses" SKIP
                   "Rectifique!" VIEW-AS ALERT-BOX ERROR.
  ...
  DO WITH FRAME F_Contractual:
     ...
     IF SUBSTR(Cmb_TipoProductos:SCREEN-VALUE,1,1) EQ "2" THEN DO: 
        ...
        IF DEC(CT_Cuota:SCREEN-VALUE) EQ 0 THEN DO:
           MESSAGE "No se puede matricular una cuenta de Ahorros" SKIP
                   "si no se ha digitado la Cuota. Rectifique!" VIEW-AS ALERT-BOX ERROR.
        ...
        IF CT_FormaPago:SCREEN-VALUE EQ "3" AND (CT_AgenciaDebito:SCREEN-VALUE  EQ "" OR
           CT_ProductoDebito:SCREEN-VALUE EQ "" OR CT_CuentaDebito:SCREEN-VALUE EQ "" OR 
           CT_FechaDebito:SCREEN-VALUE EQ "") THEN DO:
           MESSAGE "No se puede matricular una cuenta de Ahorros" SKIP
                   "No se ha escogido la cuenta de debito automatico. Rectifique!" VIEW-AS ALERT-BOX ERROR.
      ...
      /*fecha debito*/
        IF CT_FormaPago:SCREEN-VALUE EQ "3" AND DATE(CT_FechaDebito:SCREEN-VALUE) EQ ? THEN DO:
           MESSAGE "Se debe entrar la fecha en la cual se hará" SKIP
                   "el débito automático. Rectifique la fecha!" VIEW-AS ALERT-BOX ERROR.
  ...
  ASSIGN Wk_Cambio1 = "" Wk_Cambio2 = "" Wk_Cambio3 = "" Wk_DatAnt = "" NuevaCta = NO.
  IF Ahorros.Sdo_Dispon GT 0 OR Ahorros.Sdo_Canje   GT 0 OR
     Ahorros.INT_Pagar  GT 0 OR Ahorros.INT_Causado GT 0 THEN
     ASSIGN Ahorros.Estado = 1  Ahorros.Estado:SCREEN-VALUE = "1".
  ENABLE Btn_Consulta Btn_Ingresar Btn_Autorizados Btn_Libretas  WITH FRAME F_Ahorros.
  DISABLE Btn_Deshacer Btn_Salvar WITH FRAME F_Ahorros.
  FIND CURRENT Ahorros NO-LOCK NO-ERROR.
...
DO:
  FIND Pro_Ahorros WHERE 
       Pro_Ahorros.Tip_Ahorro = INT(SUBSTR(Cmb_TipoProductos:SCREEN-VALUE,1,1)) AND
       Pro_Ahorros.Cod_Ahorro = INT(SUBSTR(Cmb_Productos:SCREEN-VALUE IN FRAME F_Ahorros,1,3))
       NO-LOCK NO-ERROR.
  IF AVAIL Pro_Ahorros THEN DO:
     ...
     IF Tpdt EQ 2 THEN DO:
       ...
       END.
          ...
          END.
  ELSE DO:
     MESSAGE "Producto Seleccionado No Encontado" VIEW-AS ALERT-BOX.
     RETURN NO-APPLY.
  END.
...
DO: 
    DEF VAR xtas LIKE Ahorros.tasa.
    IF NOT AVAIL Ahorros OR (AVAIL Ahorros AND Ahorros.Sdo_disponible + ahorros.sdo_canje <> Ahorros.Monto_Apertura) THEN
       MESSAGE "El Registro No Existe. Salvelo Para Poder Imprimirlo o" SKIP(1)
               "El Monto de Apertura es Diferente al Saldo Disponible." VIEW-AS ALERT-BOX.
    IF AVAIL Ahorros AND Ahorros.tip_Ahorro = 3 AND Ahorros.Sdo_disponible + ahorros.sdo_canje = Ahorros.Monto_Apertura THEN DO:
       InputFile = "Formatos\CD - 300.xls".
       RUN Abrir_Excel.
       RUN Imprime_Formato_CDAT.
      /* RUN Cerrar_Excel. */
    END.
/*     xtas = DEC(Tasa_Efectiva:SCREEN-VALUE IN FRAME F_Ahorros ).                                   */
/*     IF Ahorros.tip_Ahorro EQ 3 THEN DO: /*CDAT*/                                                  */
/*        IF AVAIL Ahorros AND Ahorros.Sdo_dispon + Sdo_Canje GT 0 AND                               */
/*           Ahorros.cod_Ahorro EQ 14 AND Ahorros.Fec_Apertura GT (W_Fecha - 60) THEN DO:            */
/*           MESSAGE "Desea Imprimir el Título CDAT" VIEW-AS ALERT-BOX                               */
/*                    QUESTION BUTTONS YES-NO UPDATE Ask AS LOG.                                     */
/*           IF Ask THEN RUN F-Atermino.R (INPUT Ahorros.Nit, INPUT Ahorros.Cue_Ahorros,INPUT xtas). */
/*        END.                                                                                       */
/*        ELSE                                                                                       */
/*        MESSAGE "El Título CDAT debe tener El Valor Consignado y ser de la Fecha," SKIP            */
/*                "              Para poder Imprimirlo" VIEW-AS ALERT-BOX INFO BUTTONS OK.           */
/*     END.                                                                                          */

  IF AVAIL Ahorros AND (Ahorros.Cod_Ahorro = 17 OR Ahorros.Cod_Ahorro = 18) THEN DO:
...
DO:
  DEF VAR viCnt AS INT NO-UNDO. /* contador */
  FIND Pro_Ahorros WHERE 
       Pro_Ahorros.Tip_Ahorro = INT(SUBSTR(Cmb_TipoProductos:SCREEN-VALUE,1,1)) AND
       Pro_Ahorros.Cod_Ahorro = INT(SUBSTR(Cmb_Productos:SCREEN-VALUE IN FRAME F_Ahorros,1,3))
       NO-LOCK NO-ERROR.
  IF NOT AVAIL Pro_Ahorros THEN DO:
     IF SUBSTR(Cmb_TipoProductos:SCREEN-VALUE IN FRAME F_Ahorros,1,1) = ? THEN DO:
     ...
     RETURN NO-APPLY.
  END.
  IF INT(SUBSTR(Cmb_TipoProductos:SCREEN-VALUE,1,1)) = 4  THEN DO:
     FIND FIRST BAhorros WHERE BAhorros.nit EQ Ahorros.nit:SCREEN-VALUE AND
          BAhorros.tip_Ahorro EQ 4  AND (BAhorros.Sdo_Disponible > 0 OR Bahorros.estado EQ 1)  NO-LOCK NO-ERROR.
     IF AVAIL BAhorros THEN DO:
        MESSAGE "Cliente ya Tiene Cuenta de Aportes :" BAhorros.Cue_Ahorros VIEW-AS ALERT-BOX.
        Cmb_Productos:SCREEN-VALUE = Cmb_Productos:ENTRY(1).  
        RUN inicializar_variables.
         APPLY "CHOOSE" TO Btn_Cancelar.
        /*APPLY "entry" TO Cmb_Productos. */
  ...
  DO WITH FRAME F_Ahorros:
     ...
     END.
     ASSIGN vlTpAct = IF Clientes.Tipo_Actividad BEGINS "Independiente" THEN TRUE ELSE FALSE. /* si es independiente entonces true */
   /*GCamacho Abril 17/08 - Validacion para Ahorro permanente*/
/*   IF SUBSTR(Cmb_TipoProductos:SCREEN-VALUE,1,1) EQ "2" AND SUBSTR(Cmb_Productos:SCREEN-VALUE,1,3) EQ "221" THEN DO: */
        IF Clientes.Tipo_Vinculo EQ 1 THEN DO: /* Es Asociado */
         /*  IF NuevaCta THEN DO:
             FIND FIRST BAhorros WHERE BAhorros.nit EQ Ahorros.nit:SCREEN-VALUE AND 
                   BAhorros.tip_Ahorro EQ 2 AND (BAhorros.cod_Ahorro EQ 17 OR
                                                 BAhorros.cod_Ahorro EQ 18 )  NO-LOCK NO-ERROR.
              IF AVAIL BAhorros THEN DO: 
                 MESSAGE "Asociado ya posee cuenta de Ahorro Permanente, no se puede crear otra cuenta."
                          VIEW-AS ALERT-BOX INFO BUTTONS OK TITLE "No se puede crear cuenta".
                 APPLY "CHOOSE" TO Btn_Cancelar.
                 RETURN NO-APPLY.
              END. 
           END. */        
           viCnt = 0.
           FOR EACH BAhorros WHERE BAhorros.nit EQ Ahorros.nit:SCREEN-VALUE AND 
               BAhorros.tip_Ahorro EQ 4  AND BAhorros.estado EQ 1 NO-LOCK:
               viCnt = viCnt + 1.
           END.
           IF viCnt GT 1 THEN DO:
              MESSAGE "Asociado tiene " viCnt " Cuentas de Aporte Obligatorio" SKIP
                      "Por favor depurar." VIEW-AS ALERT-BOX INFO BUTTONS OK.
              APPLY "CHOOSE" TO Btn_Cancelar.
              RETURN NO-APPLY.
           END.
           FIND FIRST BAhorros WHERE BAhorros.nit EQ Ahorros.nit:SCREEN-VALUE AND 
                BAhorros.tip_Ahorro EQ 4 AND( BAhorros.cod_Ahorro EQ 8 OR BAhorros.cod_Ahorro EQ 9 ) AND 
                BAhorros.estado EQ 1 NO-LOCK NO-ERROR.
           IF AVAIL BAhorros THEN DO: /*Tiene Aporte Voluntario */
              MESSAGE "Asociado tiene un saldo de" STRING(BAhorros.Sdo_Disponible,"$>>>,>>>,>>9.99") SKIP
                      "en Aporte voluntario."
                       VIEW-AS ALERT-BOX INFO BUTTONS OK TITLE "Asociado con Aporte Voluntario".
           END.
           RUN getAhorros(NuevaCta). /* GCamacho - Implementacion Ahorro permanente */
        END. /*Clientes.Tipo_Vinculo EQ 1*/
        ELSE DO:                
          /* MESSAGE "Cliente no es asociado. No se puede crear este producto a no asociados"
                    VIEW-AS ALERT-BOX INFO BUTTONS OK TITLE "No Asociado".
           APPLY "CHOOSE" TO Btn_Cancelar.
           RETURN NO-APPLY.  */
        END. /*ELSE DO:*/
     END. /*IF SUBSTR(Cmb_TipoProductos:SCREEN-VALUE,1,1) EQ "2" AND ....*/
   /*lista clinton ojo*/
     FIND FIRST ListaNegra WHERE ListaNegra.Nit EQ Clientes.Nit NO-ERROR.
     IF AVAIL ListaNegra THEN DO:
        ASSIGN ListaNegra.Id_HaVenido = YES ListaNegra.Fec_HaVenido = W_Fecha.
     ...
     IF Clientes.Estado EQ 2 AND Clientes.Fec_Retiro NE ? THEN DO:
        MESSAGE "No se pueden crear cuentas de Ahorro para clientes Retirados" VIEW-AS ALERT-BOX WARNING.
        APPLY "CHOOSE" TO Btn_Cancelar.
        RETURN NO-APPLY.
     END.
   /*  IF Clientes.Tipo_Vinculo GT 2 THEN DO:
     ...
     IF Pro_Ahorros.Id_Asociado EQ 2 AND Clientes.Tipo_Vinculo EQ 2 THEN DO:
        MESSAGE "Este producto es Exclusivo para Asociados de la Cooperativa" SKIP
                ...
                "Rectifique...! Si desea Continuar, recuerde que debe consignarle los APORTES," SKIP
                "Luego de la Apertura." VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO
                 TITLE "Confirmar crear Aportes" UPDATE W_SiAporte AS LOGICAL.
        IF NOT W_SiAporte THEN DO:
           APPLY 'CHOOSE' TO Btn_Cancelar.
           RETURN NO-APPLY.
        END.
     END.
     ...
     /* APPLY 'CHOOSE' TO Btn_Cancelar.*/
        ASSIGN Ahorros.Nit:SCREEN-VALUE = "" W_NomTitular:SCREEN-VALUE = "".
        Cmb_Productos:SCREEN-VALUE = Cmb_Productos:ENTRY(1).
        APPLY "entry" TO Cmb_Productos.
        RETURN NO-APPLY.
     END.      
     ...
     /* APPLY 'CHOOSE' TO Btn_Cancelar.*/
        ...
        RETURN NO-APPLY.
     END.
     IF INT(SUBSTR(Cmb_TipoProductos:SCREEN-VALUE,1,1)) EQ 4 THEN DO:
       R_Eleccion:SCREEN-VALUE IN FRAME F_Relaciones = "4".
       APPLY 'value-changed' TO R_Eleccion IN FRAME F_Relaciones.
        FIND FIRST Ahorros WHERE Ahorros.Tip_Ahorro EQ 4 AND
             Ahorros.Nit EQ SELF:SCREEN-VALUE AND Ahorros.Estado EQ 1 NO-LOCK NO-ERROR.
      /*IF AVAIL Ahorros THEN DO:
          MESSAGE "No se puede crear otra cuenta de aportes" SKIP
                  "el cliente ya tiene una cuenta de este producto" SKIP
                  "creada. se cancela la operación de ingreso!" VIEW-AS ALERT-BOX ERROR.
          APPLY "CHOOSE" TO Btn_Cancelar IN FRAME F_Ahorros.
          RETURN NO-APPLY.
        END.*/
        IF NuevaCta THEN RUN Apertura.
     END.
/*   ELSE
     IF INT(SUBSTR(Cmb_TipoProductos:SCREEN-VALUE,1,1)) EQ 1 THEN DO:
        FIND FIRST Ahorros WHERE Ahorros.Nit        EQ SELF:SCREEN-VALUE AND
                                 Ahorros.Tip_Ahorro EQ 1                 AND
                                 Ahorros.Exento_3xM                      AND
                                 Ahorros.Estado     EQ 1 NO-LOCK NO-ERROR.
        IF NOT AVAIL Ahorros THEN 
           ASSIGN Ahorros.Exento_3xM:SCREEN-VALUE = "Yes".
     END.            */
     RELEASE Ahorros. 
        
        /*
     IF WEdad < 18 AND Pro_Ahorros.Id_Asociado EQ 2 THEN DO:
        ENABLE Ahorros.Id_Tutor  WITH FRAME F_Ahorros.
        ENABLE Ahorros.Nit_Tutor WITH FRAME F_Ahorros.
       ASSIGN Btn_Libretas:VISIBLE = TRUE.
       ENABLE Btn_Libretas.
       ASSIGN  tgl-TarjetaDB:VISIBLE = FALSE.
       ASSIGN  Ahorros.TarjetaDB:VISIBLE = FALSE.
       ASSIGN  wcomenTdb:VISIBLE = FALSE.
     END.        
     ELSE DO:
        ASSIGN Ahorros.Id_Tutor:SCREEN-VALUE = "" Ahorros.Nit_Tutor:SCREEN-VALUE = "" Nombre_Tutor:SCREEN-VALUE = "".
        DISABLE Ahorros.Id_Tutor  WITH FRAME F_Ahorros.
        DISABLE Ahorros.Nit_Tutor WITH FRAME F_Ahorros.
       ASSIGN  tgl-TarjetaDB:VISIBLE = TRUE.
       ASSIGN  Ahorros.TarjetaDB:VISIBLE = TRUE.
       ASSIGN  wcomenTdb:VISIBLE = TRUE.
     END.
        */
      DISABLE ahorros.num_formato ahorros.sdo_mincta WITH FRAME f_ahorros.
    IF INT(SUBSTR(Cmb_TipoProductos:SCREEN-VALUE,1,1)) = 3  THEN DO:          
...
DEFINE VAR tippro AS INTEGER.

FIND FIRST Pro_Ahorros WHERE Pro_Ahorros.Tip_Ahorro = INTEGER(SUBSTRING(Cmb_TipoProductos:SCREEN-VALUE IN FRAME F_Ahorros,1,1))
                         AND Pro_Ahorros.Cod_Ahorro = INTEGER(SUBSTRING(Cmb_Productos:SCREEN-VALUE IN FRAME F_Ahorros,1,3)) NO-LOCK NO-ERROR.

/* oakley */

ASSIGN tippro = INT(SUBSTR(Cmb_TipoProductos:SCREEN-VALUE IN FRAME F_Ahorros,1,1)). 
IF NuevaCta THEN DO:
     CREATE Ahorros.
     IF AVAIL Pro_Ahorros THEN DO:
        ...
        ELSE Ahorros.Cue_Ahorros = Ahorros.Cue_Ahorros:SCREEN-VALUE.
     ...
     IF SUBSTR(Cmb_TipoProductos:SCREEN-VALUE,1,1) LE "2" THEN DO:
        ...
        IF SUBSTR(Cmb_Perliquidacion:SCREEN-VALUE,1,2) EQ "02" OR
           SUBSTR(Cmb_Perliquidacion:SCREEN-VALUE,1,2) EQ "03" THEN RUN FProxLiqAV.
     END.
  END.
  FIND CURRENT Ahorros NO-ERROR.
  DO WITH FRAME F_Relaciones.
  /* JJMP - Controles TRF */
     IF NOT NuevaCta THEN ASSIGN Ahorros.trf = INT(SUBSTR(Cmb_TRF:SCREEN-VALUE,1,2))
                                 Ahorros.TRF_notas = CAPS(TRIM(W-DescTRF:SCREEN-VALUE)).
  END.
  DO WITH FRAME F_Ahorros.
     ASSIGN t-execobro.
...
ELSE
  IF AVAIL pro_Ahorros AND Pro_Ahorros.Tip_Ahorro = 1  THEN DO:
        ...
        ENABLE Btn_Libretas ahorros.Id_Subsidio_Entidad Ahorros.Exento_3xm  
                Ahorros.Id_tesoro_Nacional
              Ahorros.Id_Mesada_pensional Ahorros.Ind_tipo_Subsidio.          
        IF (Pro_Ahorros.Cod_Ahorro = 1 OR Pro_Ahorros.Cod_Ahorro = 3) THEN DO:
               ...
               IF NOT nuevacta THEN
                   ASSIGN Ahorros.tarjetaDB:SCREEN-VALUE IN FRAME f_Ahorros = Ahorros.TarjetaDB.
        END.
            ELSE   DO:          
...
IF AVAIL Pro_Ahorros AND NuevaCta THEN DO:     
   ...
   IF Pro_Ahorros.Id_Salminimo THEN DO:
      ...
      IF NOT Pro_Ahorros.Tip_Salminimo THEN Ahorros.Sdo_MinCta:SENSITIVE = TRUE.
   END.
   IF Pro_Ahorros.Id_Consecutivo THEN 
...
END.
 ...
 ASSIGN Tpdt                   = Pro_Ahorros.Tip_Ahorro.
  DO  WITH FRAME f_ahorros:
       IF Tpdt EQ 3 THEN ASSIGN Cmb_Perliquidacion:SENSITIVE = TRUE.
     
       IF (Tpdt EQ 1 OR Tpdt EQ 5)  THEN DO:
       ASSIGN 
           Ahorros.Exento_3xm:VISIBLE           = TRUE
...
/* IF AVAIL Ahorros AND AVAIL Pro_Ahorros THEN DO: */
  ...
  CASE Tpdt:
      WHEN 2 THEN DO:
        HIDE FRAME F_ALaVista FRAME F_Atermino.
        VIEW FRAME F_Contractual.
        DO WITH FRAME F_Contractual:
            IF AVAIL Ahorros AND AVAIL Pro_Ahorros AND NOT NuevaCta THEN RUN Mostrar_Contractual.
            /*Gcamacho - Implementacion Ahorro permanente */
            IF AVAIL Pro_Ahorros AND Pro_Ahorros.tip_Ahorro EQ 2 AND 
                (Pro_Ahorros.cod_Ahorro EQ 17  OR Pro_Ahorros.cod_Ahorro EQ 18
                 OR Pro_Ahorros.cod_Ahorro EQ 38) THEN DO:
            ...
            END.
            ENABLE CT_Cuota CT_FormaPago.
            IF CT_FormaPago:SCREEN-VALUE EQ "3" THEN ENABLE BT_Debito CT_FechaDebito.             
            IF NuevaCta THEN DO: 
               ENABLE CT_Monto CT_Cuota CT_Plazo porce_aporte CT_FormaPago.
      ...
      END.
      WHEN 3 THEN DO:
        HIDE FRAME F_Contractual FRAME F_ALaVista.
        VIEW FRAME F_Atermino.
        DO WITH FRAME F_Atermino:
            IF AVAIL Ahorros AND AVAIL Pro_Ahorros AND NOT NuevaCta THEN RUN Mostrar_Atermino.
            HIDE AT_VerTasa.
            DISABLE Tasa_Efectiva WITH FRAME F_Ahorros.
            ENABLE AT_Destino_Intereses.
            IF INTEGER(AT_Destino_Intereses:SCREEN-VALUE) LT 3 THEN ENABLE  Bt_DesAT.
                                                               ELSE DISABLE Bt_DesAT.
            IF NuevaCta THEN DO:
               ENABLE AT_Monto AT_Plazo.
               IF AVAIL Pro_Ahorros AND Pro_Ahorros.Id_Tasa EQ 1 THEN DO:
                  ASSIGN Tasa_Efectiva:BGCOL = 18
