...
namespace CreditosAPI.Domain
{
    public class ODBCConecction
    {
        ...
        const string cedulaInexistente = "La cédula especificada no existe";

        const string nitInexistente = "El nit especificado no existe";
        ...
        public Response GetAsociado(string nit)
        {
            try
            {
                ...
                if (rows == 0) {
                    return new Response(1, cedulaInexistente);
                }
                return new Response(0, JSONString);
            }
            catch (System.Exception ex)
            {
                return new Response(2, $"{ex.Message} - {ex.StackTrace}");
            }
        }
        ...
        public Response GetData(string tipo, string cuenta, string nit)
        {
            try
            {
                ...
                JSONString = JsonConvert.SerializeObject(dt);
                if (rows == 0) {
                    return new Response(1, nitInexistente);
                }
                return new Response(0, JSONString);
            }
            catch (System.Exception ex)
