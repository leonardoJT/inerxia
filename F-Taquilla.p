...
DEFINE VAR W_Comentario AS CHARACTER FORMAT "X(48)".
...
IF NOT AVAILABLE(Taquilla) THEN DO:
    MESSAGE "No se encuentra en taquilla el documento(Nro_Transaccion) :" Taquilla.Nro_Transaccion SKIP
            "que desea imprimir."
...
IF AVAILABLE(Usuarios) THEN
  ...
  IF AVAILABLE(agencia) THEN DO:
   W_NomOfi = STRING(Agencia.Agencia,"999")  + " - " + CAPS(Agencia.Nombre).
   FIND Entidad WHERE Entidad.Entidad EQ Agencia.Entidad NO-LOCK NO-ERROR.
   IF AVAILABLE(Entidad) THEN DO:
      ASSIGN W_ConcatEnti   = entidad.nit + " - " + CAPS(TRIM(entidad.nombre)).
   END.
  END.
  ...
  DEFINE FRAME F-Encabezado
    HEADER
        "Entidad              : "       AT COL 3 ROW 1
        W_ConCatenti                    AT COL 26  ROW 1
        "Tipo de Documento    : COPROBANTE DE TAQUILLA"     AT COL 3  ROW 2
        "Fecha  :"                    AT COL 88 ROW 2
        TODAY                         AT COL 97 ROW 2
       
        "Oficina Nro          :"      AT COL 3  ROW 3
        W_NomOfi                      AT COL 26 ROW 3
        "Hora   :   "                 AT COL 88 ROW 3 STRING(TIME,"HH:MM:SS") SKIP
       
        "Nro Transacción      :"       AT COL 3  ROW 4
        TRIM(STRING(P_Transa,"99999999"))  AT COL 26 ROW 4        
        "Pagina :            "         AT COL 88 ROW 4 PAGE-NUMBER FORMAT ">9"
        ...
        "------------------------------------------------------------------------------------------------------------------" AT COL 3 ROW 7
        "Comentario           :"      AT COL 3  ROW 8
        P_Comentario                  AT COL 26 ROW 8 SKIP(1)
        "__________________________________________________________________________________________________________________" AT COL 3 ROW 9
        "Cuenta"                      AT COL 3  ROW 10
        "Nit"                         AT COL 16 ROW 10
        "Nombre"                      AT COL 29 ROW 10 
        "Descripcion"                 AT COL 55 ROW 10
       /*  "Consigno"                  AT COL 60 ROW 8*/
        "V A L O R "                  AT COL 108 ROW 11 /*skip(1)*/
  
  WITH DOWN WIDTH 300 FRAME F-Encabezado NO-LABEL NO-BOX NO-UNDERLINE PAGE-TOP 
                              USE-TEXT.

  DEFINE FRAME F_Movimiento
     Taquilla.Nro_cuenta      AT 3   FORM "X(12)"
     Taquilla.Nit             AT 16  FORM "X(12)"
     W_NomCli                 AT 29  FORMAT "X(25)"
     w_Comentario             AT 55  
     W_VrOpera                AT 103
  ...
  DEFINE FRAME F-Totales
    HEADER
    "Total Operación "                      AT COL  80 ROW 1
  ...
  Listado = W_PathSpl + W_Usuario + "L_Ingr.Lst".
...
/*        w_VrOpera */
      ...
      IF  SUBSTRING(Taquilla.Cta_Contra,1,4) NE "1904"
      AND SUBSTRING(Taquilla.Cta_Contra,1,4) NE "2705" THEN
