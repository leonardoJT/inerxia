...
FOR EACH Cuentas WHERE Cuentas.Id_Cuenta = 1 AND Cuentas.Cam_CapTrabajo  < 3 AND Cuentas.Tipo = 2:
...
END.
W_TotPor = 0.
FOR EACH Cuentas WHERE Cuentas.Id_Cuenta = 2 AND Cuentas.Cam_CapTrabajo < 3 AND Cuentas.Tipo = 2:
 RUN SaldosCuenta.p(INPUT Cuentas.Cuenta, INPUT W_Ano, INPUT W_Age,INPUT W_Age1, INPUT Cuentas.Naturaleza, INPUT W_Mes, OUTPUT W_TotPor).
...
ASSIGN W_Resultado = STRING(W_TotPor,'->>>,>99.99')  
       ...
       W_Operando = '/'
       W_Cuenta = 'Id_Cuenta = 1 / Id_Cuenta = 2'. 
