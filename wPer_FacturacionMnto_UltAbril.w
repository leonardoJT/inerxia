...
------------------------------------------------------------------------------*/
  ...
  CASE currentPage: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN constructObject (
             INPUT  'dfacturacion.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_dfacturacion ).
       /* Position in AB:  ( 1.27 , 95.00 ) */
      ...
      FOR EACH tmp_Enc_Extracto:
         ...
         IF AVAILABLE(creditos) THEN DO:
           ...
           IF Facturacion.fec_mora NE ? AND Facturacion.val_atrasokpen GT 0 AND facturacion.sdo_mora GT 0 THEN DO:
              ...
              ASSIGN Creditos.Val_Atraso  = Facturacion.val_atrasokpen
                     creditos.Dias_Atraso = (w_fecha + 1) - Facturacion.Fec_mora.
/*               ASSIGN creditos.Dias_Atraso = Facturacion.Fec_mora - (w_fecha - 1).  */
/*           ELSE                                                                     */
/*              ASSIGN creditos.Dias_Atraso = (w_fecha + 1) - Facturacion.Fec_mora.   */
           END.
           ELSE
...
DEFINE VARIABLE vdfecmora AS DATE INITIAL ? NO-UNDO.
...
------------------------------------------------------------------------------*/
    ...
    IF AVAILABLE(per_facturacion) THEN  DO: /* Nueva Facturacion */ 
       ...
       IF w_oficial = TRUE THEN DO:
          ...
          /*IF wfec_vcto EQ W_Fecha  THEN DO.*/
          IF (((wfec_vcto    EQ W_Fecha) AND (vthora GE 16 AND vthora LE 17)) /*(vthora GE 23 AND vthora LE 24))*/ OR
...
/*            FIND LAST mov_creditos WHERE mov_creditos.agencia     EQ creditos.agencia     AND                     */
/*                                        (mov_creditos.cod_credito EQ 570 OR mov_creditos.cod_credito EQ 870) AND  */
/*                                         mov_creditos.Num_credito EQ creditos.Num_credito AND                     */
/*                                         mov_creditos.fecha       LT wfecini /*NO Ojo debe ser wfecfin */         */
/*                                         NO-LOCK NO-ERROR.                                                        */
...
/*                 Temcreditos.TNumcred EQ mov_creditos.num_credito   NO-ERROR.                                                                  */
